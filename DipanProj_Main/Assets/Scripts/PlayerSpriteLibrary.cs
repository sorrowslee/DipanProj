...
public class PlayerSpriteLibrary
{
    ...
    //      （Bloodseeker attack 是蹲姿，高度比 idle 矮 → 被放大 1.036，實際體積已經是 idle 的 1.12 倍）。
    ...
    // 實測七組血統（2026-08-27）attack 的縮放：Base 0.917、Jiangshi 0.945、Bloodseeker 0.961，其餘 0.99~1.06；
    // walk 與舊高度法幾乎相同（Bloodseeker walk 三種指標一致地小 13%，舊法本來就對）。
    //
    ...
    // 實測 Bloodseeker / 旱魃 / Base 的第 1 幀就已經是伸手施法的姿勢，Cain 與 Crimson Count
    ...
    // 實測峰值 Cain 只有 25%、Bloodseeker 有 86%。任何絕對門檻都會對其中一邊失效：
    // 門檻設高，Cain 整段都達不到（起播幀退回 0，等於沒修）；門檻設低，Bloodseeker 的第 1 幀就過關
