...
### 10.0 ⭐ 近戰分兩種（作者 2026-09-18 拍板的分類）

| 型 | 行為 | Brain | 判準 |
|---|---|---|---|
| **衝撞型** | 一路貼上去磨，傷害走接觸傷害 | `ChaseBrain`（`BrainType=Chase`） | **沒有 `attack/` 圖** |
| **揮舞型** | 追到定點 → **站定播完攻擊動畫** → 才能再移動，傷害在命中幀結算 | `MeleeChaseBrain`（`BrainType=MeleeChase`） | **有 `attack/` 圖** |

**判準就是「有沒有 attack 圖」**，因為那正好是這隻怪「有沒有揮武器這個動作」的定義。
現況：紅嫁衣的家人幽靈 12 隻與 ZhaYu 都沒有 attack 圖 ⇒ 天生是衝撞型；
吸血鬼兵（25 張）、狼人兵（24 張）、狂族皇家衛士（12 張）有 ⇒ 揮舞型。

⚠ **把沒有 attack 圖的怪填成 `MeleeChase` 會讓牠完全無害**：本 Brain 會關掉「碰到就痛」、
改由揮擊的命中幀結算，而沒有 attack 圖就永遠不會揮。所以 `EnsureConfigured` 偵測到沒圖時
**整段退回衝撞型並印一則 warning**——不會靜靜地變成一隻不會傷人的怪。

> 跳躍踐踏型（§9）是「揮舞型 ＋ 開場一次跳躍」：`LeapSlamBrain` 跳完之後就是委派給 `MeleeChaseBrain`。
...
| 參數 | 值 | 說明 |
|---|---|---|
| `AttackHitFrame` | **CSV 欄** | 武器揮到位的那一幀。**幀號即事件**，改 `AnimFPS` 自動跟著對。留空＝張數 × 0.7 粗估 |
...
⚠⚠ **命中幀一定要逐怪量，比例完全靠不住**（2026-09-18 加第二、三隻揮舞型怪時發現）：

| 怪 | attack 張數 | 命中幀 | 佔比 |
|---|---|---|---|
| 狂族皇家衛士 | 12 | 9 | **75%** |
| 狼人兵 | 24 | 8（左揮最寬 218；幀 15 還有第二段右揮） | **33%** |
| 吸血鬼兵 | 25 | 11（武器衝出畫布右緣） | **44%** |

三隻從 33% 到 75%，任何「乘個比例」的猜法都會在某一隻上完全錯掉。
**量法**：印出 attack 每一幀不透明像素的 bbox，**寬度／邊緣突然暴增的那一幀**就是武器揮出去的時刻
（一行 python + PIL 的 `getbbox()`，比在 Unity 裡一張張點快得多）。

所以 `HitFrame` 已經從寫死的常數搬進 **`MonsterData.csv` 的 `AttackHitFrame` 欄**。
留空時退回「張數 × 0.7」只是讓沒量過的新怪至少能動，**不要依賴它**。
值一律夾進實際張數，免得填太大時一輩子揮空不傷人。

⚠ **一次攻擊只結算一個命中幀**。狼人兵其實是左右兩段連擊（幀 8 與 15），目前只認第一段；
要做連擊得把 `AttackHitFrame` 改成可以填多個（還沒做）。
...
## 11. 自爆型（炸彈怪／爆裂蟲…）— `SuicideBombBrain`

> **狀態：✅ 程式完成（2026-09-22），⏳ 未編譯、未實機驗證。** 第一隻使用者：**怪物 22「ZhaYu_Bomb」**（Main）。

`ChaseBrain` 是貼上去「磨」（每 `AttackInterval` 扣一次接觸傷害），這支是貼上去「**一次結算、同歸於盡**」。
`BrainType` 填 `SuicideBomb`。

### 11.1 三段

| 階段 | 行為 | 常數／CSV |
|---|---|---|
| `Chase` | 走向目標；**兩者水平半徑和 ＋ `FuseSlack`(0.05)** 的中心距離內就點引信 | `FuseSlack` 0.05 |
| `Fuse` | **站定**閃紅（越接近爆炸閃越快）＝玩家的逃命窗口。**沒有任何中途取消** | CSV `BombFuse`，空＝0.6 秒 |
| `Done` | 已引爆，等這一幀結束被銷毀 | — |

⚠⚠ **引信判定刻意「不」用 `Physics2D.Distance` 的框對框邊緣距離**（2026-09-22 改）：
身體框貼合整個可見身體 ⇒ 它的**高度就是身高**，而俯視角的 Y 軸同時是地面深度
⇒ 玩家從上／下方接近時，離怪還有一個半身位就已經算「碰到框」⇒ 畫面上是
**「怪離很遠就停下來自爆」**（作者 2026-09-22 回報）。
改成**只取水平半徑的圓形距離**（`兩者 bounds.extents.x 之和 ＋ FuseSlack`）：各方向門檻一致，
而且不受這隻怪畫得多高影響。半徑取自碰撞框（已含 `Scale`）⇒ **體型變大時門檻自動跟著長**（同 §11.4b）。

實測 ZhaYu_Bomb `Scale` 1.5：水平 2.43／垂直 2.46 → **一律 2.13**（靠近約 0.3 世界單位），
此時怪的「圖」邊緣離玩家碰撞圓只剩 0.20 ⇒ 視覺上就是走到身邊才點火。
**想讓它更早／更晚引爆，調 `FuseSlack` 就好**（放大＝更早、更難閃）。

### 11.2 ⭐ 鐵則：進了引信就一定會爆

照抄 §8 射手型的「出手就一定要射」。`Fuse` 階段不重驗距離、不看目標還在不在——
玩家跑掉了、被擊退推開了，都照爆。

**為什麼**：一隻「嗶嗶叫了兩聲又把引信收回去」的炸彈怪，在畫面上看起來就是壞掉的怪。
玩家的應對是**閃開爆炸範圍**或**在引信燒完前把牠打死**，不是讓牠自己放棄。

**逐項確認過的中斷來源**（2026-09-22 作者要求再檢一次）：

| 來源 | 會不會中斷 | 為什麼 |
|---|---|---|
| 玩家跑出範圍 | ❌ | `Fuse` 不重驗距離 |
| 玩家不見了（死亡／換圖） | ❌ | 引爆完全不讀 target，只用自己的位置 |
| 被擊退 | ❌ | 擊退窗口會整段跳過 `Think()`（**F19**），但引信走 `Time.time`、時間照跑 ⇒ 窗口結束的第一幀就引爆 |
| 被打（沒死） | ❌ | 受擊只重置受擊自己的視覺——`HitReactionHandler.ResetVisuals` **刻意不清** `_chargeAmount` |
| **被打死** | ✅ | **唯一的例外**，設計上留給玩家的拆彈窗口（見 §11.3） |

### 11.3 被玩家打死不會爆（而且不用寫特例）

作者 2026-09-22 拍板：**拆彈成功應該有獎勵**。
實作上這是**天生行為**——爆炸只發生在引信燒完的那一刻，而怪一死 `MonsterController.Update` 就提前 return、
Brain 不再被呼叫。一行特例都不用寫。

想改成「打死也炸」的話不要在 Brain 裡加判斷，那條路走不到；要在 `MonsterController.Die()` 掛。

### 11.4 ⚠⚠ 自毀不能用 `TakeDamage(超大數字)`

`TakeDamage` 會先問 `HitReactionHandler.TryHitReaction`，**怪正處在無敵幀內就直接 return false、一滴血都不扣**
⇒ 變成「爆炸放了、怪沒死」，而 Brain 已經進了 `Done` ⇒ 牠會站在原地不動、不再追人也不再爆。

所以 `MonsterController` 加了 **`Kill()`**（直接走 `Die()`，略過受擊反應；死亡特效／掉落／死亡旗標／
RunProgress『已清』／召喚回收**全部照跑**）。

> ⭐ **通則：一個「一定要成功」的自毀，不該把成敗交給另一條會拒絕它的路徑。**
> 目前自爆怪的 `InvincibleTimeMs` 填 0 所以碰不到，但那是 CSV 隨時可以改的值——
> 「自爆會不會成功」不該取決於另一個欄位（同 PROBLEMS **F19**「無敵時間是一份隱形的行為預算」）。

### 11.4b ⚠ 半徑與門檻都要隨體型縮放（F29）

`BombRadius` 是「**體型 1 時**的半徑」，實際走 `MonsterController.ScaledRadius()` ×`Scale`。
引信門檻也取自碰撞框（已含 `Scale`）。**兩者同步變動**，所以放大縮小怪都不必回來改常數。

再加一層**防呆下限**：`radius ≥ 自身水平半徑 ＋ FuseSlack` ⇒ **引信點著就一定炸得到**
（目標的半徑在觸發與殺傷兩邊都有、自動抵銷，所以這條不必知道玩家多大）。
這是「進了引信就一定會爆」的必然延伸——**爆了卻炸不到，等於沒爆**。
完整記錄與踩坑過程見 [PROBLEMS.md](PROBLEMS.md) **F29**。

### 11.5 傷害與視覺

- **傷害**：`ImpactDamageArea.Spawn`（＝§9.4 踐踏／§10.4 揮擊同一支）。零新傷害程式，
  誰能傷誰查 `FactionRelations`（所以自爆也會炸到敵對陣營的怪）。
  ⚠ **實際殺傷 ＝ `BombRadius` ＋ 目標碰撞框半徑**（玩家約 0.5）。
- **接觸傷害預設關掉**（`DisableContactDamage = true`，同 `MeleeChaseBrain`）：
  自爆怪擦過你身邊不該扣血，牠的全部威脅就是那一下爆炸。
- ⚠ **判定圈用怪的 `transform`、視覺用腳底**（`BodyCenterWorldPos`）——兩者刻意不同，理由同 §9.4 與
  PROBLEMS **F27**：玩家的碰撞圓 offset 是 0（圓心＝transform），而 route B 怪物 pivot 在畫布中心、
  腳底離 transform 有半個身高，**那個落差隨 `Scale` 線性長大** ⇒ 判定圈以腳底為心的話，怪一放大就炸不到人。
...
- **引信視覺 ＝ 整隻怪逐漸燒紅 ＋ 脈動越來越急**，是玩家**唯一**的預告
  ...
  自爆怪沒有 attack 圖、沒有舉手動作，拿掉這個就變成「走過來然後你就死了」。
  ⚠ 用**方波**不是 sin：要的是「滴、滴、滴」的斷點感，平滑呼吸沒有緊迫性。
- ⭐ **發光走 shader 的加法通道，不是 `SpriteRenderer.color`**（2026-09-22）：
  `Custom/SpriteFlash` 新增 `_ChargeAmount`／`_ChargeColor`（預設 0 ＝ 那一行不執行，其他角色逐位元無變化），
  由 **`HitReactionHandler.SetCharge()`** 寫入。兩個理由，都踩過才知道：<br>
  ① 這些怪本體幾乎全黑，**乘法 tint（黑 × 紅 ＝ 黑）根本看不出來**，只有加亮才會「燒起來」；<br>
  ② `SpriteRenderer.color` 會被**受擊無敵閃爍**改成半透明再還原成 `_originalColor`
  ⇒ 引信顏色會被一起洗掉。而且 `HitReactionHandler.ApplyPropertyBlock` 是這個 renderer
  **唯一的 MPB 寫入點**（`SetPropertyBlock` 是整包覆蓋的），自己另外寫會把環境融合參數沖掉。
  **要在角色身上加任何 shader 效果，一律從那支加入口，不要另開一條路。**

### 11.6 加一隻新的自爆型怪（SOP）

| # | 做什麼 | 備註 |
|---|---|---|
| 1 | 素材丟 `Monsters/SequenceImage/<怪名>/idle｜walk` | **不需要 attack**（自爆沒有出手動作） |
| 2 | `MonsterData.csv` 加一列：`BrainType=SuicideBomb`、`Weapon=Contact` | `AnimFPS` 依張數算 |
| 3 | 填表尾三欄 `BombDamage`／`BombRadius`／`BombFuse` | 留空＝`ContactDamage`×3／1.8／0.6 秒 |
| 4 | `Project Tools → 角色 → 計算影子錨點` ＋ `Sync Map Assets` | 同其他 route B 怪 |

**手感參數是全域共用的**（`SuicideBombBrain` 檔頭的 `const`）：引信距離 `FuseSlack`、閃爍節奏、特效 ID。
會自動跟著怪走的只有「邊緣距離」（框對框算，體型多大都對）與 CSV 那三欄。

### 11.7 待辦 / 已知限制

- [ ] **未編譯、未實機驗證**。實測看：引信 0.6 秒夠不夠閃、`FuseSlack`(0.35) 會不會離太遠就點火、
      `Speed 3.2` 追不追得到玩家（玩家 5）。
- [ ] **沒有「引信中被擊退就延後」**：擊退期間整段 `Think()` 被跳過（**F19**），但引信用的是 `Time.time`
      ⇒ 被打飛的途中照樣爆。目前看起來合理（炸彈被打當然更容易炸），真的難打再說。
- [ ] **爆炸不會連鎖**：兩隻自爆怪站在一起，A 爆炸把 B 炸死 ⇒ B 是「被打死」⇒ 不會跟著爆。
      要連鎖得在 `Die()` 掛，見 §11.3。
- [ ] **引信閃爍會和受擊白光搶顏色**：兩邊都在寫 `SpriteRenderer.color`／flash。
      引信期間被打時顏色會互搶一下，不影響爆炸。真的難看再把閃爍改成獨立的疊加層。
- 🔧 手感對不上時開 `SuicideBombBrain.DebugLog = true`：印點引信的邊緣距離、引信長度、引爆的傷害與半徑。
