...
| 旱魃 12 | **焚天火雨** 61／61 | Parabolic＋`LaunchSource=Offscreen`：6 顆火球從畫面外砸向滑鼠附近（散布 2.5）、落地爆炸＋燒地。⚠ 爆炸半徑＝`AreaRadius 0.6 × BulletScale 3`＝1.8 |
| 尼德霍格 72 | **焚獄炎杖** 69／70（2026-09-23 換掉龍息 62） | Normal 隱形載體（地裂刺同一套）：5 道 80° 扇形、追蹤 240、穿透 2、扣一次連發 2 波；沿路每 0.8 單位噴一根**地獄火柱 VfxTable 48**（素材庫 `Fire/directional_fire_burst_001/red`），命中怪物放燃燒地面＋火焰爆發 17。**是正式道具**（ItemTable 69，icon `Weapon/single/weapon_firestaff`），但目前沒進任何抽選池 |
...
| 芬里爾 32 | **魔狼爪** 71／72（2026-09-23 換掉天狼焚爪 64；同日先做成冰屬性「冰狼爪」，作者覺得火焰狼拿冰武器怪，改火） | Normal 焰刃：一次 3 道（30° 扇形）、速度 12（中等）、穿透 3、飛 1.2 秒；子彈是素材庫 `pj1_slash/red` 的 6 幀動畫，**匯入時放大 4 倍（最近鄰）**存成 `Weapon/animation/magicWolfSlash/`，再配 `BulletScale 16`（原圖 96×48，子彈 prefab 本身縮 0.1）；`SpriteAngleOffset 360`（見 PROBLEMS E38）；判定 `Radius 1.4` 手動對齊月牙大小（Normal 的判定半徑不吃 BulletScale）；命中只在怪身上爆一團火焰 17（**不燒地**，作者 2026-09-23 拍板）。**是正式道具**（ItemTable 71，icon `Weapon/single/weapon_wolfclaw`），沒進抽選池 |
