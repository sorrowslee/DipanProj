...
| 規則 | 內容 |
|---|---|
| 系列 | 一個系列 = 三個階段。目前有七個系列：殭屍系列 **殭屍 → 毛殭 → 旱魃**、血族系列 **覓血者 → 血伯爵 → 該隱**、狂族系列 **狼人 → 望月者 → 芬里爾**、土裔系列 **石像鬼 → 山嶽巨人 → 泰坦**、靈根系列 **築基期 → 元嬰期 → 化神期**、雲龍系列 **蛟人 → 螭吻 → 應龍**、熾龍系列 **龍奴 → 法夫納 → 尼德霍格**（英文：血族系列 **Bloodborn**、第一階 **Bloodseeker**（也是外型資料夾名）；狂族系列 **Feralborn**、三階 `Werewolf`／`Moonwatcher`／`Fenrir`（2026-08-27 加）。2026-08-27 前叫「夜裔系列／夜裔／Nightborn」，PROBLEMS／PROGRESS 歷史條目裡的 Nightborn 就是它；土裔系列 **Gaiaborn**、三階 `Gargoyle`／`MountainGiant`／`Titan`（2026-09-09 加）；靈根系列 **SpiritRoot**、三階 `Foundation`／`Nascent Soul`／`Divine Form`（2026-09-10 加，後兩個資料夾名含空白）；雲龍系列 **Cloudborn**、三階 `Jiao`／`Chiwen`／`Yinglong`，熾龍系列 **Blazeborn**、三階 `Thrall`／`Fafnir`／`Nidhogg`（兩個系列 2026-09-13 加）。⚠ **靈根系列在 2026-09-10 當天曾短暫叫「靈脈／SpiritVein」**，同日全面改名為靈根／SpiritRoot；資料夾、icon、兩張表與文件都已一併更新，專案裡不該再有 SpiritVein 這個字） |
...
目前二十三瓶。**ID 編號規則**：`30x` ＝各系列第一階、`32x` ＝第二階直達、`33x` ＝第三階直達，
末位固定是系列序（1 殭屍／2 血族／3 狂族／4 土裔／5 靈根／6 雲龍／7 熾龍）；`310`／`311` 是兩瓶全系列通用的進階藥劑。
...
| ID | 名稱 | 欄位 | 取得 |
|---|---|---|---|
| 301~307 | 各系列第一階（`<1>階<系列>藥劑`） | `BloodlineID`＝該系列 Stage1Id | 血統祭壇（`BaseBloodRoll.csv`，300 元、不連抽，七瓶權重都是 10） |
| 321~327 / 331~337 | 第二／三階直達（`<2\|3>階<系列>藥劑`） | `BloodlineID`＝該階的血統 Id | ⏳ **還沒放進任何池**（等作者決定投放方式，見 §8） |
...
第三階設定上是神明一般的存在，所以多了三層常駐視覺（七個系列的第三階目前都填了）。
...
- 目前七個系列（殭屍、血族、狂族、土裔、靈根、雲龍、熾龍），資料都齊；再加系列照 §7 走。**雲龍／熾龍的 `BodyScale` 全填 1、`WalkSpeed` 留空、五屬性是佔位值，六個血統都還沒實機看過**；兩個第三階的神格特效已填（應龍 `Fade` 青殘影、尼德霍格 `Stone` 熔岩岩輪，見 §5b），同樣還沒實機看過。狂族的立繪除了 `normal` 之外目前是暫代圖（與旱魃同一張），美術到了直接覆蓋檔案即可
