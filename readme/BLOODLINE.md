...
| 規則 | 內容 |
|---|---|
| 系列 | 一個系列 = 三個階段。目前有八個系列：殭屍系列 **殭屍 → 毛殭 → 旱魃**、血族系列 **覓血者 → 血伯爵 → 該隱**、狂族系列 **狼人 → 望月者 → 芬里爾**、土裔系列 **石像鬼 → 山嶽巨人 → 泰坦**、靈根系列 **築基期 → 元嬰期 → 化神期**、雲龍系列 **蛟人 → 螭吻 → 應龍**、熾龍系列 **龍奴 → 法夫納 → 尼德霍格**、蟲族系列 **寄生體 → 獵殺者 → 蟲皇**（英文：血族系列 **Bloodborn**、第一階 **Bloodseeker**（也是外型資料夾名）；狂族系列 **Feralborn**、三階 `Werewolf`／`Moonwatcher`／`Fenrir`（2026-08-27 加）。2026-08-27 前叫「夜裔系列／夜裔／Nightborn」，PROBLEMS／PROGRESS 歷史條目裡的 Nightborn 就是它；土裔系列 **Gaiaborn**、三階 `Gargoyle`／`MountainGiant`／`Titan`（2026-09-09 加）；靈根系列 **SpiritRoot**、三階 `Foundation`／`Nascent Soul`／`Divine Form`（2026-09-10 加，後兩個資料夾名含空白）；雲龍系列 **Cloudborn**、三階 `Jiao`／`Chiwen`／`Yinglong`，熾龍系列 **Blazeborn**、三階 `Thrall`／`Fafnir`／`Nidhogg`（兩個系列 2026-09-13 加）；蟲族系列 **Swarmborn**、三階 `Parasite`／`Ravager`／`Swarm Emperor`（2026-09-14 加，第三階資料夾名含空白）。⚠ **靈根系列在 2026-09-10 當天曾短暫叫「靈脈／SpiritVein」**，同日全面改名為靈根／SpiritRoot；資料夾、icon、兩張表與文件都已一併更新，專案裡不該再有 SpiritVein 這個字） |
...
6,Cloudborn,雲龍,60,61,62,雲龍系列：蛟人 → 螭吻 → 應龍
7,Blazeborn,熾龍,70,71,72,熾龍系列：龍奴 → 法夫納 → 尼德霍格
8,Swarmborn,蟲族,80,81,82,蟲族系列：寄生體 → 獵殺者 → 蟲皇
```

這是「系列 ↔ 階段 ↔ 血統 Id」的**唯一真相**。`BloodlineSeriesTable.cs` 載入時順便建一份
「血統 Id → (系列, 第幾階)」的反查索引，`TryLocate()` 就是查它。

**血統 Id 慣例**：一個系列吃一個十位段（殭屍 10~12、血族 20~22、狂族 30~32、土裔 40~42、靈根 50~52、雲龍 60~62、熾龍 70~72、蟲族 80~82、下一個系列 90~92…），**Id 1 保留給人類**。
...
（雲龍 60~62、熾龍 70~72、蟲族 80~82 同樣格式，此處略，完整內容以 CSV 為準）
```
...
目前二十六瓶。**ID 編號規則**：`30x` ＝各系列第一階、`32x` ＝第二階直達、`33x` ＝第三階直達，
末位固定是系列序（1 殭屍／2 血族／3 狂族／4 土裔／5 靈根／6 雲龍／7 熾龍／8 蟲族）；`310`／`311` 是兩瓶全系列通用的進階藥劑。
...
| 雲龍 | 306 1階雲龍藥劑 `=60` | 326 2階雲龍藥劑 `=61` | 336 3階雲龍藥劑 `=62` |
| 熾龍 | 307 1階熾龍藥劑 `=70` | 327 2階熾龍藥劑 `=71` | 337 3階熾龍藥劑 `=72` |
| 蟲族 | 308 1階蟲族藥劑 `=80` | 328 2階蟲族藥劑 `=81` | 338 3階蟲族藥劑 `=82` |
...
| ID | 名稱 | 欄位 | 取得 |
|---|---|---|---|
| 301~308 | 各系列第一階（`<1>階<系列>藥劑`） | `BloodlineID`＝該系列 Stage1Id | 血統祭壇（`BaseBloodRoll.csv`，300 元、不連抽，八瓶權重都是 10） |
| 321~328 / 331~338 | 第二／三階直達（`<2\|3>階<系列>藥劑`） | `BloodlineID`＝該階的血統 Id | ⏳ **還沒放進任何池**（等作者決定投放方式，見 §8） |
...
二十四列的 `IconPath` 目前填的是 `bloodline_BaseBottle`（程式不讀它、一律走 `ItemIcons` 用 `Key` 組檔名）。
...
第三階設定上是神明一般的存在，所以多了三層常駐視覺（七個系列的第三階已填；2026-09-14 加入的蟲族＝蟲皇尚未指派，三層全空）。
...
| `OrbitCount` | 繞幾顆（2026-09-13 加） | 用元件預設（4 顆） |
| `OrbitSize` | 單顆高度 = 角色高 × 此值（2026-09-13 加） | 用元件預設（0.13） |
| `AttackVfxId` | **攻擊時才播一次**的罩身特效 id（2026-09-13 加；那列要 `Loop=0`、`Duration` 留空） | 沒有這一層 |
| `AttackFxGap` | 兩次播放的最短間隔（秒）——壓住連射時就是每隔這麼久播一次 | 用元件預設（3 秒） |
...
| 應龍（62） | （留空） | （留空） | — | （留空） | — ／ **`OrbitVfxId`=38**（環繞水球，`OrbitCount`=1、`OrbitSize`=0.4） |
| 尼德霍格（72） | （留空） | （留空） | — | （留空） | — ／ **`AttackVfxId`=39**（攻擊時罩身熔岩爆，`AttackFxGap`=3） |
| 蟲皇（82） | （留空） | （留空） | — | （留空） | — ／ **`HitVfxId`=40**（擊中怪物時在怪身上播咬擊，見 **§5c**） |
...
> ⚠ 上面那段 2026-09-10 的設計方向（旱魃＝龜裂赤日輪、芬里爾＝殘月輪＋冷藍虛影、泰坦＝岩輪＋揚塵）
> **已經全部被實機結果取代**：旱魃改 `Disk`、芬里爾改 `Step` 火焰腳印、泰坦改 `Orbit` 環繞碎石。
> 那三句留著只當歷史脈絡，別照它填。

**應龍＝環繞水球（`OrbitVfxId=38`、`OrbitCount=1`、`OrbitSize=0.4`）**。
`fx2_bubble_loop` 的藍版（7 幀循環），走**繞行層**而不是罩身層：一顆放大的水球繞著應龍轉，
形式與泰坦的環繞碎石相同、但**只有一顆而且大三倍**，所以兩者一眼分得開。

> **這一版是第三次改**（作者實機逐版看過）：
> ① `Fade` 青色殘影 → 「勉強可以，但我要的是水球把它籠罩在裡面」；
> ② `AuraVfxId` 水球罩全身 → 「兩個都不行」，改成縮小、變成繞在身邊的一顆；
> ③ 現在這版。**記錄前兩版不是為了留戀，是因為「罩住全身」與「繞在身邊」聽起來很近、
> 但那是兩個不同的層（`BloodlineAura` / `BloodlineOrbit`）**，下次描述需求時先分清楚是哪一種。

**尼德霍格＝攻擊時腳下竄起的熔岩噴發（`AttackVfxId=39`、`AttackFxGap=3`）**。
`directional_particle_burst_002` 的 red 版（14 幀 96×128，作者在特效預覽器裡挑的；
第一版用的 `stylized_explosion_002` 已移到 `_to_delete/`）。
`Stone` 岩輪與罩全身的 `AuraVfxId` 火爆兩版都被退回，素材已移到專案根的 `_to_delete/`。

### 第五層：`BloodlineAttackFx`（攻擊時才出現）

**與另外四層的差別是「時機」，不是輪廓。** 前四層都是站著就一直在的常駐視覺；這一層平常完全看不到，
**只在出手那一瞬間罩上來一次**。所以它**不佔用「一個血統只掛一層」那條原則的額度**——
已經有常駐層的血統還是可以再掛這一層。

> 2026-09-14 又加了**第六層 `BloodlineHitFx`（打到怪物時在那隻怪身上播）**，同樣不佔額度，見 **§5c**。
> 第五層是「出手就播、罩自己」，第六層是「打中才播、播對方」——描述需求時先分清楚是哪一種。

- **節奏**：單擊＝播一次；**壓住連射不會每發都播**，播一次之後至少隔 `AttackFxGap` 秒才會再播。
  實作就是一個冷卻時間戳；單擊之所以每次都播得到，是因為兩次點擊本來就隔得比冷卻久。
- **播放速度**用 VfxTable 那列的 `AnimFPS` 調（幀數 ÷ FPS ＝ 一輪多長），**不要在元件裡另外做計時**——
  兩邊各自計時就會出現「表格改了幀率、特效長度卻沒跟著變」。尼德霍格是 10 幀 ÷ 8 FPS ≒ **1.25 秒**。
- **怎麼知道玩家攻擊了**：⚠ **要接兩條路，只接一條會漏一半**（[PROBLEMS.md](PROBLEMS.md) **F21**）。
  - **離散武器**：`PlayerController.TrySpawnFireEffect` 是四條發射路徑的共同節點、**每發都會經過**，
    通知掛在那裡的最前面——刻意放在「這把武器有沒有 `FireEffectID`」的 early-return **之前**，
    否則沒填發射特效的武器就不會通知。
  - **持續型武器（雷射／佛光）**：上面那個節點**只有按下的那一幀**會經過，按著不放的期間一次都不再呼叫
    （程式本來就設計成「持續存在期間不每幀重播砲口特效」）。所以另外靠 `BloodlineAttackFx.Update`
    每幀問 `PlayerController.IsContinuousFireActive`。兩條路都走同一個 `NotifyAttack()`，冷卻自然擋掉多餘的。
  - **通則**：「玩家正在攻擊」在這個專案是**兩種東西**——離散是一連串瞬間事件、持續型是一段狀態。
- **錨點要看素材構圖**：`AnchorAtFeet`（元件上，預設打勾）＝特效**底邊對齊腳底**，
  給「從地面往上竄」的素材用——那種圖的底邊就是地面，用身體中心對齊會整個懸空。
  取消勾選則是特效中心對齊 `BodyCenterWorldPos`，給球狀爆炸用。
  `SpawnSizedToHeight` 收的是特效**中心**，所以腳底錨要自己往上推半個高度。
  ⚠ 目前只有一個血統用這一層，所以這個開關還在元件上；**第二個血統需要不同錨點時**，
  照 `OrbitCount`／`OrbitSize` 的方式開成表B 欄位即可。
- ⚠ **這一層的 `SortingOrder` 會生效**（不像 Aura／Orbit 每幀被接管），所以 VfxTable 那列**留空**
  ＝用 VfxManager 全域預設。填 22050 那種表演層的值會讓爆炸浮在柱子與牆的前面。

#### ⚠ 挑「罩住全身」的素材時先量中央的 alpha

火焰類素材幾乎都是**實心的一團**，直接罩上去會把角色糊掉。實測（取中間幀、量中央 36% 區域）：

| 素材 | 中央 alpha | 結果 |
|---|---|---|
| `fx3_lightning_aura`（化神期青雷） | 0 | 完全中空，只有外圍電弧 |
| `fx2_bubble_loop`（應龍水球） | 68 | 半透明，角色看得見 |
| `symmetrical_explosion_004`（尼德霍格火爆） | ≈0 | 中空環狀 |
| `fanfx1_power_up`（往上竄的火柱） | **233** | ✗ 幾乎不透明，角色會被蓋掉 |
| `fanfx2_fire_spell`／`fire_looping_001` | 高（白色核心） | ✗ 同上 |

**整個特效庫裡「中空 ＋ 循環」的罩身素材只有三種**：`fx3_lightning_aura`、`fx2_bubble_loop`、
`scifx*_scan_loop`（科技感、不合本作）。所以**要第四種就得靠 `PulseGap`**——
它讓「一次性的爆發」也能當常駐層（播一輪、留白、再播一輪），尼德霍格就是這樣來的。
挑素材時用「中央 alpha 低、外圈 alpha 高」去篩，比用眼睛翻圖快得多。

⚠ **`VfxTable` 的 `Scale` 欄對這一層不生效**：`BloodlineAura` 走 `SpawnLoopSizedToHeight`，
特效一律被縮放到「角色高 × `HeightRatio`(1.25)」，所以**素材原始尺寸是幾 px 都不影響最終大小**，
Scale 填 1 就好。素材接近正方形時，罩出來就是一顆比角色略大的球。
...
⚠ **`Count` 與 `SizeRatio` 兩個可以逐血統覆寫**（2026-09-13 加）：表B 的 `OrbitCount` / `OrbitSize`，
**留空／0 ＝ 用元件上的值**（所以泰坦那一列一格都不用填、行為完全沒變）。
會開這兩欄是因為泰坦是「一圈小碎石」、應龍是「單獨一顆大水球」——同一個元件要同時做出這兩種，
顆數與大小就不能寫死在元件上。其餘參數仍是全血統共用，之後有需要再照同一個方式開。
...
## 5c. 第六層：`BloodlineHitFx`（打到怪物時，在**那隻怪**身上播）

> **一句話**：三階血統的擊中特效**凌駕一般武器**——打到怪物時，武器自己的 `HitEffectID` 讓位，
> 只播血統這一個，而且是播在**被打的那隻怪身上**、大小跟著牠走。

**這是作者 2026-09-14 拍板的通則，不是蟲皇的特例**：以後每個三階血統只要在表B 填了 `HitVfxId`，
它的擊中特效就一律蓋過該武器原本的擊中特效。填表即生效，程式不用再動。

### 與第五層 `BloodlineAttackFx` 的差別是「在誰身上」

| | 第五層 `AttackVfxId` | **第六層 `HitVfxId`** |
|---|---|---|
| 觸發 | **出手**就播（有沒有打到都播） | **打中怪物**才播 |
| 位置 | 自己身上（罩身） | **對方身上** |
| 大小基準 | 玩家的 `ScaledCharacterHeight` | **那隻怪的可見高度**（大怪大特效、小怪小特效） |
| 節流 | 全域一個冷卻（`AttackFxGap`） | **每隻怪各自計時**（`HitFxGap`） |

⚠ 第六層是**唯一一層定位基準不在玩家身上**的——另外五層都靠 `FeetWorldPos`／`BodyCenterWorldPos`／
`ScaledCharacterHeight`（PROBLEMS **E14**），這一層讀的是目標的 `SpriteRenderer.bounds`
（同怪物死亡特效 VfxTable 7 的做法，見 [VFX.md](VFX.md)）。

⚠ 與第五層一樣，**這一層不佔用「一個血統只掛一層」的額度**（§5b）——它平常看不到，
只在命中那一瞬間出現，不會和常駐層搶辨識度。

### 兩欄

| 欄位 | 作用 | 留空 |
|---|---|---|
| `HitVfxId` | 擊中特效的 VfxTable id（那一列要 `Loop=0`、`Duration` 留空） | 沒有這一層，武器的 `HitEffectID` 照常運作 |
| `HitFxGap` | **同一隻怪**兩次之間的最短間隔（秒） | 用元件預設（0.35 秒） |

### ⚠ 讓位的邊界：打到怪才讓，打到牆不讓

**「以血統為主」只在打到怪物時成立。** 子彈打到牆、打爆地上物、拋物線落地爆炸、定點法陣的施放點，
一律照播武器自己的 `HitEffectID`——那些都不是「攻擊到怪物」，讓位會讓打牆的回饋憑空消失。

實作在 `PlayerController.TrySpawnHitEffect(weapon, pos, hitEnemy)`：**多收一個 `hitEnemy`**，
只有它為 true 且玩家掛著這一層時才 `return`。六個呼叫點分成兩類：

| 呼叫點 | 傳什麼 | 為什麼 |
|---|---|---|
| 雷射 tick、子彈命中 | 現成的 `hitEnemy` | 本來就分得出打到怪還是環境 |
| 連鎖閃電 | `IsOnEnemyLayer(targets[i])` | 連鎖目標可能是**可破壞地上物**（搜尋遮罩是 `EnemyLayer｜EnvLayer`），不能寫死 true |
| 分段雷柱的地面爆炸、拋物線落地、定點法陣 | `false`（預設） | 這三個播的是**落點／施放點的視覺**，不是打在怪身上的命中回饋 |

⚠ 近戰扇形（`ShootMelee`）與範圍施放那兩處是**直接 `Spawn`、不走這支**，刻意不動——
它們在傷害**之前**播、播的是「揮砍動畫」而不是命中回饋（程式裡本來就註明「視覺只播一次 HitEffect，
避免每打到一隻怪就疊一套揮砍動畫」）。血統咬擊會照樣在被打中的怪身上出現，兩者不衝突。

### 怎麼知道打中了：掛在傷害的單一入口

通知點是 **`CombatSystem.Apply` 的結尾**（傷害真的結算出去之後）。那是全遊戲傷害的唯一入口，
所以子彈／雷射 tick／連鎖閃電／AOE 爆炸四條路一條都不會漏，**之後新增武器模式也不必回來補**。

> 對照第五層的痛：它接的是「玩家有沒有出手」，而出手在這個專案是**兩種東西**
> （離散＝一連串瞬間事件、持續型＝一段狀態），所以得接兩條路，第一版還漏了一半（PROBLEMS **F21**）。
> 第六層接的是「有沒有造成傷害」，那件事只有一個入口，所以天生不會有這個問題。

`BloodlineHitFx.NotifyHit(source, target)` 自己做四道判斷（來源是不是玩家、目標是不是怪物、
有沒有掛這一層、那隻怪的冷卻到了沒），所以 `CombatSystem` 只多一行、不需要知道血統的任何事。
⚠ **來源必須是玩家**——`Apply` 也負責怪打怪（陣營演戲傷害），不擋的話場上互毆會滿場咬擊。

### 節流為什麼是「每隻怪各自計時」

雷射與火焰噴射器**每個 `DotInterval` tick 都會命中同一隻怪**，不節流會在牠身上狂閃；
但全域計時又會讓「AOE 一次打中五隻」只有一隻身上看得到，範圍感整個不見。
分怪計時同時解掉這兩件事。字典鍵是怪的 `InstanceID`，每 10 秒掃一次把過期的丟掉（怪死了鍵會留著）。

### 目前填了誰

| 血統 | HitVfxId | 素材 |
|---|---|---|
| 蟲皇（82） | **40** | `fx3_bite` 的 **green 原色版**（17 幀 64×64 @15fps，Super Pixel Effects Pack 3） |

### 加一個血統要做的事

1. 從特效預覽器挑好顏色／套色，把那組 PNG 複製進 `Resources/VfxEffects/BloodlineHitFx/<血統>/`，
   **檔名改成 `_01`~`_NN` 兩位補零**（原始包是三位數 `_001`，不改的話 `VfxManager` 一張都載不到）
2. `VfxTable` 加一列：`Loop=0`、`Duration` 留空、`SortingOrder` 留空（用全域預設）。
   ⚠ **`AniNumber` 必須與實際張數完全相符**——差一張整個特效就完全不出現（見 [VFX.md](VFX.md)）
3. 表B 那一列填 `HitVfxId`（`HitFxGap` 要調再填）

**程式不用動。**

---

## 5d. 夢境武器（`DreamWeaponId`，2026-09-23）

新手夢境教學裡，玩家是隨機一個**三階血統**，手上的武器也跟著血統走。表B 第 26 欄 `DreamWeaponId` 填 WeaponTable 的 ID。

### 怎麼裝、怎麼收

- `DreamTutorialFlow` 等玩家生出來、外觀換好之後，呼叫 `PlayerController.SetScriptedWeapon(DreamWeaponId)`。
- 底層是 `WeaponManager` 的**劇情武器覆寫**（`SetScriptedOverride`）：`GetCurrentWeapon()` 的優先序是
  **工坊模擬 ＞ 劇情覆寫 ＞ 背包武器欄**。所以**不進背包、不進存檔**，清掉覆寫就等於收回，玩家原本裝的那把自動回來。
- 離開夢境地圖（`DreamTutorialFlow.Update`）與流程物件被銷毀（`OnDestroy`）都會收回，重複呼叫無害。
- 夢境期間**玩家選單全鎖**（背包 B／倉庫 K／鍛造 Y／設定 O／ESC 開設定），見 [UI_SYSTEM.md](UI_SYSTEM.md)〈玩家選單鎖〉。
  理由：武器不在背包裡，開背包換裝只會出事。
- 留空 ＝ 不覆寫（印一則警告，沿用玩家自己裝的；新角色通常是空手 ⇒ 打不出東西）。**新增三階血統一定要填**。

### 目前的配給（全部耗魔 0；數值是起手值，實機在武器工坊再調）

| 血統 | 武器（WeaponTable／配方） | 做法 |
|---|---|---|
| 化神期 52 | **萬劍歸宗** 60／60 | Normal：5 發扇形 70°、追蹤 360、穿透 2、擊中特效冰凍 2（2026-09-23 改：拿掉燒地、雷地爆 26 → 冰凍 2）；飛劍圖 `weapon_sword` 大小 5.07 |
| 旱魃 12 | **焚天火雨** 61／61 | Parabolic＋`LaunchSource=Offscreen`：**9 顆**火球（2026-09-24 從 6 加到 9）從畫面外砸向滑鼠附近（散布 2.5）、落地爆炸＋燒地（2026-09-24：燒地換成**自繪的 GroundEffect 9「焚地(大)」**（＝8 的圖、半徑 1.6→2.13）（單張 12 幀循環、焦黑地面＋岩漿裂紋＋外圈火環）；落地爆炸換成 **VfxTable 50「隕石砸地」**（素材庫 `epic_explosion_001/orange`，畫布下方補透明讓撞擊點落在圖中心、Scale 5））。⚠ 爆炸半徑＝`AreaRadius 0.6 × BulletScale 3`＝1.8 |
| 尼德霍格 72 | **焚獄炎杖** 69／70（2026-09-23 換掉龍息 62） | Normal 隱形載體（地裂刺同一套）：5 道 80° 扇形、追蹤 240、穿透 2、扣一次連發 2 波；沿路每 0.8 單位噴一根**地獄火柱 VfxTable 48**（素材庫 `Fire/directional_fire_burst_001/red`），命中怪物只爆一團火焰 17（**不燒地**，2026-09-24 拿掉）。**是正式道具**（ItemTable 69，icon `Weapon/single/weapon_firestaff`），但目前沒進任何抽選池 |
| 該隱 22 | **血鳴古鐘** 70／71（2026-09-23 換掉血月雙爪 63，作者嫌太弱） | Normal 隱形載體：**不分裂、不追蹤**、單一直線慢速（速度 5、飛 2.4 秒≈12 單位＝一路到螢幕外）、無限穿透、**穿牆**（`BlockedByEnvironment 0`）；沿路每 2 單位炸一團**血鳴碎響 VfxTable 49**（素材庫 `fanfx2_shatter/red`，直徑約 3.3＞一隻怪），一條線約 6 團。判定半徑 1.3 對齊炸開的大小。**是正式道具**（ItemTable 70，icon `Weapon/single/weapon_ancientbell`），沒進抽選池 |
| 芬里爾 32 | **魔狼爪** 71／72（2026-09-23 換掉天狼焚爪 64；同日先做成冰屬性「冰狼爪」，作者覺得火焰狼拿冰武器怪，改火） | Normal 焰刃：一次 **5 道（100° 扇形，每道間隔 25°）**（2026-09-24 從 3 道 30° 改）、速度 12（中等）、穿透 3、飛 1.2 秒；子彈是素材庫 `pj1_slash/red` 的 6 幀動畫，**匯入時放大 4 倍（最近鄰）**存成 `Weapon/animation/magicWolfSlash/`，再配 `BulletScale 16`（原圖 96×48，子彈 prefab 本身縮 0.1）；`SpriteAngleOffset 360`（見 PROBLEMS E38）；判定 `Radius 1.4` 手動對齊月牙大小（Normal 的判定半徑不吃 BulletScale）；命中只在怪身上爆一團火焰 17（**不燒地**，作者 2026-09-23 拍板）。**是正式道具**（ItemTable 71，icon `Weapon/single/weapon_wolfclaw`），沒進抽選池 |
| 泰坦 42 | **山崩地裂** 65／65 | Normal 隱形載體：8 道放射（`SpreadAngle 315`＝每 45° 一道）、不追蹤、傷害 8（2026-09-23 從 5 加強）、沿路每 1 單位長地刺（`TrailEffectID 3`）、飛 0.8 秒 |
| 應龍 62 | **雷雨天罰** 66／66 | SkyStrike：5 道落雷（80° 扇形、吸附 3）、分段雷柱，每道落點再接連鎖閃電（子配方 **67**，跳 3 次、藍色閃電） |
| 蟲皇 82 | **萬蟲巢** 67／68 | Normal：一次 4 個蜂巢（50°），打到怪或牆各迸出子武器 **蟲群 68／69**（6 隻、`SpreadAngle 300`＝每 60° 一隻、追蹤 360） |

⚠ **放射狀要用 `360 − 360/N`，不要填 360**：分裂角度是「頭尾兩顆各在 ±角度/2」，填 360 會讓第一顆和最後一顆重疊在同一個方向。
...
3. 美術：`SequenceImage/<系列Key>/<SpriteFolder>/{idle,walk,dead,attack}/` 與 `Talk/<系列Key>/<SpriteFolder>/<6 種情緒>.png`（normal/happy/angry/cry/fear/hurt）
...
7. 第三階那一列填 **`DreamWeaponId`**（新手夢境教學會抽到它，留空＝夢裡空手），見 §5d
...
- 目前八個系列（殭屍、血族、狂族、土裔、靈根、雲龍、熾龍、蟲族），資料都齊；再加系列照 §7 走。**雲龍／熾龍的 `BodyScale` 全填 1、`WalkSpeed` 留空、五屬性是佔位值，六個血統都還沒實機看過**；兩個第三階的神格特效已填（應龍 `Fade` 青殘影、尼德霍格 `Stone` 熔岩岩輪，見 §5b），同樣還沒實機看過。狂族的立繪除了 `normal` 之外目前是暫代圖（與旱魃同一張），美術到了直接覆蓋檔案即可
- **蟲族系列（2026-09-14 加）目前的狀態**：兩張表、三瓶藥劑（308／328／338）、血統池 308、icon 成品瓶 `bloodline_Swarmborn` 都齊，素材四個動作與六種情緒立繪也都在。`BodyScale` 三階全填 1、`WalkSpeed` 留空（＝不改速度）、五屬性是佔位值（寄生體偏敏捷、蟲皇全面高），**都還沒實機看過**；第三階蟲皇的神格特效已指派＝**第六層擊中特效**（`HitVfxId`=40 咬擊，2026-09-14 加，見 §5c），§5b 那五個常駐層仍全空。
- ~~⚠ 蟲皇的 `dead/` 只有一張圖~~ → **2026-09-14 已解決**：那張其實是 AutoSprite 合圖，被切割工具的舊防呆誤判而拒切（[PROBLEMS.md](PROBLEMS.md) **C14**），工具修好後切出 25 張。⚠ **要在 Unity 跑一次 `Project Tools → Sync Map Assets`**，catalog 目前還記著舊的「單張」。
...
- 立繪揭示只用 `normal` 表情。之後若想讓不同血統用不同表情（例如旱魃用 `happy`），把 `BloodlineIntroPanel.PortraitEmotion` 改成從表B 讀一個新欄位即可
