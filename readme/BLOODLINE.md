...
| 規則 | 內容 |
|---|---|
| 系列 | 一個系列 = 三個階段。目前有兩個系列：殭屍系列 **殭屍 → 毛殭 → 旱魃**、夜裔系列 **夜裔 → 血伯爵 → 該隱** |
...
2,Nightborn,夜裔,20,21,22,夜裔系列：夜裔 → 血伯爵 → 該隱
```

這是「系列 ↔ 階段 ↔ 血統 Id」的**唯一真相**。`BloodlineSeriesTable.cs` 載入時順便建一份
「血統 Id → (系列, 第幾階)」的反查索引，`TryLocate()` 就是查它。

**血統 Id 慣例**：一個系列吃一個十位段（殭屍 10~12、夜裔 20~22、下一個系列 30~32…），**Id 1 保留給人類**。
...
20,Nightborn,夜裔,Nightborn,1,10,15,25,20,15,,夜裔系列 第一階
21,CrimsonCount,血伯爵,Crimson Count,1,10,35,40,40,35,,夜裔系列 第二階
22,Cain,該隱,Cain,1,10,60,55,60,55,,夜裔系列 第三階
```
...
⚠ **夜裔系列的可見高一階比一階矮**（174 → 157 → **138**），該隱只有 Base 的 71%。正規化是「把可見高拉成同一個世界高度」，
...
| 302 | 血統藥劑・夜裔 | `BloodlineID=20` | 血統祭壇（同上，權重與殭屍同為 10） |
| 310 | 血統進階藥劑・中階 | `BloodlineUpgrade=2` | 道具祭壇（`BaseItemRoll.csv`，權重 3） |
| 311 | 血統進階藥劑・高階 | `BloodlineUpgrade=3` | 道具祭壇（`BaseItemRoll.csv`，權重 1） |

icon 都在 `Resources/UI/Icons/Items/positions/bloodline/`：`bloodline_Jiangshi`（301）／`bloodline_Nightborn`（302）／`bloodline_lvup_middle`（310）／`bloodline_lvup_high`（311）。
...
2. **立繪比例不保證一致。** `Base` 是 1122×1402（比例 0.80），殭屍三階與夜裔三階都是 1024×1536（0.667）。
...
1. 表A 加一列（Id 接續，例如第三個系列）：`3,Werewolf,狼人,30,31,32,...`
2. 表B 加三列：Id 30/31/32，填 `SpriteFolder`、`BodyScale`（先填 1，實機看過再調）與五屬性
...
- 目前兩個系列（殭屍、夜裔），素材與資料都齊；再加系列照 §7 走
- **夜裔三階的 `BodyScale` 全是 1，還沒實機看過**——該隱那張 idle 可見高只有 138px（Base 193），正規化會把它放大約 1.4 倍，是最需要用眼睛校正的一個
