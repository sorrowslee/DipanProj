...
| 規則 | 內容 |
|---|---|
| 系列 | 一個系列 = 三個階段。目前有五個系列：殭屍系列 **殭屍 → 毛殭 → 旱魃**、血族系列 **覓血者 → 血伯爵 → 該隱**、狂族系列 **狼人 → 望月者 → 芬里爾**、土裔系列 **石像鬼 → 山嶽巨人 → 泰坦**、靈脈系列 **築基期 → 元嬰期 → 化神期**（英文：血族系列 **Bloodborn**、第一階 **Bloodseeker**（也是外型資料夾名）；狂族系列 **Feralborn**、三階 `Werewolf`／`Moonwatcher`／`Fenrir`（2026-08-27 加）。2026-08-27 前叫「夜裔系列／夜裔／Nightborn」，PROBLEMS／PROGRESS 歷史條目裡的 Nightborn 就是它；土裔系列 **Gaiaborn**、三階 `Gargoyle`／`MountainGiant`／`Titan`（2026-09-09 加）；靈脈系列 **SpiritVein**、三階 `Foundation`／`Nascent Soul`／`Divine Form`（2026-09-10 加，後兩個資料夾名含空白）） |
...
5,SpiritVein,靈脈,50,51,52,靈脈系列：築基期 → 元嬰期 → 化神期
```

這是「系列 ↔ 階段 ↔ 血統 Id」的**唯一真相**。`BloodlineSeriesTable.cs` 載入時順便建一份
「血統 Id → (系列, 第幾階)」的反查索引，`TryLocate()` 就是查它。

**血統 Id 慣例**：一個系列吃一個十位段（殭屍 10~12、血族 20~22、狂族 30~32、土裔 40~42、靈脈 50~52、下一個系列 60~62…），**Id 1 保留給人類**。
...
50,Foundation,築基期,SpiritVein/Foundation,1,,15,20,30,15,,靈脈系列 第一階
51,NascentSoul,元嬰期,SpiritVein/Nascent Soul,1,,25,30,55,30,,靈脈系列 第二階
52,DivineForm,化神期,SpiritVein/Divine Form,1,,40,45,85,50,,靈脈系列 第三階
```
...
末位固定是系列序（1 殭屍／2 血族／3 狂族／4 土裔／5 靈脈）；`310`／`311` 是兩瓶全系列通用的進階藥劑。

| 系列 | 第一階（30x） | 第二階直達（32x） | 第三階直達（33x） |
|---|---|---|---|
| 殭屍 | 301 血統藥劑・殭屍 `=10` | 321 血統藥劑・毛殭 `=11` | 331 血統藥劑・旱魃 `=12` |
| 血族 | 302 血統藥劑・血族 `=20` | 322 血統藥劑・血伯爵 `=21` | 332 血統藥劑・該隱 `=22` |
| 狂族 | 303 血統藥劑・狂族 `=30` | 323 血統藥劑・望月者 `=31` | 333 血統藥劑・芬里爾 `=32` |
| 土裔 | 304 血統藥劑・土裔 `=40` | 324 血統藥劑・山嶽巨人 `=41` | 334 血統藥劑・泰坦 `=42` |
| 靈脈 | 305 血統藥劑・築基期 `=50` | 325 血統藥劑・元嬰期 `=51` | 335 血統藥劑・化神期 `=52` |

| ID | 名稱 | 欄位 | 取得 |
|---|---|---|---|
| 301~305 | 各系列第一階 | `BloodlineID`＝該系列 Stage1Id | 血統祭壇（`BaseBloodRoll.csv`，300 元、不連抽，五瓶權重都是 10） |
| 321~325 / 331~335 | 第二／三階直達 | `BloodlineID`＝該階的血統 Id | ⏳ **還沒放進任何池**（等作者決定投放方式，見 §8） |
...
（`bloodline_Jiangshi`／`bloodline_Bloodborn`／`bloodline_Feralborn`／`bloodline_Gaiaborn`／`bloodline_SpiritVein`，
...
| `TrailStyle` | 移動殘影：`Fade`（角色虛影）/ `Dust`（腳下揚塵，**尚未接**） | 沒有這一層 |
| `TrailColor` | 殘影染色 | 白＝角色原色 |
...
|---|---|---|---|---|---|
| 該隱（22） | **（留空）** | Disc | A01018 | Fade | C0202A |
| 旱魃（12） | **（留空）** | **Disk** | C08A1E | Fade | B85426 |
...
⚠ **該隱刻意不用環繞電光**（2026-09-10 作者實機看過後決定）：電弧的白色像素太搶眼、蓋過角色本身，
他要的是「只有背後那圈紅環」。`VfxTable` 的 id 32 與那 22 張紅色電弧素材**留著沒刪**——
其他第三階若要用電光，那一列可以直接複製改路徑。
...
| `BloodlineTrail` | 移動殘影：複製當下那格 sprite 留在原地淡出 | 每個殘影都是獨立物件 |
...
- 目前五個系列（殭屍、血族、狂族、土裔、靈脈），資料都齊；再加系列照 §7 走。狂族的立繪除了 `normal` 之外目前是暫代圖（與旱魃同一張），美術到了直接覆蓋檔案即可
...
- **靈脈三階的 `WalkSpeed` 刻意留空**（＝不套用、維持 Inspector 的 5），沒有跟著其他血統填 10——
  因為那些 10 本來就是待重填的佔位值（見上一條）。作者統一決定要填什麼時，靈脈這三格也要一起填
