...
>
> **拿到的是「一張大序列圖」而不是單張幀？** 用 `Project Tools → Split Sprite Sheet` 系列工具切。**格線由每張圖自己推測，排版與解析度都不必固定**——所以合圖高清化放大 2~4 倍、或 AutoSprite 的 perfect loop 讓張數不滿 25，都不用改任何設定。原理：AutoSprite 每一格的角色四周都留白，所以切對時沒有一格的內容會碰到格線（實測真合圖貼邊率 0%），切錯就幾乎每格都被切開（31%~100%）；認不出格線就不切、並附候選一覽說明原因。詳見 PROBLEMS **G8**：
