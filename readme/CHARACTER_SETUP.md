...
>
> **拿到的是「一張大序列圖」而不是單張幀？** 用 `Project Tools → Split Sprite Sheet` 系列工具切（**欄數固定 5、列數由圖自己算**：格邊長＝圖寬÷5、列數＝圖高÷格邊長。所以 1280×1280→5×5＝25 格、**1280×768→5×3＝15 格（AutoSprite 的 perfect loop 版）**、1280×512→5×2＝10 格，放大過的 5120×3072 也一樣；寬不是 5 的倍數、或高不是格邊長的整數倍會被擋下，見 PROBLEMS **G8**）：
