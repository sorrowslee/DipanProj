...
| `cageScale` | 牢籠大小倍率(空=1) | 正式骨牢：1 ＝ 牢籠內徑約為玩家**可見身高**的 0.7 倍（見 `BoneCageVisual.InnerWidthPerBodyHeight`）。舊路徑：相對那一列 VfxTable `Scale` 的倍率 |
...
⚠ **牢籠特效是 `Loop=1`、不會自己消失**——要另一顆 `bindPlayer`（`bind=0`）來收。
忘了收也不會把玩家永久卡住：`PlayerBind` 在換圖／玩家死亡（`OnDisable`）時會自己解綁並清掉特效。
