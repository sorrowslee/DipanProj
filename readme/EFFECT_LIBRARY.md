...
3. **改名對齊遊戲載入格式**：編輯器庫用 `_NNN`（3 位補零），但遊戲的 `VfxManager` 讀 `_NN`（`_{i:D2}`，即 `_01`~`_09`、`_10`…、`_100`…）。複製後把 `_001` 改成 `_01`…。在該資料夾快速改名：
   ```bash
...
*建立於 2026-07-08：地圖編輯器端特效素材庫（`allEffects/` 原始包 → `organize_bundle.py` 通用整理 → `Effects/` → 特效預覽器瀏覽/換色/匯出）。整理庫放 `Assets/` 外避免 Unity 匯入拖慢；不被 sync；挑好再手動複製進遊戲（注意 `_NNN`→`_NN` 改名與 Scale 調整）。*
