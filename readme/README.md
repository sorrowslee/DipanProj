...
| [RECIPE_AND_WEAPON.md](RECIPE_AND_WEAPON.md) | CSV 雙表的**程式面**：`CsvTable` 依表頭取值、`WeaponModeSpec` 規格表（單一真相）、`RecipeEntry.FromFields`、Recipe／WeaponManager（含模擬武器覆蓋掛點）、BounceTarget、序列圖、SpriteAngleOffset | 改讀表程式、加新模式／新欄位、接武器效果模擬系統 |
| [ACTORS_AND_COMBAT.md](ACTORS_AND_COMBAT.md) | 玩家控制器、怪物 AI、受擊反應系統（閃爍／擊退／無敵） | 改玩家或怪物行為、受擊反應 |
| [MONSTER_SETUP.md](MONSTER_SETUP.md) | 怪物量產（route B：程式逐格動畫、地圖素材管線載圖、零 prefab/Animator；idle/walk 必備、attack 可選、防呆；CSV 加 AnimFPS）<br>**CSV 表尾欄位總表**：逐動作顯示倍率（`IdleScale/WalkScale/AttackScale/JumpScale`）、命中幀／放彈幀（`AttackHitFrame`／`ReleaseFrame`）、自爆三欄（`BombDamage/BombRadius/BombFuse`）、踐踏兩欄、**掉落表 `DropTableId`**、**移動拖尾 `MoveTrailFx`** | 量產新怪、換怪物外觀、加怪物動作動畫、**調某隻怪的大小／命中時機／掉落／拖尾** |
| [MONSTER_SPEECH.md](MONSTER_SPEECH.md) | 怪物頭上對話框（CSV 句子1~4＋血量%門檻、發現玩家後隨機說、boss 頻率兩倍、水墨泡泡底板隨機輪流＋避邊鏡像） | 加/改怪物台詞、調說話頻率、換對話框底板 |
| [BOSS_MODULE.md](BOSS_MODULE.md) | Boss 戰鬥模組框架（一隻強怪＝一個 Brain 模組：MonsterContext／MonsterWeaponUser seam）＋**§7 撲擊型（狗/狼）`PounceBrain`：觀望→蓄力→直線撲擊→收招；§7.6 有「加一隻新犬類怪」的 SOP**＋**§8 射手型（弓/弩/火槍）`ArcherBrain`：評估「原地射得到嗎」→射不到才移動；§8.4 是怪物投射武器管線 `WeaponCastService`（玩家與怪物共用發射）**＋**§10 近戰追擊 `MeleeChaseBrain`：貼身才揮武器、揮的期間站定把動作做完才能再移動（`ChaseBrain` 會舉著劍追人）**＋**§9 跳躍踐踏型 `LeapSlamBrain`：開場一記躍擊→落地裂地 AOE→之後永久追擊；傷害＝短命 trigger＋既有接觸傷害，裂痕＝程序化 `GroundCrack` shader；騰空靠 `IAirborneVisual` 把影子與 Y 排序扣回地面**＋**§11 自爆型 `SuicideBombBrain`：走到身邊點引信→閃爍預告→一次性爆炸圈＋自毀；進了引信就一定爆、被打死不爆**＋紅嫁衣女殭屍（逃跑＋召喚）＋召喚做成表驅動武器（RecipeTable Mode=Summon＋四欄） | 做/改 boss 與強怪戰鬥模式、加召喚、讓怪物用武器、**加會撲擊的獸型怪**、**加會射東西的遠程怪**、**加會跳躍踐踏的怪**、**做落地裂地那類地面 shader**、**怪「舉著攻擊動作還在移動」**、**加會自爆的怪** |
...
| [CHARACTER_SETUP.md](CHARACTER_SETUP.md) | 主角外型：**已改走路線 B（程式逐格動畫、血統換外型）**——Characters/SequenceImage/<系列>/<血統>/idle·walk·dead、`Bloodline` 欄/`SetBloodline`；下半為舊 Animator 流程（已取代，存參考） | 換主角外型、加血統、加死亡動畫 |
...
| [RECIPE_DESCRIBE.md](RECIPE_DESCRIBE.md) | **RecipeTable 填表手冊**（2026-08-26 新表：一列一種 `Mode`、50 欄分群（含 2026-08-26 晚加的連擊 2 欄、平行 3 欄）、空白＝預設；模式 × 欄位矩陣、能力珠 × 模式矩陣、GroundEffectID 在各模式的角色、舊表遷移對照） | 填／改配方表、加新模式或新欄位、想知道某顆珠子對某武器有沒有用 |
...
| [SCENE_TIP.md](SCENE_TIP.md) | **進場表演**：**卍字進場**（§0，2026-09-17；離場卍字的倒放——從天而降、落地吐出主角，整段暫停）＋**場景說明**（進圖後畫面上方淡入一次場景名＋血紅分隔線，整段暫停遊戲）：MapsTable 的 `SceneTip` 欄填 key → 載 `UI/Texts/SceneTipPanel_Text_<key>`；**同一趟關卡同一個 key 只跳一次**（所以整關房間可全填同一個 key）；掛在 `FireEnterTriggersRoutine` 等過場播完才跳 | 加/改場景名字圖、想讓某張圖跳（或不跳）名字、調字級位置節奏、**改進場卍字** |
...
| [TRIGGER_CHAIN.md](TRIGGER_CHAIN.md) | 觸發鏈：trigger 接 trigger（next/startDisabled/旗標條件）、動作型 giveItem/teleportTo/cameraFocus、**進場觸發 onEnter（一進地圖自動觸發、0 格不塗格子）**、傳送點綠幕連動<br>**§3.4b 震退玩家 `pushPlayer` ／ 束縛玩家 `bindPlayer` ／ 場景吞噬 `sceneVanish`（整張地圖被吞成虛空、只剩角色）（＋用 `camZone` 零程式拉遠鏡頭）**、**§3.5b 波次刷怪 `maxWaves`／`waveGroup` 與「全滅之後做什麼」** | 編排「對話→給物品→開門→傳送」這類事件流程、加條件分支、做「一進房間就播對話」、**做一波波湧怪＋清空接鏈**、**把玩家轟回定位／綁住演出**、**讓場景消失只剩角色** |
...
| [PERF_QUALITY_AUDIT.md](PERF_QUALITY_AUDIT.md) | 效能與畫質稽核（2026-07-05 四大根因與修正：Rigidbody2D Interpolate、60Hz 物理、地圖貼圖 Bilinear+mipmap、UI maxTextureSize；素材尺寸規範；**§4.1 整張背景圖每格 ≥128px 與各地圖現況實測**；**§4.2 背景解析度↔編輯器格數換算（格數比必須等於圖的長寬比，附快查表與階梯表）**） | 排查卡頓/畫面粗糙、訂素材尺寸前、**產或換背景底圖前**、**開新地圖要決定格數時** |
