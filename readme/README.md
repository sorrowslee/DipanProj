...
| [BOSS_MODULE.md](BOSS_MODULE.md) | Boss 戰鬥模組框架（一隻強怪＝一個 Brain 模組：MonsterContext／MonsterWeaponUser seam）＋**§7 撲擊型（狗/狼）`PounceBrain`：觀望→蓄力→直線撲擊→收招；§7.6 有「加一隻新犬類怪」的 SOP**＋紅嫁衣女殭屍（逃跑＋召喚）＋召喚做成表驅動武器（RecipeTable Mode=Summon＋四欄） | 做/改 boss 與強怪戰鬥模式、加召喚、讓怪物用武器、**加會撲擊的獸型怪** |
