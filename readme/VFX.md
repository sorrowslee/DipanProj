...
⚠ **這一列的 `SortingOrder` 對常駐用途不生效**：`BloodlineAura` 每幀把排序改成「與角色同一條 Y 排序算式 +1」，
否則常駐特效會用固定的表演層值蓋過場上所有地上物（玩家走到柱子後面、電光還在柱子前面）。表裡那個值只是沒被接管時的退路。

⚠ 常駐這一列**必須** `Loop=1` 且 `Duration=-1`。填錯的話特效會播完自毀、而元件看到它不見了會補生一顆 ⇒ 每幀生一顆。
`BloodlineAura` 對此有防呆（連續三次「生出來馬上就沒」就停用該層並印警告），但別依賴它。

完整說明見 [BLOODLINE.md](BLOODLINE.md) §5b。
