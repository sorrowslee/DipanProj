...
兩者都引用同一張 `VfxTable.csv`、用同一個 `VfxManager.Spawn(id, position, angle)`。
...
| 欄位 | 說明 |
|---|---|
| `ID` | 特效唯一識別碼（WeaponTable 的 FireEffectID / HitEffectID 引用此值） |
| `Name` | 僅供辨識，程式不使用 |
...
* `VfxManager`（場景單例）：`Awake` 載入 CSV、預載序列圖；`Spawn(id, position, angle)` 工廠。**不需要 prefab**——每次 Spawn 自己 `new GameObject` + `SpriteRenderer`，排序由 Manager 上的 `SortingLayerName` / `SortingOrder` / `VfxMaterial`（選用）序列化欄位統一決定。
