...
| B | 地圖載入 (Map Loader) | B1~B16 |
| C | 地圖編輯器 / 素材同步 | C1~C15（⚠ C6/C7 排在 C1 前面） |
| D | 存檔 / 常駐單例 (Save & Persistent Singletons) | D1~D25 |
| E | 效能 / 顯示 (Performance & Display) | E1~E42（⚠ E21 誤植在 J 段開頭、E31 誤植在 F 段開頭，都維持原位不搬；E36/E37 接在 E31 前面；E38、E39 在 E 段本體結尾；E40~E42 接在 E39 之後、E36 之前） |
| F | 戰鬥 / 傷害 (Combat) | F1~F31（⚠ G 章整段插在 F3 與 F4 之間；**F27 在 G 章之前、F26 之後**；F28 接在 F27 之後。**F28 其實是顯示問題偽裝成碰撞問題**；F29 接在 F28 之後、F30、F31 接在 F29 之後） |
| G | 角色圖像 / 序列化 (Character Visuals & Serialization) | G1~G12（位置在 F3 之後）；**G13 在 F26 之後、H 段標題之前；G14~G16 誤接在 H 段標題之後、H1 之前**（維持原位不搬） |
...
### B15. 改了 MapsTable 的 `IsLevelStart`，測試選單「直接進關卡」還是進舊那張圖
- **症狀**：想換某劇本的起始地圖，於是在 `MapsTable.csv` 把新圖的 `IsLevelStart` 改 1、舊圖改 0（module 欄相同），但 `Project Tools → 測試 → 直接進關卡 → <該關>` 進去仍然是舊那張。CSV 沒填錯、地圖檔也同步過了。
- **原因**：`DevQuickStart` 的選單項有**兩種目標型別**，只有其中一種吃 `IsLevelStart`：
  - `Set("<module>")` → `MapManager.DevStartModuleOverride` → `StartLevel` → `MapTable.FindLevelStart(module)`＝**讀 `IsLevelStart`**。
  - `Set("map:<id>")` → `MapManager.DevStartMapId` → `GoToMap(id)`＝**寫死地圖 id，完全繞過 `IsLevelStart`**。這型是給「非模組首圖」的測試地圖用的（邪佛廣場 12、競技場 2），但新關卡照著競技場複製選單項時很容易也做成這型，於是那一關就變成「CSV 怎麼改都沒用」。
  全專案只有 `MapTable.FindLevelStart` 讀 `IsLevelStart`；正式流程（廣場選劇本進關）走的是 module 那條，所以會出現**正式流程已經換圖了、測試選單還進舊圖**的分裂狀態，更難察覺。
- **解法**：關卡類的選單項一律用 `Set("<module>")`（2026-09-02 已把血狂之爭從 `map:17` 改成 `Set("BloodFang")`），之後換起始地圖只改 CSV。`map:<id>` 只留給「該 module 首圖以外」的單張測試地圖。
- **⚠ 改完程式還要再點一次選單**：目前選的是什麼存在 **EditorPrefs**（`Dipan.DevQuickStart.Target`），改常數／改型別**不會**動到已存的舊值。舊值還是 `map:17` 時照樣進舊圖，而且選單上那一項**不會打勾**（驗證函式比對的是新字串）——「選單沒打勾」就是這個狀況的識別特徵，重點一次該項即可。
- **通則**：**同一個入口有兩種語意不同的目標型別時，要能一眼看出某一項是哪一種**——這裡「吃不吃資料表」的差別藏在字串前綴 `map:` 裡，複製貼上時語意不會跟著複製。（2026-09-02 記）

### B16. 召喚出來的怪被生到**牆外／牆裡**——用 `Physics2D.OverlapCircle` 判「這個點是不是牆」根本靠不住
- **症狀**:紅嫁衣放大絕一次召 11 隻,好幾隻直接出現在房間外面的黑色區域(2026-09-16 新娘房實機)。生怪點明明有做「避開牆/水」的檢查。
- **原因**:那個檢查是 `Physics2D.OverlapCircle(p, 0.35f, Environment|Water) == null` ⇒ 沒撞到就當空地。但專案全域 **`queriesStartInColliders = false`**(`ProjectSettings/Physics2DSettings.asset`),查詢起點若落在某個 collider **內部**,那個 collider 會被**整個略過** ⇒ 回 null ⇒ 被判成空地。**點愈深入牆裡愈容易通過檢查**,所以「稍微擦到牆邊」會被擋下、「整個埋進牆中」反而放行,看起來就是被丟到房間外。同一個設定造成的另一種症狀(貼身重疊漏抓、且不對稱)見 **B7**。房間可走區愈小、召喚半徑愈大,踩中的機率愈高——平時一次只召 2 隻、半徑 2 時幾乎遇不到,大絕一次 11 隻、半徑 3.5 就滿場都是。
- **解法**:判「怪站不站得住」一律問 **`MapNavGrid.IsWalkableWorld(p)`**,不要用物理查詢。那份格子已經把**可走層 ＋ 地上物/牆的物理碰撞(`UnionPhysics`)＋ 怪身半徑淨空(`AgentRadius`)** 聯集好了,是全遊戲同一份真相(A* 走的也是它),而且**不受 `queriesStartInColliders` 影響**。`SummonSystem` 已抽成 `IsSpawnSpotOk(nav, p)`,隨機召喚與大絕的定角召喚共用;沒有尋徑格時(單場景測試)才退回物理查詢。
- **通則**:本專案要問「這個世界座標能不能站人」,**先想到 `MapNavGrid`**;`Physics2D` 的 overlap/cast 只適合問「這附近有沒有東西」,不適合問「我是不是在東西裡面」。順帶一提**位置放不下時往外找比往內擠好**——房間小的時候往施放者身邊擠更沒位置,放遠一點沒人在意,出界才是不能接受的。
...
### C11. 可走層塗了、也存檔了，重開編輯器卻整張變回全牆（而且進遊戲角色卡死不能動）
- **症狀**：在編輯器塗好可走層 → 存檔 → **不關編輯器**去讀別張圖再讀回來，看起來是對的；但**關掉編輯器重開**再讀，整張變回全紅（全牆）。進遊戲則是角色卡在地圖邊緣完全不能動。
- **原因**：**自動生成的預覽（`EditorUI.AutoPreview`）是記憶體裡一張浮在上面的遮罩，不是地圖資料。** 2026-09-07 加自動生成時做了「預覽中筆刷改預覽本身」，於是作者生成後直接用筆刷修、再存檔 —— **塗的東西全在那張沒套用的遮罩裡，寫進檔案的還是原本的全牆**（實測檔案 6048 子格全是 `'1'`、0 個可走）。
  「讀別張圖再讀回來是對的」是**幻覺**：換地圖時沒清掉預覽，讀到別張圖因尺寸對不上而不顯示、讀回同尺寸的圖預覽又生效了，看到的仍是那張沒存進去的遮罩。
  進遊戲卡死是同一個根因的下游：整張圖沒有任何可走格，`MapManager` 的落點安全機制（`FreeSpotNear`）找不到地方放玩家。**傳送點設定本身完全正常**，查那邊是白查。
- **解法**：三道一起補（少任何一道都還是會遺失）。
  1. **換地圖就清掉自動生成狀態**：`EditorUI.Update` 用 `ReferenceEquals` 比對 `MapSession.Instance.Map` 的物件參考（讀檔/新建/Undo 還原都會換新物件），一變就清預覽、種子與快取。
...
### C13. 用 `Tools/sync_map_assets.sh` 代替 Unity 的 Sync Map Assets：catalog 會**少掉動畫地上物、而且所有 footprint 全丟**
- **症狀**（2026-09-14 加蟲族血統時踩到）：加完素材後在終端機跑 `bash Tools/sync_map_assets.sh`，輸出看起來一切正常、蟲族 30 筆也確實進了 catalog；但 `git diff` 是 **5395 行刪除 / 1200 行新增**。細查 catalog：筆數 469 → 491，其中**消失 8 筆**（`Main/Environment/Buddha`、`Cultist`、`Teleport`、`Torch1`、`Torch2`、`BloodFang_Garden_Pool`、`Ghost_Dog_Idle`、`RedBridalGown_Kneel`），而且**178 筆地上物的 `footprint` 全部不見了**。實體 PNG 一張都沒少，所以檔案面完全看不出問題。
- **原因**：**bash 腳本與 C# 版 `MapAssetSyncTool` 是兩份獨立實作**（腳本自己的檔頭就寫著這個警告，同 C1/C3/C5/I4/F16），而且 bash 版功能是**子集**：
  - 它的序列圖掃描只涵蓋 `Monsters/SequenceImage/` 與 `Characters/SequenceImage/`，**不收 `Environment/` 底下的動畫資料夾** ⇒ 那 8 個會動的地上物（佛像、信徒、傳送門、兩支火把…）直接從 catalog 消失。
  - 它**完全不算 `footprint`**（佔地格數 subdiv/cols/rows）——那是 C# 版才做的。footprint 是地上物擋路的來源之一，178 筆一次全丟。
  - 順帶：單張素材 bash 版省略 `frameCount`、C# 版寫 `1`，353 筆因此有無害的格式差異——但它會把 diff 撐大到看不出真正的損失。
- **解法**：**素材同步一律在 Unity 裡跑 `Project Tools → Sync Map Assets`**。已經誤跑 bash 版的話，`git` 還原 catalog.json 再把新素材那幾筆併回去即可（實體 PNG 是無條件覆蓋、不刪，所以只有 catalog 要救）——這次就是這樣修的：還原 HEAD 版 → 把蟲族 30 筆照舊格式（單張補 `frameCount: 1`）插進 `Main/Characters/` 那一段 → diff 收斂成純新增 526 行、`footprint` 178 筆與那 8 筆動畫地上物都回來了。
- **通則**：**「有兩份獨立實作」的警告要連『它們功能不對等』一起讀。** 腳本檔頭寫的是「改分類時兩邊都要改」，很容易被理解成「兩邊等價、跑哪個都行」；實際上 bash 版是給沒有 Unity 的場合用的**應急子集**。另一條：**驗收同步結果不能只看「新東西有沒有進去」，要看「舊東西有沒有掉」**——`git diff --stat` 出現大量刪除行就是訊號，把兩版 catalog 的 id 集合相減一次比讀 diff 快得多。

### C14. Split Sprite Sheets 對某一張合圖說「認不出格線」——防呆的前提被「倒下」這種橫向撐滿的動作打破了
- **症狀**（2026-09-14，蟲皇 `dead`）：AutoSprite 正常產出的 1280×1280 合圖，三個切割選單都拒切、跳「認不出格線」。同一個角色的 idle／walk／attack 都切得好好的，只有 dead 這一張不行；圖本身目視完全正常。
- **原因**：`SpriteSheetSplitter.GuessGrid` 初版判的是**貼邊率**——「內容有沒有**碰到**格線」，前提是「AutoSprite 每一格的角色四周都留白」。**倒下／展翅這類橫向撐滿整格的動作會打破那個前提**：蟲皇 dead 的第 7~11 幀翅膀張開，內容寬度從站姿的 151~165px 衝到 **249 / 224 / 256 / 212 / 211**（格子就是 256），5 格貼到左右格線 ⇒ 貼邊率 **20%** > 當時的門檻 10% ⇒ **正確的 5×5 被判成切錯，整張拒切**。
  ⚠ 關鍵是**貼邊 ≠ 切穿**：實測那張圖在正解格線上只有 **2 個像素**同時不透明，而且 alpha 只有 2 與 5（抗鋸齒的羽化尾巴），角色一根寒毛都沒被切掉。防呆擋的是一個其實正確的答案。
- **解法**（2026-09-14 改）：判準從「碰到格線」換成「**跨過**格線」——`CrossRatio()` 數的是**內部格線兩側同時為實體像素**（alpha ≥ `SolidAlpha` 16，濾掉羽化）的點數 ÷ 內部格線總長度，門檻 `MaxCrossRatio` 2%。**外框不算**（內容貼著圖的外緣是正常的，那不是切下去的位置）。分離度比舊版更寬：
  | 對象 | 跨線率 |
  |---|---|
  | 正解（蟲皇 dead 5×5） | **0.00%** |
  | 同一張圖的 8×8 / 4×4 / 2×2 | 21% / 29% / 49% |
  | 單張立繪 `ZhaYu/idle`、`Ghost_GrandMa` | 39% / 19% |
  | 已切好的 256 幀 | 24% |
  舊版正解 20% vs 非合圖 31%（差 11 個百分點），新版 0% vs 19%（差 19 個）。
- **⚠ 新版唯一的風險點：`cell` 剛好是正解的倍數時也會零跨線。** 例如 4×6 合圖（1024×1536、cell 256），`cell=512` 的 2×3 排版格線是正解格線的**子集**，實測同樣是 **0.00%**。靠 **「同分取較小的 cell」** 排除（迴圈 `size` 由小到大 ＋ 嚴格 `<`，天然成立）——**改這個迴圈的順序或把 `<` 寫成 `<=` 就會壞，而且壞法是「把 4 幀併成一張切出來」這種不會報錯的形式。**
- **另外兩道守衛**：切出來「有內容的格子」必須 ≥ `MinNonEmptyCells`（2），擋掉「角色只佔一角的單張圖被切成 4 格、實際只輸出 1 張」；以及原有的檔名 `_數字` 與 loneFileGuard。
- **回歸驗證**（改完當場跑的，用 Python 重現同一套算式）：把專案裡 **108 個已切好的動作資料夾**全部拼回合圖 → **0 個誤判**；同一批幀改拼成 4 欄／3 欄／2 欄 → 都切回原排版；整張放大 2×／3× → 一樣切對（`cell` 自動變 512／768）；四個非合圖樣本全部擋下。
- **通則**：**防呆的判準要對準「真正要防的那件事」，不要對準它的代理指標。** 這裡真正要防的是「把角色切成兩半」，而「內容碰到格線」只是它的**代理**——代理在多數素材上等價，卻會在邊界素材上背叛你，失敗方式還是「拒絕正確答案」這種看起來像壞掉的形式。另一條：**演算法只能假設「不變的東西」**。這支工具刻意不假設欄數、列數與圖片尺寸（AutoSprite 的排版會變、素材會被重繪工具放大），但初版不小心假設了「四周有留白」——那同樣是會變的東西。寫防呆時先把「哪些是真的不變」列出來。

### C15. NPC 在編輯器裡看起來超小，把 CSV `Scale` 填大之後遊戲裡變巨人
- **症狀**（2026-09-15，血族士兵／狂族士兵）：新加的兩隻 NPC 在地圖編輯器的預覽裡只有示範村民的 1/4，小到看不出站哪；把 `NpcTable.csv` 的 `Scale` 填到 **3** 編輯器就看起來對了，**進遊戲卻變成三倍高的巨人**（同一張圖裡的示範村民 ZhaYu 兩邊都正常，所以第一直覺會怪那兩張新素材）。
- **原因**：**編輯器與遊戲算「角色多大」的方式根本不同，一直沒對齊。**
  - 遊戲端（`MonsterController.Start`）：量 idle 首幀的**不透明像素高度**，把角色縮放成固定的 `CharacterWorldHeight`＝**1.95 世界高**（等效 PPU ＝ 可見px ÷ 1.95），**與原圖畫布多大無關**；`Scale` 只是在這之上的倍率。
  - 編輯器端（`PreviewSpriteLoader`）：PPU ＝ **256 ÷ tileSize**，等於假設「256px 畫布＝一格」，**角色顯示大小取決於人物在畫布裡佔多少像素**。
  - 實測：ZhaYu 畫布 500×500、可見高 451px → 編輯器 **1.76 格**（湊巧接近遊戲的 1.95，所以這個落差一直被蓋住）；兩隻士兵是 iso 去背圖，畫布 256 但可見高只有 **135px**（血族）／**193px**（狂族）→ 編輯器只有 **0.53／0.75 格**，差 2.6~3.7 倍。照編輯器把 `Scale` 填成 3，遊戲端就是 1.95 × 3 ＝ **5.85 格高**。
- **解法**（2026-09-15）：讓 NPC 預覽走與遊戲相同的公式——`PreviewSpriteLoader.Load` 加 `normalizeHeight` 參數（`NpcView` 傳 1.95），>0 時 PPU ＝ 可見px ÷ normalizeHeight；`Scale` 回填 1。
  ⚠ **劇情演出預覽刻意維持舊模式**（`normalizeHeight` 預設 0）：那邊的演員走位是照舊的畫布基準排好的，一起改會讓既有過場的演員大小全部變掉。要改再一起重看走位。
- **通則**：**同一件事有兩份算法就會有兩份真相**，而且壞法是「一邊對、另一邊錯」這種會誘導你去改資料的形式（這次差點把兩張好素材判成畫錯）。看到「編輯器對、遊戲錯」或反過來，**先懷疑兩邊的換算不同，別急著改資料**。
...
### E23. 人走到哪、另一邊的燈就整排熄掉（庭院這種全圖一屏的房間）
- **症狀**：四合院庭院（相機「整張地圖」模式）裡，玩家走到左半邊，右半邊的燈籠光池與傳送點冷光全部熄掉；走到右邊換左邊熄。燈籠素材本身還亮著（自發光的圖），熄的是照明光池。
- **原因**：兩個限制疊在一起。① 同框光源上限（當時 12 盞，`AtmosphereController.MaxLights`＝shader `MAX_LIGHTS`），超過的**整盞不餵給 shader**——不是變暗、是消失；② 挑選基準是**離玩家最近**——全圖一屏的房間所有燈同時在畫面上，庭院的門樓燈籠＋側牆燈籠＋六個傳送點冷光＋月光＋玩家體光超過 12 盞，「離玩家最遠的那半邊」就隨走位輪流被踢。相機跟隨的房間看不出來，因為被踢的燈都在畫面外。
- **解法**（2026-08-28）：① 上限 12 → **20**（三處同步：`MaxLights`、shader `MAX_LIGHTS`、編輯器 `LightPreview.GameMaxLights` 警告門檻）；② 挑選基準改**相機中心**——全圖模式相機不動，挑選結果恆定不再隨走位跳燈；跟隨模式相機≒玩家、行為不變。玩家自己的燈（佛燈/體光）永遠佔第一個名額，不受影響。
- **通則**：**「取離 X 最近的 N 個」這種截斷，X 要選「觀察者」而不是「角色」**——決定該不該畫的是畫面，不是玩家腳站在哪。另外編輯器照明預覽的超標警告就是防這個坑的，擺樞紐房照明時開著看盞數。

---

### E24. 踩進「鏡頭區」拉遠時，畫面邊緣露出地圖外的黑邊，約 1~1.5 秒才收回去
- **症狀**：camZone 填 `zoom=1.5`、`offsetY=2`，**從地圖邊緣那一側走進區域**時，畫面某一邊整條露出黑（地圖外），鏡頭像飄出去一樣，過 1~1.5 秒才慢慢收回正常。從地圖中間走進同一個區域則看不出來。
- **原因**：兩個獨立的洞，症狀疊在一起。
  1. **`orthographicSize` 是當幀立刻變大的，相機位置卻要靠 `SmoothDamp` 追，而 `ClampToBounds` 只夾「目標」、從不夾相機的實際位置。** 視窗一放大，相機還停在放大前那個「貼著邊界」的位置就已經超界了——貼邊進區最明顯，因為玩家本來就卡在 clamp 的極限上。更糟的是兩個過渡在互相追逐：`zoneTransitionTime=0.4` 走 `1-exp(-dt/0.2)`，收斂到 `ZoneSettled` 的 2% 容差要 ≈0.78 秒，這期間 zoom 一直變大、合法區間一直內縮，`followSmoothTime=0.12` 的 SmoothDamp 永遠落後半拍 ⇒ 破綻持續 **1~1.5 秒**。
  2. **`offsetX/Y` 加在夾制之外**：`basePos` 夾過邊界，`desired = basePos + _offsetCur` 又把它推出去。`offsetY=2` 等於合法地把鏡頭頂出地圖上緣 2 格，只要玩家靠近該邊就穩定露黑、**而且不會自己恢復**（跟過渡無關）。
  實測數字（24×27 地圖、`followViewHeightTiles=10`、16:9）：正常 halfW≈7、貼右邊界時相機 x≈17；zoom 1.5 後 halfW≈10.5、合法上限降到 13.5 ⇒ 瞬間超界約 3.5 單位，正是畫面右側那條黑帶。
- **解法**：`MapCameraController.LateUpdate` 兩道一起（只在跟隨模式且非 `_focus` 時做——對準點是刻意不夾邊界的，見該處註解）：① `desired` 加完 offset 後再 `ClampToBounds` 一次；② **`SmoothDamp` 之後、疊震動之前，對 `_cam.transform.position`（實際位置）再夾一次**。第二道才是關鍵，它保證「任何一幀」都不越界，過渡中也不例外。
- **代價（要知道）**：貼近地圖邊緣時 `zoom` 與 `offset` 會被夾住打折——想往上多看 2 格，邊緣處只給得出剩下的空間。這是不露黑邊的必然結果（地圖外本來就沒東西可看）。**要真的看到更遠的景，得從資料端解**：地圖那一側多留幾格景，或把 camZone 區域往內挪，讓玩家還沒貼到邊界就開始拉遠。
- **通則**：**「夾制」要夾最終寫進 transform 的值，不能只夾算式中間的目標**——任何在夾制之後才加上去的位移（offset、震動、後續修正）都等於繞過了夾制。另一條：**當「視窗大小」與「視窗位置」由兩套不同速度的過渡各自驅動時，中間態一定會出現不合法的組合**；不要靠「位置追上去就好了」，要在每一幀強制合法。（2026-09-02 記）

### E25. 新增 Atmosphere mode 後，那張圖變成電視雜訊（或畫面偶爾整列水平撕裂）
- **症狀**：在 `MapsTable.csv` 填了新的 `Atmosphere` 值（例如 16），shader 也加了對應分支，進圖卻看到 type 15 的雪花/掃描線；或色調對了、但畫面偶爾有一列水平撕裂。
- **原因**：`Atmosphere.shader` 的 mode 分支是**由大到小的開區間 if-else 鏈**，type 15 的判斷是 `_Mode > 14.5`——**任何比 15 大的編號都會掉進去**。而且這種開區間在檔案裡有**兩處**：frag 主分支段一處，前面「取樣前 UV 位移」段還有一處（雜訊的水平撕裂）。只改主分支、漏掉位移段，就會得到「顏色是新 mode 的、但還在撕裂」這種半套症狀。
- **解法**：新分支一律加在 `_Mode > 14.5` 那行**之前**（用更大的門檻，例如 `_Mode > 15.5`），**兩處都要**。2026-09-02 加室內系 16~19 時，主分支段照此加在前面，位移段那個則改成閉區間 `_Mode > 14.5 && _Mode < 15.5` 並加了註解。
- **順帶**：新 mode 若要吃照明，還得加進 `AtmosphereController.BuildLights()` 的白名單，否則 shader 拿到的 `v` 恆為 0、場上的燈完全沒作用（type 14 當初就漏了這個，症狀是「有照光半徑卻看不出來」）；若是暗場景，`DarknessLevel` 的 switch 也要補一格，血球 HUD 才會跟著收斂。
- **通則**：**開區間的 if-else 鏈對「往上加一個值」是不安全的**——它把「目前最大的編號」偷偷寫成了「以後所有更大的編號」。同一個列舉在檔案裡分散成多段判斷時，加值要先搜出全部的判斷點，不能只改看得見的那一段。（2026-09-02 記）

### E26. 兩個「顏色明明設得不一樣」的後處理 mode，實機看起來幾乎一模一樣
- **症狀**：Atmosphere 16（室內暖光）與 17（莊嚴金輝）的 tint 常數差很多（一個暖、一個冷→金），實機在石材大廳上並排比對卻只差一點點；順帶 16 說好的「柔和 Bloom」也完全看不到。
- **原因**：兩個獨立的原因，都出在「拿沒想清楚的量當權重」。
  1. **色彩內插直接用 `lum` 當權重**。一般場景**大半是中間調**——灰石材 lum≈0.55~0.6，正好落在內插中點。於是 16 的 `lerp(暖, 更暖, 0.55)` 與 17 的 `lerp(冷, 金, 0.55)` 都算到接近中性，兩者差不到 3%。**兩端設得再開，中點都是一樣的**：色調的差異只在曲線兩端，而畫面根本沒有幾個像素走到兩端。
  2. **Bloom 門檻用「亮點溢出」的直覺去定**（0.62）。室內是**亮場景**，整片石材 lum 才 0.55~0.6，沒有任何像素過得了門檻 → bloom 恆為 0。暗場景的 bloom 是「亮點溢出」，亮場景的 bloom 是「光瀰漫」，門檻的量級完全不同。
- **解法**：① 亮度路徑改走 `smoothstep(0.5-split, 0.5+split, lum)`，用 `split` 控制曲線陡度——split 小＝中間調被歸到暗側、只有真正的亮部才轉暖/金（17 用 0.20），split 大＝整張圖同一個調性（16 用 0.35）。② bloom 門檻 0.62 → 0.45、knee 0.30。③ 順手把兩者其他常數也拉開（16 加深暈影 0.50、17 收到 0.15；石材提亮 0.12→0.20）。
- **診斷方法**：懷疑「兩個 mode 太像」時，先估**這張畫面的主要亮度落在哪**，再把那個值代進內插算一次兩邊的實際顏色——通常會發現兩邊算出來根本一樣，那就不是顏色設得不夠飽，是**權重曲線的問題**。
- **第二輪才挖到真正的根因：門檻用錯了色彩空間（E11 的變體）**。上面那次調完，實機**還是差不多**。拿兩張截圖量出來才發現：畫面亮度中位數只有 0.32（截圖值），p99 才 0.49；而專案跑 **Linear 色彩空間**，shader 裡拿到的是 linear 值——截圖上 0.32 的中間灰，`lum` 其實只有 **0.083**。實測該場景 linear 分布：`p25=0.055 / p50=0.083 / p90=0.135 / p99=0.20`，**整張畫面都擠在 0.02~0.20**。於是：
  - `smoothstep(0.5±split)` 的門檻永遠達不到 ⇒ `mixw` 恆為 0 ⇒ **`litTint` 從頭到尾沒有參與、`split` 完全沒作用**，畫面上只有兩個 `baseTint` 的差別（量出來 R−B：16 = +0.093、17 = +0.036）。
  - bloom 門檻 0.62 → 0.45 都遠高於 p99 ⇒ **bloom 兩版都恆為 0**，「柔和 Bloom」始終不存在。
  - 石材提亮的 `smoothstep(0.15, 0.55, mx)` 同理幾乎為 0。
  修正：內插中心改成常數 `pivot = 0.085`（＝中間調中心）、`split` 改 linear 尺度（0.060 涵蓋 p10~p95＝平緩；0.030 只涵蓋 p35~p85＝陡）；bloom 門檻 **0.09**／knee 0.06（貼著中位數，讓「亮於平均的地方」開始溢光）；石材門檻改 `smoothstep(0.06, 0.16, mx)`。
- **第三輪：門檻的量級對了，但 `pivot` 的語意想錯了**。改完 linear 尺度後 bloom 確實生效（量測：p90 0.136→0.185），但**兩個 mode 又撞在一起**（R−B 差距從 +0.057 掉到 −0.007，17 的 R−B 從 +0.036 暴衝到 +0.100、變得跟 16 一樣暖）。兩個原因：
  1. **`pivot` 被當成「中間調中心」設在中位數 0.085**——那代表**一半的畫面在 pivot 之上**，配上陡曲線後那一半直接翻到 `litTint`（金），整片石材變金，當然又跟「暖」分不開。`pivot` 的正確語意是「**顏色從 baseTint 翻成 litTint 的分界**」：要「中間調維持底色、只有亮部才轉色」就得把它放在**中間調之上**（p85~p90 一帶＝0.14）；放中位數只會讓整張圖變成高光色。
  2. **bloom 加回去的是場景原色**，而石材本身偏暖褐 ⇒ bloom 越強畫面越暖，17 的 0.30 把自己的冷調沖掉了。修法：疊加時乘上該 mode 的 `litTint`（光暈的顏色本來就該是該 mode 的高光色），並把 17 收到 0.15。
- **怎麼量（以後定任何亮度門檻都照這個做，別用眼睛估）**：截個圖，把 sRGB 轉回 linear 再取 percentile——
  ```python
  from PIL import Image; import numpy as np
  a = np.asarray(Image.open('shot.png').convert('RGB')).astype(float)/255
  a = a[:int(a.shape[0]*0.75)]                     # 避開 HUD
  lin = np.where(a <= 0.04045, a/12.92, ((a+0.055)/1.055)**2.4)
  lum = (lin*[0.2126, 0.7152, 0.0722]).sum(2)
  print([f'p{p}={np.percentile(lum, p):.3f}' for p in (25, 50, 90, 99)])
  ```
  門檻就定在這組數字之間；**跨過 p99 的門檻等於沒有效果**。
- **通則**：**用「畫面亮度」當後處理的權重時，要先問這張畫面的亮度分布長什麼樣。** 線性權重會把所有中間調壓成同一個結果，而多數場景幾乎全是中間調；要讓差異看得見，得把曲線調陡、把中間調推向某一端。同一家族：AtmoTint 的暗度權重當初從 `smoothstep(0.10,0.60)` 收到 `(0.04,0.42)`，也是為了「別把中間調算進去」（2026-08-28）——**一個要放過中間調、一個要歸類中間調，方向相反，理由同一個：中間調是畫面的大宗。**
- **第三條通則**：**「權重曲線的中心點」要照效果的意圖擺，不是照畫面的統計中心擺。** 分界放在中位數＝把一半畫面判給另一端；
  想要「只有少數地方變色」，分界就要放在那少數地方的門口（p85~p90）。統計數字告訴你刻度在哪，**意圖決定你把刀切在刻度的哪一格**。
  另一條小的：**加法類效果（bloom、光暈）疊回去的是場景原色，會把 mode 自己的調性沖淡**——要嘛染成該 mode 的高光色，要嘛就別給它太大的權重。
- **第二條通則（更貴的那條）**：**shader 裡的每一個亮度門檻都是 Linear 空間的數字，不是你看截圖估的那個亮度。** Linear 專案裡「看起來一半亮」實際只有 0.08 上下，憑直覺填 0.5 的門檻永遠不會觸發，而且**症狀是「效果好像沒做」而不是報錯**，會讓人一直往「顏色不夠飽、強度不夠大」的方向調（這次就白調了一輪）。同一家族：E11（alpha 憑 Gamma 直覺填會重一倍）。順帶印證：AtmoTint 的暗度權重 `smoothstep(0.04, 0.42)` 之所以是這組小數字，正是因為它是實測調出來的 linear 尺度。（2026-09-02 記）

### E27. 全屏 Bloom：一移動就有一片斑紋跟著爬，停下來之後還會再動半秒
- **症狀**：Atmosphere 16（bloom 強）在細節多的場景（石磚縫、環形紋樣）裡，**只要移動就看到一片雜訊般的斑紋跟著跑**；放開按鍵後畫面停下來，斑紋還要再動 0.5 秒左右。氛圍本身沒問題，就是會動的那層很怪。bloom 弱的 mode（17，bloomK 0.15）完全看不出來。
- **原因**：**降採樣的抗鋸齒覆蓋不足**。首版 prefilter 從全解析度直接輸出到 1/4，卻只用 4-tap、offset 半個 texel——等於每個目標像素只平均了中心 2×2，而 1/4 降採樣需要覆蓋 4×4。高頻紋理欠採樣就產生 moiré，再降到 1/8、放大 8 倍加回主畫面，moiré 就變成大塊斑紋；相機一動，取樣格點相對場景移動，斑紋就開始爬。
  **那半秒不是 bloom 有延遲**（它沒有任何時間累積）：是相機 `followSmoothTime = 0.12` 的 SmoothDamp 餘滑——放開按鍵後鏡頭還要滑 0.4~0.5 秒才真的停，畫面還在動、斑紋就還在爬，感覺起來就成了「停下來之後雜訊才跟著動」。
- **解法**：prefilter 改 **9-tap tent、offset 一整個來源 texel**（每個 tap 靠硬體 bilinear 各自平均 2×2，合起來覆蓋約 4×4），模糊 `_Spread` 1.0 → 1.6（更糊＝殘餘斑紋更不可見；在 1/8 解析度上做，幾乎不花錢）。成本從約 0.39 提到約 0.7 次全螢幕取樣，仍然很便宜。
- **第二輪（9-tap 還是不夠）**：改成 9-tap tent 之後斑紋大幅減輕但**沒有消失**——一次從全解析度跳到 1/4，就算塞 9 個 tap，覆蓋也只有 4×4，殘留的 moiré 一移動照樣爬。**正解是逐級 2x 降採樣**：`src →(無材質 Blit) 1/2 →(4-tap box + threshold) 1/4 →(9-tap 模糊) 1/8`，合計覆蓋原始畫面 8×8。
  **而且這樣反而更便宜**：第一級不帶材質，`Graphics.Blit` 縮到一半時硬體 bilinear 本來就會平均 2×2，等於**免費的 box filter**；總取樣量 0.64 次全螢幕，比單級塞 9 個 tap 的 0.7 還低。
  同時把 **threshold 從全解析度那級移到 1/4 那級**：它是非線性運算，直接作用在高頻資料上會把微小亮度差放大成「有／沒有」，自己製造閃爍。
- **通則**：**降採樣的 tap 覆蓋範圍要跟降採樣倍率相稱**——降 N 倍就要平均 N×N 個像素，少了就是欠採樣。這是 **J5**（模糊的 tap 數要跟半徑成比例）的孿生兄弟：J5 少 tap 得到重影，這裡少 tap 得到 moiré，而且 moiré 只有在**畫面移動時**才看得出來，靜態截圖完全正常——**驗收會動的效果一定要動起來看，不能只看截圖**。
- **通則二**：**抗鋸齒要用「多級小步」而不是「一步塞更多 tap」。** 每級 2x 可以白拿硬體 bilinear 的 box filter，
  一次跳 4x 卻要自己用一堆 tap 去補、還補不滿——多級不但效果好，總成本通常更低。
- **通則三**：**非線性運算（threshold、pow、step）要放在資料已經平滑之後**，放在高頻資料上等於自己放大雜訊。
- **診斷小技巧**：「效果的瑕疵在移動停止後還持續一小段」通常不是那個效果有延遲，而是**畫面本身還在動**（相機平滑、慣性）。先問「這半秒裡相機停了沒」，比去找效果裡不存在的時間累積快得多。（2026-09-02 記）

### E28. 「角色影子偏在腳的斜後方」——**其實影子沒偏，是量測的基準選錯了**
- **症狀**：血狂之爭大廳的腳部放大圖看起來，`BlobShadow` 的橢圓中心明顯落在雙腳的斜後方，四種測試模式都一樣。量化之後更嚇人：影子中心與腳中心差 **+57px**（換算 1920 寬）。
- **原因**：**量測時拿「腳的重心」當角色的位置基準**——而角色走路時兩腿前後分開，那個重心會隨姿勢在身體中心兩側擺動數十像素，**每一幀都不一樣**。量到的 57px 幾乎全是腿部擺動，不是定位誤差。
  真正的偏差只有 **12px**（sprite 的不透明區在畫布裡不置中造成的），小到肉眼看不出來。
- **這個誤判的代價：連續改錯三次，每次量出來都「更偏」**——
  ```
  未修            +57px      ← 基準是走路中的腳
  v1 用 bounds 換算 -87px      ← 判定「更糟」，其實 v1 是對的，只移動了 12px
  v2 解析算        -76px      ← 又判定「更糟」，其實 v2 與 v1 數學等價
  v3 改成不修正      0px      ← 繞了一圈回到原點
  ```
  三個數字的差異來自**每次截圖時角色的姿勢不同**，不是程式改動造成的。
- **解法**：**把實際數值印出來**（`rect` / `pivot` / `PPU` / `lossyScale` / `bounds` / 算出的 offset），一次就看清楚：
  ```
  rect=256x256 pivotX=128.0 ppu=99.0 scaleX=0.800
  bounds cx=11.500 minx=10.465 w=2.069 | tf.x=11.500
  中心比例=0.471 → 偏移 (0.471-0.5)×2.069 = -0.060 世界單位 ≈ 螢幕 12px
  ```
  再改用「角色**整體**暗像素中心」當基準重量，得到 **-12.4px**，與理論值 -12.5px 完全吻合。
- **通則（比這個 bug 本身重要得多）**：
  1. **驗證「某個東西的位置對不對」時，基準不能選會隨動畫變動的部位。** 拿走路中的腳去量，量到的是腿部擺動。要用整體、或用靜止姿勢。
  2. **連續兩次「修正」都讓數字變糟時，先懷疑量測方法，不要改第三次程式。** 三次都在同一個假設下打轉，只是把錯誤放大。
  3. **印數值永遠比猜便宜。** 這裡花了三輪往返才做這件事，而它一次就結案了。
- **附帶結論（有用）**：這條管線（`MapSpriteLoader` 依 tileSize 定 PPU ＋ `PlayerSpriteLibrary` 執行期 `Sprite.Create`）的 **renderer bounds 是 FullRect**——`bounds.center.x` 恰等於 `transform.position.x`、寬恰等於 `rect/ppu×lossyScale`。所以影子的**寬度**（`b.size.x × widthFrac`）與**腳底 Y**（`b.min.y + b.size.y × bottomFrac`）這兩個既有算法都是對的，不必動。⚠ 但 [E14](#e14) 那條「mesh 型別沒有保證」仍然成立，所以 X 的偏移還是寫成 rect/pivot/PPU 的解析算，不依賴 mesh 型別。
- **後續（同日）**：把基準改成「只取最底部一帶的水平中心」之後，主角 idle 仍然偏、走路正常——**因為長袍、披風、背包、爪子這些突出物本來就垂到腳邊**，最底部的不透明像素根本不是腳；毛殭那類角色偏得最誇張。至此四種自動算法（整張外框中心／bounds 換算／rect·pivot·PPU 解析算／只取最底部一帶）全部有反例。
- **結論：這件事不該靠程式猜，要走專案的資料驅動鐵則**——每個角色一組偏移值調一次到位。**2026-09-02 的所有影子改動已全部還原**（`BlobShadow.cs` 回到未動過的狀態），資料化的缺口記在 [TODO.md](TODO.md)。
- ⚠ **給後來的人**：不要再往「用程式自動偵測腳在哪」投工。它的症狀是**大部分角色看起來對、少數角色明顯錯**，很難靠抽樣發現，而且每換一種服裝造型就可能多一個反例。（2026-09-02 記）
- **後續（2026-09-03）——已資料化**：量了全部序列圖之後找到「idle 偏、walk 準」的確切機制——AutoSprite 各動作的腳在畫布裡不在同一個位置（主角 idle 最底帶中心固定在中心左 25px、walk 兩腳跨在中心兩側），影子用畫布中心定位就必然如此。
  做法＝「程式先算八成正確的預設值（最底帶中心的全幀中位數）→ 存 `ShadowAnchorTable.csv` → 看拼圖不對的角色手改標 manual」，見 [SHADOW.md](SHADOW.md)〈定位：影子錨點表〉。上面「不要再投工」指的是**想靠程式全中**；算預設值仍然值得，因為它把手工量降到「只修反例」。

### E29. 角色像「貼紙貼在背景上」——背景是全畫面唯一被放大顯示的東西（解析度不一致）
- **症狀**：血狂之爭大廳（Atmosphere 17）角色透視比例都對、色彩處理（黑階抬升）也做了，作者看起來還是「貼在背景上」。放大看：角色邊緣銳利、背景石磚縫軟糊。
- **原因**：整張背景底圖全是 ChatGPT 的輸出尺寸 **1448×1086**，拉伸貼齊整張地圖之後，標準 18×10 房只有 **80 px/格**、血狂之爭 **45~63 px/格**；
  1080p 跟隨模式一格是 108 螢幕 px ⇒ **背景放大 1.35~2.4 倍顯示**（Bilinear 放大＝軟）。角色序列圖約 124 px/格、地上物 250~500 px/格都是**縮小**顯示（銳利）。
  兩種銳利度擺在一起，人眼就是會判定角色不屬於這個空間；色彩再對也沒用。[PERF_QUALITY_AUDIT.md](PERF_QUALITY_AUDIT.md) 當年是以「256px/格地磚」稽核的，改走整張背景圖之後沒人重算過。
  血狂之爭最明顯，正因它的地圖最大、px/格最低（而且作者是在那裡開始做融合 POC 的）。
- **解法**：正解＝背景每格 ≥128px（各地圖建議尺寸見 [PERF_QUALITY_AUDIT.md](PERF_QUALITY_AUDIT.md) §4.1）。過渡期＝`CharacterMipBias`：對角色貼圖設
  `mipMapBias = log2(角色px/格 ÷ 背景px/格)`，讓角色取樣密度先降到跟背景一樣（P → **M** 可即時開關）；背景換完把 `DefaultEnabled` 改 false。
- **通則**：**同一個畫面裡所有素材的「螢幕上每像素幾個貼圖像素」要一致**——有一類被放大、其他被縮小，那一類就會自成一層。
  診斷方法很便宜：每類素材算一次「貼圖像素 ÷ 佔幾格」，跟螢幕 px/格比，看誰 <1。另一條：**管線換了（地磚→整張背景），當年依舊管線做的稽核要重跑**。


### E30. 「聚光燈」黑幕的洞框不準目標——洞寫成螢幕比例，但畫面能看到幾格是隨視窗比例變的
- **症狀**：新手教學 `cameraFocus` 飄鏡頭到傳送門、壓「中央留洞」黑幕，**洞太大**（門在洞裡飄），把洞調小之後換成**門的左右被切掉**、而且門明顯偏在洞的右邊。作者在 1356×968 的視窗實測。
- **原因**：**三件事疊在一起，症狀長得很像但來源完全不同**，只修一個會再撞到下一個。
  1. **洞太大**：`TutorialDimPanel.HoleHalfX/HoleHalfY` 是**螢幕比例**常數、永遠置中，與地圖、與 trigger 位置都無關。2026-09-07 邪佛廣場畫布 90×50→48×36、地上物統一縮 0.72 之後，門從佔螢幕 26%×46% 掉到 19%×33%，洞還是 28%×48% ⇒ 從「比門大 4~8% 的貼身留白」變成「大 45%」。**改地圖尺寸時，世界座標會被轉檔腳本掃到，螢幕比例寫死的 UI 不會。**
  2. **左右被切**：跟隨模式 `orthographicSize` 固定 ⇒ **畫面高永遠 10 格，但畫面寬幾格取決於視窗寬高比**（16:9 是 17.8 格、作者的 1.40 視窗只有 14.0 格）。把半寬寫成「螢幕寬的比例」，在窄視窗框住的世界範圍就少 21%——實測門寬 299px、洞只有 274px，左右各切 12px。**這個 bug 一直都在，只是洞夠大時看不出來。**
  3. **偏移**：`ExecuteCameraFocus` 的聚焦點＝**trigger 格子的中心**，解析度 ±0.5 格；門的美術畫在背景圖裡、位置任意（`27.27`），格子中心只能是 `27.0` 或 `27.5`，怎麼塗都差 0.23~0.27 格（1080p 上約 26px）。洞縮成貼身之後這 26px 就露餡了。
- **解法**：三件分別修。
  1＋2. 洞的大小改用**世界格數** `HoleTilesW`/`HoleTilesH` 定義（現值 3.59×3.46 格，配傳送門的 3.09×2.73 格），`ShowSpotlight()` 內換算成螢幕比例時，高直接除以 `ViewTiles`(10)、**寬要再除以 `Screen.width/Screen.height`** 把視窗比例補回來。這樣任何視窗比例下洞框住的世界範圍都一樣。
  3. 給 `cameraFocus` 加 `focusX`/`focusY` 錨點（世界座標，兩個都填才生效、留空退回格子中心）——與傳送點的 `markerX`/`markerY` 同一個設計。`triggerTypes.json` 是資料驅動的，**加兩個 param 定義編輯器面板就自動長出欄位，不用改編輯器程式**。
- **診斷法（很便宜，別靠目視猜）**：截圖用 PIL 取幾條**穿過地板**（紋理均勻、不受目標物明暗干擾）的水平線印亮度剖面——黑幕是均勻 alpha 疊加，洞內外差 2.5 倍，邊界一眼可見。這次量出洞 = 螢幕 20.6%、精準置中（＝洞是對的），門中心偏右 25px＝0.26 格，才確定要修的是鏡頭不是洞。**直接對整張圖做邊緣偵測會被畫面內容騙**（第一次就抓到門的石框當洞的邊界）。
- **通則**：**「螢幕比例」與「世界大小」在正交投影下只有 Y 軸是穩定對應的**（`orthographicSize` 固定 ⇒ 畫面高恆為 N 格），X 軸還乘了視窗寬高比。凡是要框住／對齊世界物件的 UI，尺寸一律用**世界單位**定義、再換算成螢幕比例，不要反過來。另一條：**改地圖尺寸時要專門盤一次「螢幕比例寫死的表演元素」**，它們不在世界座標裡、腳本掃不到，只能靠肉眼在遊戲裡撞到。

### E32. 程序化畫「一圈線」的兩個陷阱：高斯環一加粗就糊成暈、`Mathf.SmoothStep` 不是 GLSL 的 smoothstep
- **症狀**：① 用程式畫的光環，細的時候是漂亮的一條線，作者說「線粗一點」，把寬度調大之後卻變成一團**暈開的光**，不是粗線。② 自己寫的形狀函式，門檻換成非 0/1 的值之後結果整個不對，但不會報錯、也看不出哪裡錯。
- **原因**：
  ① **高斯環（`exp(-d²)`）沒有平頂**——它的「寬度」同時是「邊緣柔化的距離」。寬度小的時候整條線都在陡峭段所以像線；一加粗，柔化距離跟著等比放大，整條就散開成暈。這是函式形狀的必然，不是參數沒調好。
  ② **`Mathf.SmoothStep(from, to, t)` 的語意與 GLSL 的 `smoothstep(edge0, edge1, x)` 完全不同**。Unity 那支是「把 t 平滑化之後在 from 與 to 之間**插值**」（回傳值落在 from~to），GLSL 那支是「以 edge0/edge1 當**門檻**把 x 映射到 0~1」。寫成 `Mathf.SmoothStep(0f, 1f, Clamp01(x))` 時兩者**剛好等價**（所以很容易一路寫對而不自知），但只要門檻不是 0/1 就靜默算出完全不同的東西。
- **解法**：
  ① **要「粗的線」就用平頂環帶**：`d = |r - c| / (w/2)`，然後 `1 - smoothstep(1-soft, 1, d)`——中間實心，只有邊緣 `soft` 那一段柔化，加粗只會讓實心區變寬。高斯留給「柔暈」那種本來就該沒有邊界的層。
  ② **自己寫一支 GLSL 語意的**，別用 Unity 那支：
    ```csharp
    ...
    ```
- **通則**：**程序化畫圖形時，先想清楚「這個函式的寬度參數控制的是實心區還是柔化距離」。** 要形狀就用有平頂的函式，要氛圍才用高斯。實作在 `BloodlineHalo.Band` / `BloodlineHalo.SStep`，用途見 [BLOODLINE.md](BLOODLINE.md) §5b。（2026-09-10 記）

### E33. 對話立繪「每換一張就飄一下」——**量的是圖檔外框，不是人物**
- **症狀**：立繪數量變多之後，對話時人物的位置與大小不穩，換一張圖就位移一次；**同一個角色的兩張表情也會跳**。用 `DramaTalkTable.csv` 的 `LeftOffsetX/Y` 之類欄位一句一句補，補不完、而且同一張圖出現幾次就要填幾次。
- **原因**：`TalkPanel.SetAvatar` 舊算法量的是**圖檔**——高度固定 `AvatarHeight`、寬度＝高×**圖檔**長寬比、左立繪把**圖檔左緣**貼 `AvatarSideMargin`。但圖檔外框 ≠ 人物：實測 132 張立繪，不透明內容佔畫布 **0.717~1.000**、左右留白 **0~16%**、畫布比例也不統一（1024×1536 之外還有 1122×1402、1149×1369、500×500）。**留白就是誤差**：`Girl_Fear` 右留白 0%、`Girl_Smile` 15.8%，同角色兩張表情在畫面上差約 78px。算下來舊算法的人物內側緣散佈達 **217px**、人物高度散佈 59px。素材尺寸沒有統一規範時，這個誤差只會越積越多。
...
- **⚠ 通則(診斷順序)**:「特效被某個地上物切斷」時**先比排序值,再懷疑碰撞**。而且 **`zOrder` 救不了**——世界帶最低是 1000(`zOrder=-1`),永遠高於腳下特效帶(可走地上物 5、地面特效 8),唯一能把地上物降進腳下帶的開關就是 `walkable`(固定 `WalkableObjectSortingOrder`)。反過來,若某個沿路/腳下特效本來就該蓋過所有地上物,就把它那筆 VfxTable 的 `SortingOrder` **留空**(改用 VfxManager 全域 22000);ID 4 目前只被火焰噴射器一把武器引用,改它零連帶影響。<br>**2026-09-23 追記**:夢境武器「龍息」(WeaponTable 62)也用火團當 `TrailEffectID`,實機火焰被**邪佛手掌蓋住**(手掌是 Y 排序帶 7000+,火是 8)——同一個成因。這次**沒改 ID 4**(那會連火焰噴射器 7 一起變),而是複製一筆 **VfxTable 47「火球(不壓地面)」`SortingOrder` 留空**給龍息用。火焰噴射器 7 仍是舊行為(火在所有角色、地上物之下),要一起改就把它的 `TrailEffectID` 也換成 47。

### E38. 子彈圖不跟著飛行方向轉（往下射，箭頭還是朝右）——`SpriteAngleOffset` 填 `0` ＝「不要轉」
- **症狀**:武器表的 `SpriteAngleOffset` 填 `0`（或留空），子彈飛出去**永遠維持圖的原始朝向**;往上下左右射都一樣,只有圖本身是對稱或圓形的子彈看不出來。以為是「補正值算錯」,把 0 改成 90/180 只會歪掉,轉回 0 又不轉了。
- **原因**:**`0` 在程式裡不是「補正 0 度」,是「這顆子彈不旋轉」的旗標**。兩處都用同一個判斷,都要 `!= 0f` 才會去設 rotation:`BallisticsEngine.cs:127`（發射當下）與 `BulletInstance.cs:141`（飛行途中每次移動,追蹤/反彈轉向後也靠它更新）。所以「圖本身已經朝右、數學上不用補」的素材反而掉進這個洞——補正值正是 0。
- **解法**:**填 `360`**。它 ≡ 0 度（`angle + 360` 與 `angle` 同向）、又滿足 `!= 0f`,而且落在武器工坊給這欄的合法範圍 `-360 ~ 360` 內。**通則:畫成「水平朝右」的子彈素材一律填 360,不要填 0**;要真的「不旋轉」（例如圓形光球、炸彈）才填 0 或留空。<br>⚠ 既有武器裡 `SpriteAngleOffset` 是 0 又有子彈圖的還有 **ID 2 彎刀／4 炸彈／11 蜂巢／13 御靈水晶**,它們現在也都不旋轉——彎刀那種帶方向感又會反彈的圖最容易被看出來,哪天覺得「這把武器的圖怪怪的」就是這條。
...
### E39. 第一次生一批怪時畫面**停一下**（夢境教學按下左鍵、四種怪一起上場那一刻）
- **症狀**：夢境教學跳「按左鍵發射」，玩家一按、第一波怪開始產生的那一刻，整個畫面卡頓一瞬間；之後再生同一種怪就不卡。作者 2026-09-23 回報，懷疑是資源問題。
- **原因**：**兩個 `MapSpriteLoader`、快取不共用**。讀取頁的「預載整個 module」（`MapLoader.PreloadModuleRoutine`）把怪物序列圖也解碼了——但解進的是 **MapLoader 自己**的快取，而怪物走的是 `MonsterSpriteLibrary` **另外 new 的**那一個。於是：① 讀取頁白解一份、還多佔一份記憶體；② 每種怪**第一次被生出來**時，牠所有動作的 PNG 在同一幀同步讀檔＋解碼，再加上「逐幀腳底對齊」對每一幀掃一次整張像素。夢境四種 ZhaYu 一起上場 ≈ 兩百多張圖，全擠在按下左鍵的那一幀。
- **解法**（2026-09-23）：新增 `MonsterSpriteLibrary.PreloadModuleRoutine`（分幀，每 8 張 yield 一次），在 `MapManager` 進入新 module 的讀取頁裡跑（進度條 0.4~0.6），連同逐幀腳底 px、腳底基準、影子錨點一起算好；`MapLoader.PreloadModuleRoutine` 改成**跳過**怪物序列圖（不再白解）。
- **通則**：**預載要暖到「真正會被用到的那一份快取」**。同一種素材被好幾個 `new MapSpriteLoader` 各自讀（目前還有 `PlayerSpriteLibrary`、劇情頭像、劇情大圖各一份），在 A 預載對 B 沒有任何幫助，只會多佔記憶體。要查卡頓時，先確認「預載的人」和「使用的人」是不是同一個 loader。

### E40. 子彈往右飛正常、**往左飛上下顛倒**（骷髏頭倒立）
- **症狀**：引魂幡（武器 37）的骷髏鬼頭往右射是正的，往左射變成頭下腳上（2026-09-24 作者截圖）。
- **原因**：`SpriteAngleOffset ≠ 0` 的子彈每幀 `rotation = 飛行角度 + 補正`，往左飛＝轉 180°。上下對稱的圖（箭、冰錐、飛劍）轉 180° 看不出來，**有上下之分的圖**就會倒立。這不是補正角度填錯，換任何角度都一樣。
- **解法**：WeaponTable 新增 `FlipYWhenLeft` 欄（填 1），`BulletInstance.ApplyFacingFlip` 在水平速度往左時開 `SpriteRenderer.flipY`（「轉 180° 再上下翻」＝水平鏡像，正是往左飛該有的樣子）。從 `BallisticsEngine.Spawn` 的 `flipYWhenMovingLeft` 參數傳入；分裂子彈靠 `Instantiate` 複製母彈繼承。見 [RECIPE_AND_WEAPON.md](RECIPE_AND_WEAPON.md)〈SpriteAngleOffset 設定說明〉。
- ⭐ **通則**：從特效庫拿**有臉／有上下之分**的飛行物時，第一次就填 `FlipYWhenLeft=1`。

### E41. 地面特效「播完一次、過一下又播一次」看起來像 bug
- **症狀**：屍毒葫蘆落地，毒雲骷髏冒一次、散掉，過一下又冒一次（2026-09-24）。
- **原因**：`GroundEffectInstance.TickAnimation` 的序列圖是**循環播放**（`% AnimationSprites.Length`），而 `Duration` 比一輪動畫長（17 幀 @10fps＝1.7 秒 < Duration 3 秒）。像毒雲、死字這種「有頭有尾的一次性動畫」一循環就穿幫；火堆、黑洞這種本來就無縫循環的圖才適合長 Duration。
- **解法**：讓 **`Duration` ≤ `AniNumber ÷ AnimFPS`**（略短一點，避免最後一幀剛好繞回第 0 幀）。屍毒雲改 `AnimFPS 8`、`Duration 2.1`（17÷8＝2.125）；想保留總傷害就把 `DamageInterval` 一起縮（0.5→0.35，仍約 6 跳）。死字咒（50÷15＝3.33）本來就是這樣填的。
- ⭐ **通則**：拿一次性動畫當地面特效時，先算「一輪幾秒」再決定 Duration；要更久就得換成可循環的素材，或另做「播完停在最後一幀」的模式（目前沒有）。

### E42. 拋物線落地的地面特效「比落點低一截」，但傷害位置其實是對的
- **症狀**：屍毒葫蘆丟到哪，毒雲卻從落點下方約 2 單位處冒出來（2026-09-24）。
- **原因**：程式端落點完全正確——`ParabolicBehavior` 落地回報的就是滑鼠目標點，落地殺傷（`TryApplyParabolicBlast`）、地面特效（`GroundEffectManager.Spawn`）、擊中特效都用同一個 `landPos`。偏的是**素材**：`fanfx2_poison` 是「從地面冒出來往上升」的動畫，第 1 幀的接地點在 128px 圖的 **y≈110**（圖中心是 64），而 Single 模式把**圖中心**對準落點、再把圖寬縮放成直徑 `2×Radius`（6 單位）⇒ 接地點被畫在落點下方 (110−64)/128×6 ≈ **2.2 單位**。
- **解法**：素材往下補透明邊，讓接地點落在圖中心（128×128 → **128×220**，原圖貼頂，y=110 正好是新圖中心）。Single 模式只看圖**寬**來縮放，所以補高度不影響大小。
- ⭐ **通則**：拿「有接地點」的特效（往上噴的火柱、冒煙、長出來的東西）當地面特效或命中特效時，先量第 1 幀的 bbox，確認接地點在圖中心；不在就補邊，不要去動程式的定位。對照 TODO 的「地獄火柱 pivot 在圖中央」是同一類問題。
- ⚠ 修好之後毒雲本體會在落點**上方**（煙本來就往上飄），而傷害圈仍是以落點為圓心的正圓——落點下方半圈看不到煙但有傷害，這是目前設計的取捨。

### E36. 大白天每隻怪身上都掛著一圈光，而且一直閃
- **症狀**（2026-09-17，競技場的戰狼）：把怪放進 `Atmosphere=1`、沒壓暗的**亮**地圖，每隻怪身上都有一圈青白光暈；
  而且圖會**一閃一閃**，像貼圖在抖。
- **原因**：**兩個獨立的成因剛好同時出現**。
  ① 怪物常駐體光是**兩層**（見 [ATMOSPHERE.md](ATMOSPHERE.md)〈怪物常駐體光〉）：`LightSource` 走照明系統、
  在亮場景自然無效；但 `CharacterGlow` 是**加色**光暈（`Blend One One`），**不受「這張圖吃不吃照明」限制**，
  所以亮場景照樣亮。`MonsterSpawner` 那段註解寫著「亮場景 shader 根本不讀光源，這盞燈不會有任何副作用」——
  **那句話只對第一層成立**，第二層被漏掉了。
  ② 閃爍是另一回事：`CharacterGlow.LateUpdate` **每幀**用 `_charSr.bounds` 算直徑與中心，而逐格動畫的素材多半是
  **trim 過**的（每幀去完透明邊之後高度都不同）⇒ 光暈直徑與中心每幀跳動。**暗場景其實一直在閃**，
  只是被黑暗蓋掉，直到放進亮場景才看得出來。
- **解法**：① `CharacterGlow` 加 `OnlyInLitAtmosphere`（預設 false＝零行為變化），怪物體光掛的時候設 true，
  查新開的 **`AtmosphereController.LightsEnabled`**（那條判定原本抄在兩處，一併收斂成單一真相）。
  ② 直徑改取「遇過的最大可見高度」（單調不減、動畫跑完一輪就定下來），中心偏移量一次快取，
  X 用 `transform.position.x`（`bounds.center.x` 會被 `flipX` 帶著跳）；體型變了呼叫 `RefreshSize()` 重量。
- **兩條通則**：
  1. **「這個效果在亮場景會不會有副作用」要分層問**——走照明系統的（乘法）自然會被擋掉，**加色的不會**。
     之後做任何「只該在暗場景出現的視覺」，一律查 `AtmosphereController.LightsEnabled`，別自己再抄一份氛圍編號清單。
  2. **不要每幀拿 `SpriteRenderer.bounds` 當尺寸來源**——只要素材是逐格動畫又 trim 過，你算出來的東西就會每幀抖。
     要嘛量一次快取，要嘛取遇過的最大值。同一家族的坑：**E33**（對話立繪量的是圖檔外框不是人物）。

### E37. 程式生成的怪**沒有繼承 prefab 的 Rigidbody2D 設定**——沒開內插，於是「看單幀位移」的判定全部誤判
- **症狀**（2026-09-17，撲擊型的戰狼）：衝刺明明設定成「一路衝到玩家身上」，實機卻**每次只前進約 1.2 單位就停下來咬空氣**，
  玩家站著不動也一樣，要撲三四次才累積到貼身。改了三輪行為邏輯（追蹤目標、動態時限、邊緣距離判定）都沒用。
- **原因**：**兩件事疊在一起，單獨看都不像問題。**
  ① `Monster.prefab` / `Player.prefab` 的 `m_Interpolate` 都是 1，`MonsterController` 的註解也寫著
  「玩家/怪物 Rigidbody2D 已開 Interpolate（見 **E5**）」——但那**只對「CSV 有填 `PrefabPath` 的舊怪」成立**。
  route B（程式量產）的怪是 `new GameObject` ＋ `AddComponent`，`Rigidbody2D.interpolation` 拿到的是
  Unity 預設值 **None**。程式裡從頭到尾沒有任何一行設過它。
  ② 物理是 **60Hz**。沒有內插時，`transform.position` **只在有物理步進的那一幀更新**——
  畫面只要跑得比 60fps 快（120Hz 螢幕、或關掉 VSync），就會有整幀位移是 **0**。
  於是任何「這一幀動了多少」的判定都會週期性拿到 0：
  - `PounceBrain` 的撞牆判定 ⇒ 起跑緩衝一過，第一個「沒動」的幀就把衝刺判成撞牆而中斷。
  - `MonsterActuator.UpdateStuck`（`moved < MoveSpeed × Time.deltaTime × 0.4`）⇒ 怪會莫名其妙側滑解卡、
    累積夠了甚至進「放棄期」站住 0.6 秒。**這一條影響的是全部的怪，不只撲擊型。**
- **解法**：① `MonsterActuator.Awake` 補 `_rb.interpolation = RigidbodyInterpolation2D.Interpolate;`
  （與 prefab、與 E5 的要求一致，順便畫面更順）。② 會因此出錯的判定改成**看一段時間的累積位移**
  （`PounceBrain` 改成每 0.2 秒結算一次），這樣即使哪天有人把內插關掉也不會壞。
- **三條通則**：
  1. **prefab 上的元件設定，程式生成的物件一個字都不會繼承。** 專案同時有「prefab 路線」與「程式量產路線」時，
     凡是 prefab Inspector 上調過的東西（Rigidbody 內插、Layer、物理材質…）都要在程式那條路線補一次，
     否則兩條路線的行為會不一致，而且**只有走程式那條的才壞**。
  2. **不要用「單幀位移」判斷「有沒有在動」。** 渲染幀率與物理步進本來就不同步，單幀位移隨時可能是 0。
     要嘛看累積位移，要嘛看 `Rigidbody2D.velocity`。
  3. **靜態模擬測不出這一類 bug**——模擬裡每一幀都有位移，不存在物理與渲染不同步。模擬能驗「邏輯對不對」，
     驗不了「引擎的時序」。症狀是「模擬說沒問題、實機就是不對」時，先懷疑時序而不是繼續改邏輯。

### E31. 面板上的名字四個字就掉到第二行（三個字都好好的）——字級寫死＋uGUI 預設會自動換行
- **症狀**：血統揭示面板的石碑上，「山嶽巨人」變成兩行；殭屍、毛殭、旱魃、覓血者、該隱、狼人、芬里爾（都是 2~3 字）從來沒出過事。作者 2026-09-09 回報。
- **原因**：算得出來的必然結果，不是偶發。`BloodlineIntroPanel` 的字區寬 ＝ `PlateW`(360) × `NameArea.width`(0.60) ＝ **216px**，字級 `NameFontSize` 寫死 56；毛筆字 Bakudai 是全形等寬 ⇒ 中文一字約一個字級 ⇒ 216 ÷ 56 ＝ **3.86 個字**。而 `UIBuilder.Text` 建出來的 uGUI `Text` 是 `horizontalOverflow = HorizontalWrapMode.Wrap`（那是 UIBuilder 的預設，不是這支面板設的），所以第 4 個字不是擠出去、是**折到第二行**。三個字的資料剛好全部塞得下，把這個上限藏了好幾個月。
- **解法**：設完 `text` 之後依字數夾一次字級——`fontSize = min(NameFontSize, floor(字區寬 ÷ 字數))`（`BloodlineIntroPanel.FitFontSize`）。四個字算出來 54、五個字 43，三個字仍是 56（**位元級零影響**，版面其餘部分都照 `NameFontSize` 算，所以只會讓長名字變小、不會把短名字放大）。**刻意不把 `NameArea` 拉寬**：那一欄當初就是為了避開石碑左右的尖刺裝飾才收到 0.60。
- **通則**：**「寫死的字級 ＋ 會自動換行的容器 ＋ 長度來自資料的字串」＝一顆等資料變長才引爆的地雷。** 三者只要有一個不成立就不會出事，所以它能安靜很久。凡是「文字內容來自 CSV／玩家命名」的固定版面，加資料時順手算一下「這個框放得下幾個字」；框放不下就縮字，不要動為了避開美術裝飾而收窄的邊界。
...
### F27. 「怪一直揮但打不到我」——**調大怪物 Scale 之後**攻擊範圍悄悄失效
- **症狀**:近戰怪站在玩家旁邊不停揮武器,**玩家完全不掉血**;把 `MonsterData.csv` 的 `Scale` 調小又好了。作者的第一反應是「不能因為我調大小就得再調攻擊範圍啊?」——**這個直覺完全正確**。
- **原因**:自己算「攻擊圈的圓心與半徑」時,踩到兩件事疊在一起:<br>① **寫死的世界單位**:`半徑 = 框半寬 + 0.55`。怪放大時**劍也跟著放大**,但那個 `0.55` 不會 ⇒ 相對變短。<br>② **兩邊的垂直基準不一致,而落差隨 Scale 線性長大**(這才是致命的):攻擊圈以**怪的腳底**為圓心,而**玩家的 `CircleCollider2D` offset 是 0 ⇒ 圓心就在玩家的 transform**。route B 怪物的 pivot 在畫布中心(見 **G13**),腳底離 transform 有**半個身高**,那個差距 **∝ Scale**。<br>實測(狂族皇家衛士):圈邊緣離玩家的圓還差多少才碰到 —— **Scale 1.3 ⇒ 多 0.27、1.5 ⇒ 0.23、2.0 ⇒ 0.12**,再大就是負的 ⇒ 永遠打不到。**Scale 就是那個隱形的開關。**
- **解法**:**命中判定不要自己算圈,改成問「兩個碰撞框的邊緣距離」**(`Physics2D.Distance`),而且要與「決定出手」用**同一個函式**。判定過了,才在**目標的碰撞框中心**開一個半徑很小的傷害圈 ⇒ 必定重疊,不會因為圓心算錯而 miss。範圍 AOE(如踐踏)無法這樣做的,圓心一律用**怪的 transform**,不要用腳底——因為玩家的碰撞圓就是以 transform 為心。
- **通則一**:⭐ **「決定出手」與「判定命中」必須用同一把尺。** 兩者各自獨立設定,就沒有任何東西保證後者涵蓋前者——動作演完了卻沒有人受傷,而且**在某一個體型下剛好會過,換一個體型就不會**。(同一條通則在 §10.2 已經踩過一次:「停止移動」的門檻必須等於傷害判定的門檻。這是第二次。)
- **通則二**:⭐ **凡是「以某個點為圓心開範圍」的判定,先問「對方的碰撞框是以什麼為基準」。** 這個專案裡玩家 pivot 在腳底、route B 怪物 pivot 在畫布中心,**兩邊的 transform 語意不同**;拿其中一邊的「視覺位置」去和另一邊的「碰撞框」比,誤差會隨體型放大。**視覺用腳底、判定用 transform**,兩者刻意不一致。
- **附帶**:所以畫面上「裂痕的圈」與「實際打得到的圈」會差一個半身高的深度。**那不是 bug**,是 pivot 不一致的後果;玩家的圓半徑 0.5 會吸收一部分。真要對齊得改 route B 的 pivot,那會動到碰撞框、影子錨點與顯示大小,不值得。
...
### F28. 換了一個**畫布尺寸不同**的動作序列圖 → 那個動作整組上下位移（走路時怪「瞬移」貼到玩家身上、對話框離圖很遠、看起來還沒碰到就被撞）
- **症狀**（2026-09-22 ZhaYu，作者附圖）：怪走過來，**快到時整隻跨越一小段距離貼到玩家身上**，動作變 idle、玩家扣血、怪又「退回去」，然後再瞬移過來，反覆震盪；怪頭上的**對話框離圖很遠**；更早之前還出現過「**圖都還沒碰到就被撞到**」。同樣是 `Chase` 衝撞型，**紅嫁衣那邊的家人幽靈完全正常**。
- **原因**：腳底對齊的 pivot 補償公式 `MapSpriteLoader.GetAnimationFrames` 寫成<br>　`pivotY = (該幀腳底px − (baselineBottomPx − **bp.y** × 0.5) × tileRatio) ÷ bp.y`<br>括號裡那一項要算的是「**基準幀**（idle 第一幀）的腳底離**基準幀**畫布中心多遠」，但 `bp.y` 是**當前這一幀**的畫布高。**兩個不同畫布的像素被混在一起相減。**<br>全部動作同尺寸時 `bp.y == 基準畫布高`，剛好等價 ⇒ **靜默算對**，所以這個 bug 藏了很久。ZhaYu 的 idle 是 **256px** 畫布、walk 換成一批 **500px** 的圖之後就爆了：實測 walk 的 pivot 被算成 **1.0369**（跑到畫布上緣之外），整組 walk 幀**往下位移 1.15 世界單位**（× CSV `Scale` 1.2 ＝ **1.38**，超過半個身高）。
- **於是三個症狀都是同一件事**：① 怪**走路時**圖在下方、**停下切 idle** 圖彈回上方 ⇒ 看起來就是「瞬移過來又退回去」；② 碰撞框是用 idle 建的、固定在 transform，圖卻跑掉 ⇒ **看到的距離不是真實距離**，玩家以為還很遠其實早就重疊（那就是更早那次「還沒碰到就被撞」的真正原因）；③ 對話框掛在 `FeetWorldPos + VisibleBodyHeight`（idle 的幾何），圖偏走了它沒有 ⇒ **框離圖很遠**。
- **解法**：基準的「腳底 px」與「畫布高 px」**必須成對傳、量自同一張圖**。`GetAnimationFrames` 多收一個 `baselineCanvasPx`，公式改成 `baseOffsetPx = baselineBottomPx − baselineCanvasPx × 0.5`；`MonsterSpriteLibrary.BaselineBottomPx` 改回傳 `Vector2Int(腳底px, 畫布高px)`（`GetFrameBottomPx` 本來就兩個都給了，舊版把畫布高丟掉）。修完實測：ZhaYu 的 idle/walk 腳底位移都是 −0.9891（完全對齊），**其餘三隻 ZhaYu 與所有既有怪（全 256px）新舊 pivot 完全相同 ⇒ 零行為變化**。
- ⚠ **這個坑文件早就預告過**：[MONSTER_SETUP.md](MONSTER_SETUP.md) 的碰撞框段落寫著「`GetAlphaLocalBox` 的 `canvas` 欄……**不能假設畫布是 256px**，目前所有角色圖剛好都是 256×256，**漏除會靜默算對，哪天丟一張 512px 的進來才會爆**」。那一天就是 2026-09-22。
- ⭐ **通則一：「同一隻怪的所有動作要同畫布尺寸」不是美觀建議，是硬契約。** 換素材時**先比對畫布尺寸**（`ls` ＋ 一行 PIL 就看得出來），不一致就先統一，不要指望系統補償。
- ⭐⭐ **通則二（這次真正該學的）：兩個素材長得一樣、只有一個出問題時，先比對「素材本身的差異」，不要先去改共用系統。** 這次的直接證據是作者說的「**紅嫁衣那邊的鬼魂一樣是衝撞型，完全正常**」——同樣的 Brain、同樣的碰撞設計，差別只在素材。當時若先 `ls` 一下兩邊的畫布尺寸，五秒就會看到 500 vs 256。**改了兩輪共用的碰撞判定都沒修好，因為根本不在那裡。**
- ⚠ **診斷時「圖在哪」與「碰撞框在哪」要分開驗**：症狀全部長得像碰撞問題（撞到／沒撞到／太早撞到），但成因在**顯示**。把兩者分開量（碰撞框可以用紙筆算；圖的位置要看 pivot）就不會被症狀帶著走。

### F29. 把怪的 `Scale` 調大之後，**範圍技（爆炸／踐踏）就打不到玩家了**
- **症狀**：`ZhaYu_Bomb` 體型 1 時自爆正常，調到 **1.5** 之後「引信照樣點著、火球照樣炸開，**玩家完全不掉血**」。作者 2026-09-22 回報，並指出「這個雷之前踩過，請從根源修正」。
- **原因**：**F27 的第一條死因換個地方又出現了——寫死的世界單位不隨體型長大。**<br>觸發與殺傷用了**兩把會不同步的尺**：<br>‧ **引信**用碰撞框的**邊緣距離**（`Physics2D.Distance`）⇒ 框隨 `Scale` 變大，觸發時的**中心距離跟著變遠**；<br>‧ **殺傷**是 `ImpactDamageArea` 的半徑，直接拿 CSV 的 `BombRadius` 當世界單位 ⇒ **完全不隨體型變**。<br>⇒ 體型一放大，觸發距離追過殺傷距離，就變成「炸了卻炸不到」。實測（`BombRadius` 1.8、對角方向最壞情況）：<br>　`Scale` 1.0 觸發 2.36／殺傷 2.30（**已經是臨界，勉強能中**）、1.2 → 2.66／2.30、**1.5 → 3.11／2.30**、2.0 → 3.86／2.30。<br>⚠ `LeapSlamBrain` 的 `LeapRadius` 是**同一個寫法**，狂族皇家衛士 `Scale` 1.5 ⇒ 踐踏範圍其實一直相對縮水，只是還沒被實測到。
- **解法**（兩層，缺一不可）：
  1. **半徑隨體型縮放**：`MonsterController.ScaledRadius(baseRadius) = baseRadius × BodyScale`。**怪的所有 AOE 半徑一律走這支**。CSV 的半徑欄語意因此是「**體型 1 時的半徑**」，填表照體型 1 去想即可。特效倍率吃的是縮放後的半徑，所以**火球也跟著變大**，不會出現「炸得到卻看不出來」。
  2. **防呆下限「引信點著就一定炸得到」**：`radius ≥ 自身碰撞框半對角 ＋ FuseSlack`。<br>只做第 1 層還會從**另一端**破：引信距離裡有一截**固定量**（`FuseSlack`）不隨體型縮，所以體型**小**到某個程度（實測 `Scale` < 0.47）同一個坑會反向出現。<br>⭐ 這條式子**不必知道玩家多大**——玩家的碰撞半徑在「觸發距離」與「殺傷距離」兩邊都有、自動抵銷。
- ⚠ **既有行為變動**：狂族皇家衛士（`Scale` 1.5）的踐踏半徑 1.6 → **2.4**。這是修正的一部分（大怪本來就該踩得更廣）；想維持原樣就在 `MonsterData.csv` 的 `LeapRadius` 填 **1.067**（＝1.6 ÷ 1.5）。
- ⭐ **通則一：一次攻擊的「觸發」與「殺傷」必須用會同步變動的尺。** 一個吃體型、一個不吃，就一定存在某個體型讓它們錯開——而且**沒有任何錯誤訊息**，`Scale` 就是那個隱形的開關（同 **F27**）。
- ⭐ **通則二：範圍技的半徑不能是裸的世界單位常數。** 凡是寫在 CSV／常數裡的半徑，都要先問「怪放大時它會跟著大嗎」。F27 修掉了近戰揮擊（改成問邊緣距離），但**它明講 AOE 沒辦法這樣做、圓心要用 transform**——那句話等於在 AOE 這邊留了一個洞，這次就是從那個洞掉下去的。**修一類 bug 時，被自己排除掉的那個例外要當場補上。**
- 🔧 **驗算方式**：對每個體型列一張「觸發距離 vs 殺傷距離」的表（兩者都是紙筆算得出來的），看有沒有哪一列反轉。比在遊戲裡一個一個試體型快得多，也才看得出 `Scale` 1.0 其實早就在臨界。

### F30. 拋物線武器的子彈圖放大後，**落地傷害範圍比畫面上的爆炸／毒雲大很多**
- **症狀**：屍毒葫蘆（配方 83）填 `AreaRadius 1.0`、畫面上的毒雲半徑 1.5，實際上站在毒雲外好幾格的怪也被落地那一下炸到。
- **原因**：Parabolic 的落地殺傷是 **`AreaRadius × BulletScale`**（`PlayerController.TryApplyParabolicBlast`，設計上是讓須彌珠「爆炸範圍跟著子彈一起變大」）。但像素飛行物為了看得見，`BulletScale` 常填到 4~9 ⇒ 殺傷半徑跟著被乘了好幾倍。而**擊中特效大小只吃 VfxTable `Scale`（×集氣倍率）、地面特效只吃 GroundEffectTable `Radius`**，三者互不連動，所以畫面上完全看不出來。
- **解法**：`AreaRadius` 填「想要的實際半徑 ÷ `BulletScale`」（屍毒葫蘆：想要 2 ⇒ 2 ÷ 8.4 ≈ **0.24**）。**改拋物線武器的 `BulletScale` 時，要同一次回頭改 `AreaRadius`**；擊中特效／地面特效的大小則要各自到 VfxTable／GroundEffectTable 對。
- ⭐ **通則**：同一把武器的「子彈圖大小／殺傷範圍／命中特效大小」可能分散在三張表、各自吃不同的倍率。調任何一個之前，先確認另外兩個會不會跟著變（Melee／SkyStrike 的半徑也乘 `BulletScale`，Normal 的判定 `Radius` 則**不**乘）。

### F31. 大判定半徑的子彈**貼牆往外射，一出生就撞牆**（當場播命中特效、飛不出去）
- **症狀**：餓鬼牙符（判定 `Radius 1.6`）站在左牆邊往右射，子彈剛生出來就在原地播咬合特效（2026-09-24 作者截圖）。
- **原因**：`BulletInstance.CheckSpawnOverlap` 在出生點做 `OverlapCircle(半徑＝Radius)`，碰到任何東西就算命中並銷毀；`Update` 的 `CircleCast` 起點若已重疊也會回 distance 0 的命中。半徑 1.6 的圓在玩家貼牆時會蓋到**背後**那面牆 ⇒ 往外射也當成撞牆。半徑 0.1~0.4 的一般子彈蓋不到，所以之前沒出現過。
- **解法**（2026-09-24，彈道核心，對所有子彈生效）：新增 `IsBehindOverlap`——重疊物的最近點在子彈**背後或正側邊**（`dot(最近點−位置, 移動方向) ≤ 0`，子彈正在離開它）就不算命中；出生檢查改 `OverlapCircleAll` 取第一個不在背後的，飛行中起點重疊且在背後時改用 `CircleCastAll` 找這一步路上真正擋路的。**子彈中心本身在碰撞體裡**（最近點＝自己）仍照舊算命中。
- ⚠ **行為變動**：以前貼牆往外射的大子彈會自爆，現在會正常飛出；站在怪身上往外射，背後那隻也不會被出生重疊打到。前方的牆／怪、生在牆裡的子彈都跟以前一樣。
- ⭐ **通則**：把子彈判定 `Radius` 調大（跟著視覺放大）時，出生點附近的東西都會被圓圈蓋到——出生重疊檢查一定要看「方向」，不能只看「有沒有碰到」。
...
### G7. 主角一出手就「長大一圈」、收手又縮回去（attack 幀比 idle 大）——高度已經對齊了還是大
- **症狀**：Base 攻擊時整個角色明顯比站著大一號，攻擊完縮回去；其他血統程度不一。素材是 AutoSprite 產的，各動作的角色大小本來就抓不準。
- **原因**：`PlayerAnimator.Setup` 從 2026-08-18 起就會把 walk/attack 的顯示縮放對齊 idle，**但它量的是「可見高度」、而且只量「第一幀」**，兩個盲點剛好都被 attack 踩中：
  1. **高度對「畫粗了」無感**：Base 的 attack 是微蹲＋出拳，可見高只比 idle 高 4%（201 vs 193px），程式只縮 0.96；但整個人（頭、軀幹、腿）都畫大了一圈，不透明面積開根號是 idle 的 **1.14 倍**。高度對齊之後看到的還是一個頭大一號的角色。
  2. **高度對「蹲下」反而會放大**：Nightborn 的 attack 是蹲姿，高度比 idle 矮 → 被**放大** 1.036，而它的體積已經是 idle 的 1.12 倍。
  3. **第一幀不代表整段**：`TryGetVisibleBox` 走 `GetAlphaLocalBox(item)`，而 catalog 的 `item.path` 是該動作第一張圖。Maojiang attack 第一幀高 190、全段中位數 180，差 5%。
  七組全量測（25 幀取中位數）：Base attack √面積比 1.143、Nightborn 1.121、Jiangshi 1.107，其餘 0.96~1.03；walk 三種指標大致一致，舊高度法本來就對。
- **解法**（2026-08-27）：`PlayerSpriteLibrary.GetActionSize`——每個動作**掃全部幀**，各幀量「可見框高」與「√不透明像素數」各取**中位數**，尺度＝√(高 × √面積)（兩者的幾何平均）；`PlayerAnimator.StateTile` 改成 idle 尺度 ÷ 該動作尺度。√面積對均勻放大是線性的、對「手伸出去」只有小幅影響，補高度的盲點；高度不會被伸出去的手臂／披風騙，補純面積會多縮的問題（Jiangshi attack 身體沒變大、只是手伸很長，純面積會多縮 10%）。結果：Base attack ×0.917、Jiangshi ×0.945、Nightborn ×0.961，其餘 0.99~1.06。`Project Tools → 角色 → 攻擊動畫幀數報告` 會印每個血統 walk/attack 的縮放與原始量測值，換圖後掃一眼：**離 1 很遠代表那組圖大小抓歪了**。
- **刻意不做的**：不加血統表手填欄（同 G6：只跟圖有關的資訊從圖算，不變成人工維護的資料）、不離線縮圖（要放大的血統會撞 256×256 畫布邊、每加一個血統都得記得跑腳本）。跨血統仍只用 idle 高度定絕對顯示高度（`BodyScale` 那條規則不變，見 BLOODLINE §2）。
- **通則**：**「已經正規化過了」要追問「正規化的是哪個量、量的是哪一幀」。** 高度只是大小的一個投影，姿勢一變（蹲、彎腰、出拳）它就跟「這個人有多大」脫鉤；對齊「大小」至少要一個對姿勢不敏感的量（面積）來配。另一條：**代表幀請用整段的中位數，不要用第一幀**——序列圖的第一幀常是起手，剛好是最不典型的那張。


---

### G8. AutoSprite 的 perfect loop 合圖切不了——`Split Sprite Sheet` 說「尺寸不符」
- **症狀**：AutoSprite 除了完整的 25 張，還會輸出它推薦的 **perfect loop** 版（張數較少）。這種合圖丟進 `Project Tools → Split Sprite Sheet` 會直接被擋，跳「尺寸不符」。實例：`Gargoyle/idle/Gargoyle-iso_idle_right-trimmed.png`，**1280×768**。
- **原因**：`SpriteSheetSplitter` 把格數寫死成 `GridCols = GridRows = 5`，檢查是 `寬 % 5 == 0 && 高 % 5 == 0`。**768 % 5 = 3 → 擋下**。但那張圖其實完全正常：依 5 欄算格邊長 ＝ 1280÷5 ＝ 256，高 768 ÷ 256 ＝ **3 列**，5×3 ＝ 15 格，而且（逐格量 alpha 驗過）**15 格全部有內容**。也就是說 **AutoSprite 的 perfect loop 是「列數變少」，不是欄數變少、也不是 25 格的殘缺版**——寫死 5 列的那個假設從一開始就只對「完整版」成立。
- **解法**：**欄數維持固定 5，列數改成從圖反推**——格邊長 ＝ 圖寬 ÷ 5、列數 ＝ 圖高 ÷ 格邊長（AutoSprite 的格子是正方形，所以能這樣反推），再驗兩件事：`高 % 格邊長 == 0`（不然格子不是正方形＝圖被裁過）、列數 ≤ `MaxRows`(10)（誤選超高長圖的防呆）。驗算：1280×1280→5×5、1280×768→5×3、1280×512→5×2、5120×3072→5×3（每格 1024）全部正確。
- **⚠ 欄數刻意不一起改成自動推算**：批次模式（「整包就地切割」）的第二道冪等保險，靠的就是「**已切好的單張幀邊長 256／512／1024 都不是 5 的倍數**」而被寬度那關擋掉。欄數一旦自由推算，正方形的單張幀就會被判成合法的 1×1／2×2 合圖 → 上次切出來的幀被再切一次、改名 + 刪原檔，**整包靜默重排**。所以固定的那一維要留著。
- **相關**：張數不滿 25 對**播放**沒有影響（catalog 記幾張就播幾張），但 `AnimFPS` 是「每秒幾幀」不是「整個動作幾秒」，所以**循環時間 ＝ 張數 ÷ AnimFPS**——換 perfect loop 要把 `AnimFPS` 一起改成新張數，否則動作會變快。見 [MONSTER_SETUP.md](MONSTER_SETUP.md) 的 `AnimFPS` 欄說明。
- **通則**：**寫死的「資料形狀假設」要分清楚哪一維是真的固定、哪一維只是當時剛好都長那樣。** 這裡真正固定的是欄數（AutoSprite 的排版），列數只是「當時只看過 25 張的版本」。把可變的那一維改成從資料反推、固定的那一維留著——而且要先查一下**固定的那一維有沒有兼著別的職責**（這裡兼著批次冪等守衛），否則「全部改成自動」會順手拆掉一道保險，而且拆掉的當下完全沒有症狀。（2026-09-09 記，見 [CHARACTER_SETUP.md](CHARACTER_SETUP.md)）

> **⭐ 2026-09-09 續：連欄數也不再寫死了（素材要高清化，尺寸與格數都保證不了）。**
> 作者接下來會把序列圖高清化（放大 2~4 倍），加上 perfect loop 的張數浮動，**尺寸與格數兩個維度都不固定**，
> 上面那個「固定 5 欄」的解撐不住，所以改成**從圖本身推測格線**。
> **關鍵觀察**：AutoSprite 每一格的角色四周都留白 → **切對時沒有任何一格的內容會碰到格線，切錯就幾乎每格都被攔腰切開。**
> 演算法（`SpriteSheetSplitter.GuessGrid`）：① 候選格邊長 ＝ 圖寬與圖高的**公因數**（格子是正方形），
> 濾掉格邊長 < `MinCell`(32)、格數 < 2 或 > `MaxCells`(64) 的；② 對每個候選算**貼邊率**＝內容碰到格線的格數 ÷ 總格數，取最低；
> ③ 最低仍 > `MaxEdgeRatio`(10%) ＝ 認不出格線，不切並附候選一覽。
> **實測分離度極高、中間完全沒有模糊地帶**：
>
> | 圖 | 最低貼邊率 | 判定 |
> |---|---|---|
> | Gargoyle idle 1280×768（真合圖） | **0%**（5×3） | 切 |
> | 紅嫁衣 walk 1280×1280（真合圖） | **0%**（5×5） | 切 |
> | `ZhaYu/idle` 500×500 單張立繪 | 76%（最低的是 5×5！） | 不切 |
> | `Ghost_GrandMa` 408×612 單張立繪 | 33% | 不切 |
> | 已切好的 256×256 幀 | 31% | 不切 |
>
> ⚠ **注意後三列：光看尺寸的幾何過濾擋不住它們**（500×500 會產生 5×5 候選、256×256 會產生 8×8／4×4／2×2 候選），
> **真正的守門員是貼邊率**。所以推測的信心門檻不是裝飾，是批次模式的安全繩。
> **批次守衛因此改成三道**：① 檔名結尾 `_數字` → 跳過；
> ② **「整包就地切割」限定：同一資料夾內 PNG 超過一張 ＝ 已經切好，跳過**（作者的製程：要切的動作資料夾裡只放一張合圖，
> 要重切就把資料夾清空只留合圖）——**「切到檔名子資料夾」不套用這條**，那個模式的來源資料夾本來就會放好幾張 sheet；
> ③ 認不出格線就不切。單張模式只有第三道（親手選檔＝明確意圖）。
> **通則**：**當「資料的形狀」再也保證不了時，改去找「資料內容裡不變的性質」**——這裡不變的是「格線上不會有像素」，
> 它跟解析度、格數、張數全都無關，所以一次解決了兩個維度的浮動。找這種性質時要挑**分離度大**的
> （0% vs 31%~100%，中間空得很開），分離度小的訊號當守衛遲早會誤判。

### G9. 新血統的攻擊動畫只播得到前 2~4 幀（起手抖一下、出拳完全沒播）——G6 那條規則的前提被素材打破
- **症狀**：石像鬼只播 2 幀、泰坦 4 幀，看起來像「抖一下就結束」，後面的出拳整段沒播。作者 2026-09-09 回報。回頭全量掃描才發現**狼人與覓血者也是 2 幀**（狼人在 [BLOODLINE.md](BLOODLINE.md) §8 早有記錄，覓血者一直沒人發現）。
- **原因**：G6 的起播／結束幀是拿「這一幀與 idle 站姿的輪廓差異」當**動作進度曲線**，結束幀＝差異**第一次**到峰值 90% 的那格。這條規則有一個沒寫出來的前提：**attack 起手 ≈ 站姿 → 中間出手到底 → 曲線有一個峰**。素材若是「整段都跟站姿差很多」（石像鬼 idle 是蹲踞石像姿、attack 全程站起來揮擊；泰坦、狼人、覓血者同理），曲線就變成一條在高檔震盪的平線——石像鬼實測 `0.78 0.72 0.62 0.75 0.74 …`，**第 0 幀就是峰值**；泰坦是 0.35~0.46 幾乎全平。這時「第一次到 90%」等於在雜訊裡挑第一個點，抓在哪一格是隨機的。**用「振幅」（(峰值−最低)÷峰值）量這條曲線有多少結構，13 個血統分得很開**：芬里爾 84%／該隱 74%／望月者 65%／血伯爵 59%／旱魃 55%／山嶽巨人 52%／毛殭 49%／殭屍 44% ‖ 石像鬼 30%／泰坦 24%／狼人 21%／覓血者 19%／Base 17%。
- **解法**（2026-09-09）：`PlayerSpriteLibrary.ComputeActionRange` 末尾加一道**演算法失效偵測**——`振幅 < ActionFlatCurveAmplitude`(0.35) **且** `播放幀數 ÷ 總幀數 < ActionMinPlayRatio`(0.2) 時，退回「起 0、播到最後一幀」＝這支功能加進來以前的行為（所以最壞情況只是「沒幫上忙」，不會更糟），並印一則 log 說明是哪個血統、振幅多少。**兩條一起判是關鍵**：只看振幅會誤傷 Base（振幅 17% 但目前播 9/25、表演正常，它只是第 1 幀就已經出手）；只看幀數比會誤傷山嶽巨人（播 4/25，但振幅 52%＝曲線有結構，那 4 幀就是它真正的動作長度）。離線全量驗算：命中的正好是石像鬼(2→25)、狼人(2→25)、覓血者(2→25)、泰坦(4→23) 四組，**其餘九組一格不動**。
- **試過、不採用的兩條**：① 曲線改成比對 attack 自己第 1 幀（[BLOODLINE.md](BLOODLINE.md) §8 提過的方向）→ Base 從 9 幀掉到 3、旱魃 9 掉到 2，反而更糟（那兩組的第 1 幀就已經是出手，拿它當基準等於把整個動作歸零）。② 結束幀改成「**最後一次**到 90%」→ 壞掉的四組確實都修好，但毛殭 13→20、旱魃 9→21、Base 9→17，等於推翻 G6 刻意設計的「只播第一拳、不播 AutoSprite 多出來的第二拳」。
- **通則**：**每一條「從資料自動算」的規則都有一個沒寫出來的資料形狀假設，要為它加一道「這個假設在這組資料上成立嗎」的偵測，而不是把門檻調來調去。** 調門檻是在同一個假設裡搬椅子——不管把 `ActionEndPeakRatio` 調到多少，平線曲線都給不出正確答案。另一條：**判斷「演算法失效」要用兩個獨立指標交叉**，單一指標必然誤傷邊界上那幾筆（Base 與山嶽巨人剛好各是一個指標的反例）。第三條：同 G6——**又一次是「所有角色都有這個問題，只是其他角色的資料剛好蓋住了」**，覓血者壞了不知道多久，是這次全量掃描才掉出來的。

### G10. 角色從來不 idle、永遠在走路（資料夾名多打一個字母）
- **症狀**（2026-09-15，狂族士兵）：這隻 NPC 在**編輯器預覽與遊戲裡都是 walk 動作**，站著不動也在走；同一批做的血族士兵正常。編輯器的 NPC 面板沒有「初始動作」可設，所以看起來像程式選錯狀態。
- **原因**：`WerewolfSoldier/idle/` 是**空資料夾**，17 張 idle 圖全在隔壁的 **`idlle/`**（多打一個 `l`）。**遊戲端與編輯器都有「idle 取不到就退回 walk」的 fallback**（`MonsterController` 算 tileSize 時 `idle→walk`、`PreviewSpriteLoader` 的 `res.idle = res.walk`），所以**不會報任何錯**，只是一路退到 walk 當待機。
...
### G12. 四足獸「一跑起來就大一圈」——逐動作**高度**正規化對牠們適得其反
- **症狀**（2026-09-17，戰狼）：idle 站著正常，一進 walk 整隻狼明顯變大變長；回 idle 又縮回去。
  以為是 AI 產的素材大小不一，量了全 65 幀才發現**幀間其實很穩**（idle 高度極差 1%）。
- **原因**：`MonsterAnimator.Setup` 從以前就有「逐動作高度正規化」——把 walk/attack 的 `tileSize` 乘上
  `idle可見高 ÷ 該動作可見高`，讓各動作看起來一樣高。對**人型**很有效（站立/走路/出拳都是直立姿勢），
  但**四足獸會反過來**：
  - idle 是 3/4 正面站姿 → 高 143px
  - walk 是側面奔跑、**身體壓低伸長** → 高只有 111px
  ⇒ 正規化倍率 143/111 ＝ **×1.288**，把本來就比較長的奔跑姿勢**再放大 29%**。
  等效寬度 221→**285px**，而 idle 才 181 ⇒ **差 57%**，就是肉眼看到的「一跑起來大一圈」。
  這與 **G7**（主角出拳長大一圈）是同一個病灶的兩面：**高度只是「大小」的一個投影，姿勢一換就跟「這東西有多大」脫鉤**。
- **解法**：`MonsterData.csv` 表尾加三欄 **`IdleScale` / `WalkScale` / `AttackScale`**（`pant` 沿用 `IdleScale`）——
  **留空＝維持原本的自動高度對齊（既有的怪一個像素都不變）**，有填就完全覆寫那個自動值。
  戰狼填 `WalkScale=0.9`：等效寬差距 **57% → 10%**。
- **為什麼是手填而不是換個自動指標**：G7 用的「√(高 × √面積)」對四足獸算出來是 **walk 要放大 20%**，
  跟肉眼相反——因為 walk 雖然長，但矮了 22%、實際畫到的像素還少 10%。
  **這兩個姿勢的長寬比根本不同**（181×143 接近方形 vs 221×111 扁長），**不存在能讓它們「看起來一樣大」的單一縮放**，
  只有取捨。演算法猜不準，作者的眼睛一眼就知道，所以開成手填。
- **通則**：**「視角/姿勢換了」的素材不能用任何單一尺度自動對齊**，只能給人一個旋鈕。
  自動正規化留給「同一視角、只是畫粗一圈」的情況（那才是 G7 要解的）。
  補一句給下一個人：**看到「某個動作特別大」先去看有沒有人在替你正規化**，而不是先怪素材——
  這次的 57% 落差裡，素材只貢獻 22%，另外 35% 是程式自己放大的。
...
- **⚠ 這一則只解掉一半**：實際位移判定擋得住「完全不動的假走路」，擋不住「在原地小幅來回」。2026-09-08 同一個症狀以另一個成因復發，見 **F18**。
...
### F18. 「原地踏步」復發：紅嫁衣被逼到牆邊時照樣播走路——尋徑把「沒路」偽裝成「有一條長度為零的路」
- **症狀**：F15 修好之後，紅嫁衣女殭屍**背後貼著牆／被逼進角落**時仍然一直播走路動畫，人只在原地小幅來回抖，像在踏步。新娘房場景改大之後反而更常看到（她有空間可跑，於是更常跑到貼牆才停）。
- **原因**：三層疊起來的，缺一層都不會發生。
  1. **決策端沒驗證可走性**：`RedBridalGownBrain` 的逃跑目標點是憑空外推的 `自身位置 + 玩家反方向 × AwayLookahead(2)`，**完全沒問過那個點可不可走**。背後是牆時，這個點就落在牆／家具裡。
  2. **尋徑對不可走終點做了吸附**：`MapNavGrid.TryFindPath` 會把終點 `NearestWalkable()` 到最近的可走格。她貼著牆時，「牆內那點的最近可走格」往往**就是她腳下那格** → `start == goal` → 函式回傳 **true**、路徑只有一個航點＝**她自己的格中心**。也就是說，尋徑回報「找到路了」，而那條路的長度是零。
  3. **移動端設計成永不停下**：`MonsterActuator` 於是以滿速 `MoveSpeed` 朝自己的格中心衝，到了之後方向反轉、路徑 0.35s 才重算；再加上解卡機制每 0.3s 觸發一次 0.4s 的 ±75° 側滑並左右換邊、以及 `SteerAround` 註明的「一整圈都被擋仍朝目標推（不凍住）」——**velocity 永遠是滿的**。這些微幅來回的實際位移剛好高過 `MoveAnimThreshold`(0.12)，所以 **F15 的「看實際位移」判定也擋不住**（它只擋得住完全靜止）。
- **解法**：兩層一起改。
  - **① 決策端「跑之前先確認真的有路」**（`RedBridalGownBrain.TryPickFleeTarget`）：以「玩家反方向」為 0°，依 `FleeScanAngles`（0, ±25, ±50, ±75, ±100, ±125, ±150，**偏離小的優先**）掃描，每個候選要同時滿足 ① 落點 `IsWalkableWorld` ② `HasLineOfSight(自身, 落點)`（和 A* 同一份障礙圖）③ 不會反而更靠近玩家；取第一個可行的當目標，**全都不可行就 `act.Stop()`** → velocity 歸零 → 位移為 0 → 動畫自然回 idle。另加**方向遲滯**（上一幀的角度仍可行就沿用）避免在兩個對稱方向間每幀跳動。⚠ **起點自己嵌在不可走格（貼牆/卡家具）或沒有 nav 時要退回舊行為直接逃**——否則起點不可走會讓所有視線檢查一律 false、掃描永遠找不到路，變成**永久 idle 卡死**。
  - **② 移動端通用「放棄期」**（`MonsterActuator`）：連 `MaxUnstickTries`(2) 次側滑都沒有效位移、**且 A* 確實無解**（`_lastNoPath`）→ 進入 `GiveUpSeconds`(0.6) 的靜止期，velocity 歸零、期滿自動重試。**「且 A* 無解」這個條件很重要**：被玩家或其他怪短暫擋住時路徑是通的，不加這個條件會讓互相推擠的怪動不動就站著發呆。
- **通則**：
  1. **尋徑 API 對「終點不可走」做最近格吸附是善意的預設，但它會把「沒路可走」偽裝成「有一條長度為零的路」**——凡是**憑空算出來**的目標點（逃跑、閃避、亂走、退後），呼叫尋徑之前要自己先驗證可走性，不能只看回傳的 true。
  2. **「看實際位移」只能擋掉完全靜止的假走路，擋不掉原地小幅來回**。假走路要治本得在**決策端**就不要下「跑不動的移動指令」，動畫端的門檻只是保險。
  3. 「絕不凍住」的避障設計（一整圈被擋仍硬推）在**追擊**怪身上是對的（免得站著發呆），但對**逃跑**行為會直接變成原地踏步——兩種行為需要不同的失敗處理。（2026-09-08 記，見 [BOSS_MODULE.md](BOSS_MODULE.md) §2）

### F19. Boss 被打的時候「完全不做決策」——擊退窗口會整段跳過 `Think()`（隱形的行為預算）
- **症狀**：靠近紅嫁衣並持續射擊時，她站著不動、不逃也不閃，**停火之後過一下才突然開始跑**。修掉 F18 的假走路之後才變得明顯。
- **原因**：`MonsterController.Update` 是這樣寫的——`if (_hitReaction == null || !_hitReaction.IsKnockedBack) { … _brain.Think(in ctx); }`，也就是**擊退期間整段跳過 Think()**：不判斷距離、不決定逃不逃、不下移動指令。單次窗口只有 `HitReactionHandler.KNOCKBACK_DURATION = 0.1s`，但**紅嫁衣 `InvincibleTimeMs = 0`（沒有無敵幀）**，每一發命中都會走一次受擊流程、累積傷害跨過 `KnockbackThreshold`(10) 就再開一次擊退 → 連射時窗口首尾相連 → 她大半時間根本沒在做決策。
  **為什麼以前看不出來**：F18 之前，這段「不做決策」的時間裡她的 `velocity` 還殘留著上一次的移動指令、加上擊退本身的來回位移，動畫判定（看實際位移）會認定「她在動」→ 照樣播走路。**F15／F18 那個「原地踏步」有一大部分其實就是這個**。假走路一修掉，同一段停頓就露出本相＝站著不動。**症狀換了樣子，根因一直都在。**
- **解法**：**不要動擊退**——那是共用的受擊反饋，所有怪都吃這條，改了要全面重測。要「跑跑停停」的手感就**自己用明確參數做**：`RedBridalGownBrain` 的 `FleeBurstSeconds`(1.5)／`FleeRestSeconds`(0.8)，跑滿一段就站著喘一段，跟有沒有被打無關。
- **通則**：
  1. **受擊反饋（擊退／硬直）會順便凍結 AI 決策，等於一份「隱形的行為預算」**：怪的 `InvincibleTimeMs`、`KnockbackThreshold` 一改，牠實際能行動的時間就跟著變，而症狀會出現在「牠不做該做的事」這種和受擊八竿子打不著的地方。看到「boss 被打時發呆」先查這條。
  2. **⭐ 好手感如果是 bug 或副作用長出來的，要馬上把它變成參數。** 這次的跑跑停停剛好順眼，但它的長度＝玩家射了多久：換高射速武器她會變沙包、換單發高傷武器她又幾乎不停；更麻煩的是 [BOSS_MODULE.md](BOSS_MODULE.md) §5 有一條待辦要給互毆的怪加 `InvincibleTimeMs`(300~500)（為了修 **F5**），那條一做下去這個節奏就會**無預警消失**，事後幾乎不可能聯想到是它害的。（2026-09-08 記）

### F20. 把 CSV 的 `Speed` 調小，怪卻變成「幾乎不動、偶爾抽動一下」——行為節奏綁了時間就會跟著 Speed 縮水
- **症狀**：把 `MonsterData.csv` 紅嫁衣的 `Speed` 從 3.5 調到 0.5，預期是「一樣的移動模式，只是走得慢」，實際變成**幾乎不移動、偶爾動一下、沒走兩步就停**，和 3.5 時的行為差很多。
- **原因**：boss 的逃跑節奏原本用「**秒**」定義（`FleeBurstSeconds = 1.5` 跑、`FleeRestSeconds = 0.8` 喘）。每一段實際跑的**距離** ＝ `Speed × 秒數`，於是跟著 `Speed` 等比縮水，而喘息的 0.8 秒**不變**：

  | Speed | 每段跑的距離（tileSize=1） | 然後停 |
  |---|---|---|
  | 3.5 | 5.25 格 | 0.8s |
  | 0.5 | **0.75 格**（不到一個身位） | 0.8s |

  時間上的動靜比其實沒變（65:35），變的是每次動作走多遠——挪不到一格就停，看起來就是「偶爾抽動一下」。
- **解法**：**行為節奏改用距離結算**——`FleeBurstDistance`(5 世界單位，≈ Speed 3.5 下跑 1.5 秒的距離) ＋ `FleeBurstMaxSeconds`(6) 當保險上限（速度極慢或一直被擋時仍會給玩家一個攻擊窗口）。累積量**量實際位移**（撞牆原地磨不算進額度），且單幀增量夾在 `MoveSpeed × deltaTime × 1.5` 之內——**擊退／傳送造成的位移不是她自己跑的，不能算數**。改完之後調 `Speed` 只改變她跑多快，不改變行為模式。
- **⚠ 同一類、目前刻意沒修的三個絕對值**（作者已把 Speed 調回 3.5，全場只有紅嫁衣是逃跑型，不值得為此動共用程式；哪天要做慢速怪再回來看這裡）：
  1. `MonsterController.MoveAnimThreshold = 0.12`（判定「有沒有在走」的門檻）是**絕對值**：Speed 3.5 時只佔 3.4%，Speed 0.5 時佔 **24%** → 慢速怪稍有折損（貼牆滑、繞路）就掉到門檻下播 idle，明明在挪卻看起來站著不動。
  2. `MonsterActuator.UnstickSeconds = 0.4`（側滑解卡時長）也是絕對值：Speed 0.5 時側滑只挪 0.2 格，往往不足以脫困 → 更容易踩進放棄期，再多停 0.6 秒。
  3. `RedBridalGownBrain.SafeRange = 6.5` 是絕對距離：Speed 0.5 時她根本拉不開（玩家速度 5），`_fleeing` 會永遠是 true、不再回到「安全了、站著」的狀態。
- **通則**：
  1. **⭐ 每一個「行為節奏」參數都要先問它該綁時間還是綁距離。** 移動類的行為（跑一段、巡邏、後退、衝刺、繞圈）綁**距離**才會隨 `Speed` 正確縮放；綁時間的話，CSV 的 `Speed` 就同時控制了兩件事（跑多快＋每次跑多遠），調一個必壞另一個。非移動類的（冷卻、施法前搖、無敵時間）才綁時間。
  2. **「資料化的速度」和「寫死的絕對值門檻」擺在同一個系統裡，就是一組定時炸彈**：CSV 改一個數字，門檻的相對意義就整個變了，而症狀會出現在完全不相干的地方（動畫在播 idle、怪站著不動），很難聯想回那次 CSV 改動。新增這類門檻時順手想一下「這個值在 Speed 0.5 和 Speed 10 各佔幾 %」。（2026-09-08 記，見 [BOSS_MODULE.md](BOSS_MODULE.md) §2）


### F21. 綁「玩家攻擊」的功能，對持續型武器（雷射／佛光）只會觸發**一次**
- **症狀**：新做的血統攻擊特效，用離散武器連射時每隔幾秒播一次（正確），但**換成雷射／佛光按著左鍵不放，只播了第一次，之後再也沒播**。看起來像冷卻欄位（`AttackFxGap`）沒有作用。
- **原因**：掛的節點是 `PlayerController.TrySpawnFireEffect`——它確實是四條發射路徑（離散／雷射／召喚／佛光）的共同節點，**但兩種武器經過它的次數完全不同**：離散武器是**每發**都經過；持續型武器**只有按下的那一幀**經過（程式裡本來就寫明「佛光在按下瞬間播一次、持續存在期間不每幀重播」「雷射在按下瞬間播一次砲口特效」）。所以按著不放的整段期間一次都不會再通知，冷卻到期了也沒有人來敲門。**冷卻沒壞，是事件source只響了一次。**
- **解法**：兩種情況分開接。離散維持走 `TrySpawnFireEffect`；持續型改成**每幀問狀態**——`PlayerController` 新增 `IsContinuousFireActive`（`_activeBeams.Count > 0 || _activeAura != null`，與攻擊姿勢那段共用同一個判斷、不另立真相），`BloodlineAttackFx.Update` 每幀問一次，兩條路都走同一個 `NotifyAttack()`，冷卻自然把多餘的擋掉。
- **通則**：**「玩家正在攻擊」在這個專案裡不是一個事件，是兩種東西**——離散武器是**一連串瞬間事件**，持續型武器是**一段持續狀態**。任何要綁攻擊的功能（特效、耗魔、集氣、成就計數…）都要問自己「我要的是每一發，還是整段期間」，只接其中一邊必然漏掉另一半，而且**症狀會偽裝成別的地方壞掉**（這次看起來像冷卻欄位失效）。（2026-09-13 記）



### F22. AI「衝過去咬」的動作播了，血一點都沒掉——貼身判定用了**中心距離**
- **症狀**（2026-09-17，撲擊型的戰狼）：狼衝過來、咬的動畫也播了，**但玩家完全不掉血**。
  作者形容「我根本沒移動，牠還是咬不到我」。靜態模擬量出來更難看：**咬中率 0%**（站著不動也一樣）。
- **原因**：Brain 的到達判定寫成「**中心距離** ≤ 1.3 就算貼上、停下來咬」，而傷害結算
  （`EnemyContactDamage`）用的是**碰撞框邊緣距離**（`Physics2D.Distance`，要求重疊或 ≤0.02）。兩把不同的尺：
  - 戰狼身體框半寬 ≈ **0.45**（可見寬 0.70 ＋ `HitboxPadding` 0.2）
  - 玩家 `CircleCollider2D` 半徑 **0.5**
  - ⇒ 中心距離 1.3 時兩個框還差 **0.35** 沒碰到 ⇒ 決策說「到了」、傷害說「沒碰到」。
  **而且這個誤差跟體型綁死**：換一隻大一點或小一點的怪，寫死的 1.3 就又錯一次，錯的方向還不一定。
- **解法**：**AI 的貼身判定與傷害結算用同一個幾何來源**——`Physics2D.Distance(自己的 collider, 目標的 collider)`，
  門檻取與接觸傷害同一條線（`≤0.02`／重疊）。決策用的「算不算貼身」可以放寬一點（0.25）避免邊緣抖動，
  但「衝刺到了沒」一定要用嚴格那條，否則就是這個 bug。
- **通則**：**判定「碰到了沒」的地方超過一處時，它們必須讀同一份幾何**。中心距離只能拿來判遠近
  （要不要開始追、要不要放棄），凡是會導致「出手／結算」的判定一律走 collider。
  同家族：**B7**（貼身判定不要用 `OverlapCircle`，專案全域 `queriesStartInColliders=false` 會漏抓重疊目標）。
- **怎麼提早發現**：這種 bug 在畫面上看起來像「AI 很笨」而不是「判定寫錯」，很容易一路往行為邏輯查下去。
  最快的辨識特徵是「**動作有播、數字沒動**」——那幾乎一定是兩套判定用了不同的尺。


### F23. 「我一攻擊牠，牠就不還手了」——擊退把怪推出牠自己的行為範圍，狀態機卡在原地打轉
- **症狀**（2026-09-17，撲擊型的戰狼）：玩家不開火時狼會正常觀望→撲擊；**一開始射牠，牠就只剩左晃右晃、幾乎不撲**。
- **原因**：**三層疊起來**，每一層單獨看都很合理。
  ① 擊退把怪往外推：距離 ＝ `sprite.bounds.size.x`（**整張圖的寬、含透明邊**）×`KnockbackPercent/100`。
     戰狼 256px / PPU 256 ⇒ 1.0 世界單位，`KnockbackPercent=50` ⇒ **每次推 0.5 單位**。
  ② `InvincibleTimeMs=0` ⇒ 每一發命中都走一次受擊流程，累積傷害跨過 `KnockbackThreshold` 就再推一次
     （見 **F19**：擊退窗口期間 `Think()` 整段不跑）。
  ③ **Brain 的「站定觀望」是絕對靜止的**。於是一段 0.6~1.2 秒的站定會被推 2~3 次＝1~1.5 單位，
     **必定**超出觀望圈的外緣 ⇒ 判定「目標走遠了」回去追 ⇒ 走回來 ⇒ 重新站定 ⇒ 又被推出去……
     而且每次回到觀望圈都重抽「要試探幾輪才撲」，**進度永遠歸零**。
- **解法**：兩處。① 回到觀望圈時**不重抽輪數**（只有「撲完一輪」與「第一次接觸目標」才重抽）。
  ② **「站定」改成「保持距離」**——被推出觀望圈就自己走回來（不中斷觀望、不消耗輪數）；
  正常沒被打時距離遠小於觀望半徑，還是站著不動，節奏感不受影響。
  靜態模擬（玩家站著不動持續開火，60 秒）：擊退每 0.35 秒一次時，撲擊次數 **0.1 → 10.4**。
- **通則**：
  1. **任何「站在原地等」的 AI 狀態，都要想一下「被擊退推出去之後會怎樣」**——受擊反饋是共用的，
     它不知道你的 Brain 有距離門檻。狀態機的距離判定要能分辨「目標走遠了」與「我自己被推走了」，
     最省事的做法就是讓那個狀態**主動維持距離**而不是絕對靜止。
  2. **「進度」類的計數（還要試探幾輪、蓄力到第幾階）不要在狀態切換時無條件重抽**，
     否則任何能打斷狀態的外力（擊退、硬直、換目標）都變成「無限重置」，玩家會覺得這隻怪永遠不出手。
...
### G13. 畫在怪腳下的特效跑到**胸口**——route B 怪物的 pivot 是**畫布中心**，不是腳底
- **症狀**:跳躍踐踏的地面龜裂（以及任何「畫在地上」的特效）不是出現在怪的腳下,而是浮在**半身高**的位置,看起來像貼在胸口。角色越大越明顯。
- **原因**:**route B 怪物的 sprite pivot 是 0.5（畫布中心），與玩家相反**（玩家的 pivot 在腳底,見 [BLOODLINE.md](BLOODLINE.md) §2）。`MonsterSpriteLibrary.GetFrames` 的「腳底對齊」只是把**各動作之間**拉到同一條腳底線,**基準幀（idle）的 pivot 刻意維持 0.5**——那是為了讓角色的絕對位置與碰撞框完全不動。<br>所以 `transform.position` ＝ 畫布中心,而畫布中心離腳底有**半個可見身高**那麼遠。狂族皇家衛士實測:idle 的可見像素底邊在畫布下緣往上 28px、pivot 在 128px ⇒ 差 100px;換算 PPU 102、再乘 `Scale` 1.3 ＝ **腳底在 transform 下方 1.27 世界單位**（可見高才 2.54）。
- **解法**:用 `MonsterController.FeetWorldPos`（腳底）/ `BodyCenterWorldPos`（可見身體中心）/ `VisibleBodyHeight`,**不要用 `transform.position`**。三者由 `FitVisibleBoxCollider` 量可見框時一併算好（`_feetLocalY` ＝ 可見框底相對 transform,再乘 `localScale`）,走 `AutoAdjustCollider` 後備時退回碰撞框底邊。
- **通則**:**這是玩家版 E14 的怪物版,而且更隱蔽**——玩家那邊 pivot 就在腳底,所以「用 transform 當腳底」在玩家身上是對的,搬到怪身上就整個偏掉半個身高。**寫任何「掛在角色身上/腳下」的東西之前,先確認那個角色的 pivot 在哪裡**;兩邊的答案不一樣。
- **附帶**:`BlobShadow` 沒有這個問題——它走的是影子錨點（`IShadowAnchorSource`）,本來就把 pivot 換算進去了。所以「影子是對的、特效是歪的」這種組合完全可能出現,**不要因為影子看起來對就以為 transform 是腳底**。

## H. 流程 / 存讀檔 (Game Flow & Save UI)

> 系統說明見 [TITLE_AND_SAVE_UI.md](TITLE_AND_SAVE_UI.md)、[SAVE_SYSTEM.md](SAVE_SYSTEM.md)。

### G14. `WalkScale` / `AttackScale` 填 1.1 想放大，**結果反而變小**
- **症狀**：覺得某個動作的圖偏小，在 `MonsterData.csv` 把 `AttackScale` 填 **1.1**，照理該放大一成，**實際卻縮了**。作者 2026-09-22 在 ZhaYu_HugeSword（`Scale` 1.2）回報。
- **原因**：舊版的手填是「**覆寫**自動高度對齊」而不是「在它之上再乘」——`tile = baseTile × manualScale`，`StateTile` 那條自動對齊整個被跳過。<br>而**自動對齊的倍率本來就不是 1**：它要把這個動作的可見高拉到和 idle 一致，而攻擊／奔跑姿勢通常**比站姿矮**（身體前傾、壓低、劍往側邊掃），所以倍率多半 **> 1**。實測 ZhaYu_HugeSword 的 attack 自動倍率是 **1.252**、戰狼的 walk 是 **1.336**、皇家衛士的 jump 是 1.138。<br>⇒ **手填只要小於自動倍率就會縮**。填 1.1 對 attack 而言是「從 1.252 降到 1.1」＝ 縮小 12%，而且**沒有任何錯誤訊息**。
- **解法**：`MonsterAnimator.Tile()` 改成 `自動對齊結果 × manualScale`（留空／0 ＝ 只做自動對齊，行為不變）。這樣 **1.0 ＝ 跟留空一樣、1.1 ＝ 比平常大一成**，符合「`Scale` 是基準、逐動作欄位在其上微調」的直覺。`jump` 的基準同步改成 `idleTile`（它刻意不走自動對齊，理由見 [BOSS_MODULE.md](BOSS_MODULE.md) §9.7）。
- ⚠ **改語意時既有的手填值必須一起換算**，否則那些怪的顯示會跟著變：**新值 ＝ 舊值 ÷ 該動作的自動倍率**。<br>‧ `IdleScale` **不必換**（idle 對齊自己，自動倍率恆為 1）——WerewolfSoldier／Wolf Archers 的 1.1 原值保留。<br>‧ `JumpScale` **不必換**（不走自動對齊）——皇家衛士的 1.2 原值保留。<br>‧ 需要換的只有走自動對齊的 `WalkScale`／`AttackScale`：戰狼 `WalkScale` **0.98 → 0.733**（0.98 ÷ 1.336）。換算後逐隻驗算顯示高度，全部維持不變。
- **通則**：**一個「留空＝自動、有填＝手動」的欄位，手填值要嘛乘在自動結果上，要嘛就別叫同一個名字。** 「覆寫」型的語意會讓使用者永遠算不準該填多少——他得先知道演算法算出來是幾倍，而那個數字在 CSV 裡看不到、在遊戲裡也看不到。
- 🔧 **想知道某個動作的自動倍率**：`idle 代表幀的可見高 ÷ 該動作代表幀的可見高`（代表幀＝該動作第一張）。一行 PIL 就量得出來，比在 Unity 裡試數字快得多。

### G15. 骨牢（罩住身體的特效）套上去**歪一邊**——影子的 X 是「兩腳中點」，不是「身體中心」
- **症狀**：骨牢關住怪之後，怪看起來不在籠子正中間，**幾乎每隻都偏**：`ZhaYu_HugeSword` 整隻站在籠子左側（偏約 0.6 世界單位）、`ZhaYu_Gun`／`ZhaYu` 偏一點點；怪在原地攻擊、轉頭時籠子還會跟著左右滑。作者 2026-09-23 附四張圖回報。
- **原因**：兩件事疊在一起。
  1. **問錯了問題**：第二、三版的籠心一律問影子（`BlobShadow.TryGetGroundSpot`）。影子錨點的 X 是「底部 15% 帶裡**兩隻腳**的中點」（`ShadowAnchorMath`），回答的是「腳下那灘影子在哪」；籠子要罩的是**整個身體**。一般角色兩者差不多，但拿武器的怪兩腳之間被**拖在地上的武器**佔住——`ZhaYu_HugeSword` idle 的影子 X 在畫布中心 **+43.5px**，軀幹卻在 **−8.7px**（差 52px ≈ 0.6 單位，與截圖量到的偏移吻合）；`ZhaYu_Gun` idle 影子 +3.2 / 軀幹 −8.4。
  2. **錨點逐動作不同**：影子本來就是每個動作一組錨點（`ZhaYu_Gun` idle +3.2、attack −9.8、walk +6.2），籠子每幀跟著影子走，被關的怪「放行攻擊」一揮刀，籠子就滑一下。影子滑沒人發現，實體籠子滑一眼就穿幫。
- **解法**（2026-09-23）：`MonsterAnimator.TryGetCageAnchorLocal` —— **只用 idle、算一次、快取**：X ＝ idle 各幀「可見框上方 60% 的不透明像素欄質心」的中位數（軀幹；下方的腿、拖地武器、衣襬都排除），Y ＝ idle 的影子錨點 Y（地面線不變）。`MonsterCage.BuildCageSpot` 把它套上當下的位置／體型／翻面／離地高度交給 `BoneCageVisual.Spawn(..., anchorSpot)`；取不到（舊 Animator 怪、貼圖不可讀）才退回問影子。玩家端（`PlayerBind`）不變，仍問影子。Console 的 `[BoneCage]` log 會印「籠心與當下影子差多少」，對位再有疑問先看這行。
- ⚠⚠ **第一次實作又踩了一個坑（同日實測四隻全部往左偏 0.6~1.0 單位，比原本更歪）**：量像素時用了 `Sprite.textureRect`。`Sprite.Create` 預設是 **Tight** 網格，Unity 會裁掉四周透明邊，`textureRect` 回傳的是**裁過的框**（左緣往右縮了 `textureRectOffset.x`），量到的 X 因此少算了「左邊透明邊的寬度」——每隻怪透明邊寬度不同，所以偏移量看起來毫無規律。對照驗證：用「真軀幹 X − 左透明邊寬」預測 ZhaYu −59／Gun −90／Bomb −43 px，實測 log 反推 −50／−93／−42，吻合。**改用 `sp.rect`（整張、未裁）**。<br>🔧 **要量 sprite 像素時一律用 `sprite.rect`**，`textureRect` 只在確定要「裁過的實際取樣區」時才用。
- **通則**：**「腳下的東西」對影子、「罩住身體的東西」對軀幹**——兩者是不同問題，不是同一個真相的兩份。凡是實體、會停留的特效（籠子、護盾、冰封），**錨點要取一次固定下來**，不要每幀跟著逐動作變的錨點走。
- ⚠ 反過來，**影子本身**對 `ZhaYu_HugeSword` 也是偏的（寬 2.65 ＞ 身高 2.34，底部帶把武器算進去了）——那是影子錨點表的事，要修就在 `ShadowAnchorTable.csv` 把那幾列改 manual（見 [SHADOW.md](SHADOW.md)〈手動調整影子〉），跟骨牢已無關。

### G16. 夾住角色的前後兩片特效，**後面那片跑到角色前面**（骨牢中間的骨柱畫在怪身上）
- **症狀**：骨牢是 back／front 兩張圖夾住角色（back 在角色後、front 在前）。實測四隻怪只有 `ZhaYu_Gun` 正確，其他三隻**後面那張中間的骨柱**清清楚楚畫在怪物身上——怪看起來像站在籠子後面。作者 2026-09-23 回報。
- **原因**：前後兩片的 `sortingOrder` 只在 Spawn 時抓一次（角色 −1／＋1），但角色的排序值是 `YSortByFeet` **每幀**依腳底 Y 算的，**每 0.01 世界單位就差 1**（`MapDepthSort.SortScale = 100`）。被關的那一幀怪還在往玩家走（Update 裡移動、LateUpdate 才更新排序）⇒ 牠的最終位置比 Spawn 時讀到的更靠上 ⇒ 排序值變小 ⇒ 比 back 還小，back 翻到前面。**`ZhaYu_Gun` 是射手型、站定才開槍，被關時根本沒在動**，所以只有牠對——純看運氣。
- **解法**（2026-09-23）：`BoneCageVisual.SyncSorting` **每幀**把兩片對齊角色當下的排序值（back＝角色−1、front＝角色＋1），並加 `[DefaultExecutionOrder(1000)]` 排在 `YSortByFeet` 之後，讀到的一定是這一幀的值。
- **通則**：**任何要跟 Y-sort 角色「差一階」的東西（夾層、貼身特效），排序都要每幀跟著角色走**，不能 Spawn 時抓一次。差 1 的相對關係，比 0.01 單位的位移還脆弱。
...
### E21. 給黑色地上物補光，燈開再大它還是黑的（亮的全是旁邊的地板）
- **症狀**：想讓暗場景裡的近黑書櫃「看得清結構」，在它上面擺一盞獨立光源。半徑/亮度越開越大，書櫃本身幾乎沒變亮，反而整圈地板、甚至半個房間被抬亮，黑重感全失。
- **原因**：這套照明是**乘法還原、不是加光**——shader 是 `col.rgb *= lerp(壓暗值, 1, v)`，把「被氛圍壓暗的畫面」還原回素材原亮度。素材原本就近黑（原亮度 ≈ 0.05），還原到滿也還是 0.05 × 1 ＝近黑；而地板素材原亮度高，一還原就整片亮起來。所以燈的提亮效果永遠落在「原本就亮的東西」上，跟你想照亮的黑素材無關。與 J4 是同一族問題（乘法效果需要底子本來有亮度）。
- **解法**：兩條，看目標選：
  1. **光暈托剪影**（便宜，多數情況夠用）：接受黑素材就是黑的，把光源調小調弱（書櫃實例：半徑 1.8、亮度 0.3、放櫃體中央偏上），讓提亮圈只在物件邊緣漏出一圈光暈——亮地板托黑剪影，結構靠輪廓讀，黑重感保留。
  2. **素材端提亮**（治本）：把該素材重產一版整體明度提一階的（照 [art_direction/SCENE_PROP_GUIDELINE.md](art_direction/SCENE_PROP_GUIDELINE.md) §11 單件風格替換鐵則：只動明度、尺寸結構全鎖），讓它有「可被還原」的亮度。
- **順帶**：擺補光用「照明」分頁的**獨立光源**（純光點、無碰撞）；擺「帶燈的地上物」（火把 prop）會有擋路碰撞，把走道堵住（2026-08-28 實際踩到：書櫃補光第一次用地上物擺，玩家被卡住）。
...
### J5. 回憶特效播放時，畫面左右兩側出現一塊塊「像馬賽克」的東西
- **症狀**：劇情演出勾了 `memoryFx`，畫面左右邊緣（尤其燈籠、柱子、書桌這種硬邊物件）看起來是一塊塊錯位的方塊／重影，作者形容「像馬賽克」。中央正常。
- **原因**：「柔邊」層的模糊是 **4-tap 十字取樣**（中心 40% ＋ 上下左右各 15%），最大半徑 `BlurPx = 6px`。取樣點只有 4 個、彼此相距 6px，這不是模糊、是**把畫面疊成四個錯位 6px 的半透明副本**——硬邊物件就變成四個重影拼在一起，遠看像格子。中央因為半徑趨近 0 所以看不出來，越往邊緣半徑越大、重影越明顯。**取樣點太少的模糊都會這樣**：相鄰取樣點距離超過 2~3px 眼睛就開始把它們分開看。
- **解法**（2026-08-27）：`MemoryFx.shader` 改成 **13-tap 圓盤取樣**（中心 16% ＋ 內圈 6 點×9%@0.5r ＋ 外圈 6 點×5%@1.0r，兩圈錯開 30°、權重近似高斯），半徑 6px 時相鄰點約 3px。加 `#pragma target 3.0`（13 次 `tex2D` 超過 ps_2_x 的指令上限）。`BlurPx` 等常數不動。若之後把 `BlurPx` 調到 10 以上又看到重影，要一起加 tap 數或改成兩 pass 降採樣模糊，不能只調半徑。
- **後續（同日）**：修完作者決定**整個關掉邊緣模糊**（`MemoryFxController.BlurPx = 0`）——劇情 npc 不會只在畫面中央走，走到邊邊整個人被糊掉，這層本來就跟「演員會走到哪」衝突。shader 的圓盤模糊留著備用。⚠ 暈影 `VigPower`（邊緣壓暗）沒動，邊邊的 npc 仍會暗一點，嫌暗再調。
- **通則**：**模糊的 tap 數要跟半徑成比例**——半徑加大而取樣點不變，得到的是重影不是模糊。做任何「越靠邊越糊」的後處理，先在最大半徑處看一個硬邊物件，重影一眼就露餡。另一條：**全螢幕的「邊緣處理」要先問「演出會不會用到邊緣」**——回憶特效的柔邊是照老照片想的，沒想到舞台是整個畫面。
- **相關**：[CUTSCENE_DIRECTOR.md](CUTSCENE_DIRECTOR.md) §6、本檔 **J4**。
