...
> ⭐ 2026-09-17 把預設值反轉：預設是「敵對」，和平改成劇本要明確進入的特例（⏳ 未編譯未實測）。**
> 驗證前**只需確認現有功能不受影響**——先跑 §6-A 的回歸清單即可；陣營本體之後照 §5 配置、§6-B~E 驗證。

給「兩族原本和平、旗標開戰三方互打、玩家選邊」這類劇本用；規則全收斂在一個檔案，之後更多陣營＝改那一處。

## 0. ⭐ 預設是「敵對」，和平是特例（2026-09-17 反轉，先讀這節）

**把一隻部族怪（Faction=狼人/吸血鬼）放到任何地圖上，什麼都不擺，牠就會照自己的 Brain 攻擊玩家、玩家也打得到。**
要牠站在場上不動手，才需要劇本明確擺一顆「三方陣營和平」。

反轉前是相反的（預設和平、要擺「開戰」才會動），那個設計有一個實際踩到的問題：

> `MonsterData.csv` 的 `Faction` 欄同時承載了兩件事——「**這隻怪是狼人族**」與「**牠的敵意由一顆全域劇本開關決定**」。
> 於是把戰狼放進競技場想測牠的撲擊模組，牠只會站著不動（開關沒被按下），還得為了測試去擺一套劇本 trigger。
> **「放上去就照自己的模組戰鬥」才該是預設，「不戰鬥」才是劇本要求的特例**（2026-09-17 作者拍板）。

反轉後 `Faction` 欄回歸單純的**種族標籤**（跟 HP、Speed 同一層級，決定牠跟誰同族、跟誰是敵人），
「現在打不打」由劇本狀態另外表達。內部旗標從 `WarActive`（預設 false）改成 **`PeaceActive`（預設 false）**，
所有判定同步反轉；**非部族的三個陣營（Enemy／PlayerAlly／Neutral）行為逐條零變化**（已用真值表窮舉驗證）。

| 你要的 | 擺什麼 |
|---|---|
| 部族怪一放上場就打人（測試地圖、一般關卡） | **什麼都不用擺** |
| 站在場上但不動手（劇本的和平段） | `onEnter` → **`factionPeace` 三方陣營和平(鏈動作)** |
| 和平之後開打 | `對話/事件` → **`factionWar` 三方陣營開戰(鏈動作)** ＝結束和平 |
| 玩家選邊 | `joinFaction` 加入陣營(鏈動作) |
| 離開關卡 | 自動重置回預設（敵對） |

⚠ `factionWar` 的 typeId 與顯示名都沒變（對作者而言按下去的效果一樣：兩族開始互咬＋攻擊玩家），
內部改成呼叫 `EndPeace()`。**沒進過和平段的地圖按它也安全**——本來就是敵對，只是重套一次 Layer。
...
| 階段 | 兩族行為 | 玩家武器 | 傷害 |
|---|---|---|---|
| **敵對**（**預設**，`PeaceActive=false`） | 兩族互咬＋攻擊玩家；靠「兩族擺得近、索敵挑最近」讓他們先互打，不寫特例 | 兩族都打得到（Enemy 層） | **兩族互打 ×1/100（演戲）**；對玩家＝正常 |
| **和平**（`factionPeace` 鏈動作，**劇本明確進入**） | 視同中立：不打人、不被打、閑晃演戲（用 NPC 擺） | **打不到**（Ally 層） | — |
| **結束和平**（`factionWar` 鏈動作） | 回到上面那列的「敵對」 | 同「敵對」 | 同「敵對」 |
| **結盟**（`joinFaction` 鏈動作） | 己方不再攻擊玩家；兩族之間照打（仍 1/100，殺敵主力是玩家） | **己方打不到**（切回 Ally 層）、敵族照打 | 玩家對敵族＝正常 |

- 狀態（和平／結盟）＝**關卡單次**：只活在這趟劇本，換 module 由 `TriggerChain.ClearLevelFlags` 自動清（死亡/回廣場＝**重置回預設的敵對**）。
...
| **2026-09-17 的預設值反轉** | 只有**部族**（Werewolf/Vampire）的行為改變。Enemy／PlayerAlly／Neutral 三個陣營的 `Hostile`／`AttacksPlayer`／`HasMonsterFoes`／`ApplyLayer` **窮舉所有組合逐條相同**（已用真值表機械驗證）。目前填了部族的怪只有 17 戰狼一隻 |
...
| `進入和平` | **三方陣營和平(鏈動作)** | 無參數；**2026-09-17 反轉後必須擺**——不擺的話部族怪一進圖就開打。典型：`onEnter 進場觸發 → next 接這顆` |
| `開戰` | **三方陣營開戰(鏈動作)**（＝結束和平） | 無參數；**完成寫旗標=`部族開戰`**（一顆同時完成：切關係＋NPC 退場＋怪物入場） |
...
- [ ] 編輯器開啟正常，Trigger 型別清單多了「三方陣營開戰」「加入陣營」「**三方陣營和平**」三項（不用就不影響任何地圖）。
      ⚠ 新型別要**重開編輯器**才會合併進 `triggerTypes.json`（見 [LEVEL_END_FLOW.md](LEVEL_END_FLOW.md) 的同一條雷）。
- [ ] **（2026-09-17 反轉後新增）** 隨便找一張測試地圖放一隻 `Faction=狼人` 的怪、**什麼 trigger 都不擺**：
      牠應該照自己的 Brain 攻擊玩家、玩家也打得到。這就是反轉要的預設行為。

### B. 和平階段（**要先擺 `factionPeace`**）

- [ ] 沒擺 `factionPeace` 的地圖：部族怪直接開打（＝預設，見 §0）。
- [ ] 擺了 `factionPeace` 之後：兩族 NPC 閑晃，玩家武器打不到（子彈直接穿過）、怪物也不理他們。
- [ ] 首領可交談。
...
- [ ] 完整離開關卡再進：一切重置回**預設的敵對**（關卡單次）——所以和平段的 `factionPeace` 也要重新觸發一次。
...
- [ ] 選邊後死亡回廣場再進 → 回預設（敵對）、未結盟（可重新選）。
- [ ] `factionWar` 在**沒進過和平段**的地圖上按下去 → 不報錯、行為不變（本來就敵對，只是重套一次 Layer）。
