...
- [ ] **第二批能力珠（GemID 9~27、ItemTable 409~427）的 icon 是暫借的**（2026-08-26）：`GemTable.Icon` 先填舊的八個 key，作者說之後外型會重製。重製時只要畫 `gemIcon_<key>.png` 丟進 `gemIcons/`、把 `Icon` 欄改成新 key，零程式改動。
...
- [ ] **只用 `normal` 表情**。想讓不同血統用不同表情（例如旱魃用 `happy`），把 `BloodlineIntroPanel.PortraitEmotion` 改成從表B 讀一個新欄位即可。
...
- [ ] **`BodyScale` 的數值要實機看過再定**：目前殭屍 1／毛殭 1.5／旱魃 1.2 是憑印象給的。素材量出來的可見高其實差不多（Base 193px／殭屍 174／毛殭 197／旱魃 175／覓血者 174／血伯爵 157／該隱 **138**），差別在姿勢與可見寬，所以只能用眼睛調。**血族三階（20/21/22）目前一律填 1，一次都還沒實機看過**；該隱因為可見高最矮，正規化後會被放大約 1.4 倍，最需要調。改 CSV 即時生效，不用改程式。
...
- [ ] **目前三個系列（殭屍、血族、狂族）**，素材與資料都齊。再加系列的步驟見 [BLOODLINE.md](BLOODLINE.md) §7（進階藥劑不用動，程式碼一行不用改）；下一個系列的血統 Id 從 40~42 起。
- [ ] **狂族（2026-08-27 加）待實機**：`BodyScale` 狼人 1／望月者 1.3／芬里爾 1.5 憑印象給的；立繪除 `normal` 外是暫代圖（與旱魃同一張）。
- [ ] **狼人的攻擊動畫只播 2 幀、芬里爾只播 3 幀**：起播／結束幀演算法拿「跟 idle 的差異」當動作曲線，狼人 idle 直立、attack 整段前傾 ⇒ 第 1 幀就是峰值。修法二選一：重做 attack 讓起手接近 idle；或把結束幀改成看「相對 attack 第 1 幀的差異」曲線（`PlayerSpriteLibrary.ComputeActionRange`）。先跑 `Project Tools → 角色 → 攻擊動畫幀數報告` 確認。
...
## 角色影子定位資料化 — 2026-09-02（✅ 2026-09-03 完成程式，待實機）

做角色場景融合 POC 時發現的缺口（過程與踩過的坑見 [PROBLEMS.md](PROBLEMS.md) **E28**）。
2026-09-03 改成「影子錨點表」：每角色每動作一組、工具自動算＋手改覆寫（見 [SHADOW.md](SHADOW.md)〈定位：影子錨點表〉）。剩下的：

- [ ] **實機驗證**（Cowork 開不了 Unity）：Project Tools → 角色 → 計算影子錨點 → 看 `TempImage/ShadowAnchors/` 拼圖 → 進遊戲看主角 idle/walk/attack 影子都在腳下、轉身正確、起步/停步影子平滑不跳；怪物與 NPC 同樣。
- [ ] **要作者做一次**：GameManagers 掛 `ShadowAnchorTableProvider`、把 `Assets/Data/ShadowAnchorTable.csv` 拖進去（沒掛：編輯器直接讀檔並印提醒；build 退回自動計算＝沒有手改覆寫）。
- [ ] 看拼圖手改反例（毛殭、旱魃那類披風長袍）。`dead` 已另走「剪影中心＋最後 1/3 幀」；倒下過程那 1~2 秒影子會先跳到躺平位置（只有平滑帶過），嫌的話再做逐幀。
- [ ] 影子寬目前＝各幀可見框寬中位數，attack 揮手時會偏寬；嫌的話改表的 `WidthPx`。

---

## 角色場景融合 POC — 2026-09-02（進行中，未結案）

`Scripts/Diagnostics/CharacterEnvPoc.cs` ＋ `SpriteFlash.shader` 的色彩參數。
遊戲中 **P → G** 切 0 原狀 / 1 色彩（2026-09-03 起只剩兩態，Test C 邊緣融合已砍）。
已驗證有效的部分與數據見 PROGRESS 同日條目；以下是還沒做完的：

- [ ] **對齊 Atmosphere 17 的參數還沒看過實機**：`BlackLift` 從 0.0157 翻到 0.0306（因為 17 的場景亮度是 16 的 1.5 倍）。
      **唯一的風險是褲子發灰**——太灰就把 `TargetDarkRatio` 從 14 往上調（20 約等於回到前一版的力道）。
- [ ] **只在血狂之爭大廳這一張圖驗證過**，其他場景沒試；換場景要照 [PROBLEMS.md](PROBLEMS.md) **E26** 重量 `SceneLuma` 與暗部色。
- [ ] **怪物身上沒看過**：色彩處理的套用範圍是玩家＋怪物（所有掛 `HitReactionHandler` 的角色），但驗證時畫面裡都沒有怪。
- [x] ~~Test C（邊緣融合）去留未定~~ → 2026-09-03 砍除（兩輪數據都在雜訊級）。
- [ ] **模組化成正式的 Character Environment System**：確認有效後才做——每個場景一組 Profile（`SceneLuma`＋暗部色＋沿用該 mode 的 baseTint/litTint），
      取代目前寫死在 `CharacterEnvPoc` 裡的一組常數。
- [ ] ⚠ **兩個已被證偽的假設，不要再走一次**：「角色沒參與 bloom」不成立（白外套本來就有 15.9% 的像素過門檻）；
      「邊緣對比是貼圖感主因」也不成立（Test C 幾乎沒有可量測的貢獻）。**真正有效的是黑階抬升**——以及 2026-09-03 找到的解析度不一致（E29）。

> 拔除方式：刪 `CharacterEnvPoc.cs`，再把 `HitReactionHandler.cs`、`PerfHud.cs`、`SpriteFlash.shader`
> 裡標了【POC】的幾段拿掉即可。shader 參數預設全 0，Mode 0 逐位元等於加這功能之前。
...
- [ ] **背景換高解析度**（作者）：各地圖建議尺寸見 [PERF_QUALITY_AUDIT.md](PERF_QUALITY_AUDIT.md) §4.1。**`Main_Square` 已結案（2026-09-07）**——畫布改 48×36＋底圖 5792×4344 ⇒ 121 px/格（見下方〈邪佛廣場改尺寸〉節）。**還沒換的**：18×10 那批（紅嫁衣各房／初始洞窟／初始森林／競技場，皆 80 px/格）與 BloodFang_MainLobby(45)、BrewingRoom(95)。
...
## 邪佛廣場改尺寸 — 2026-09-07（轉檔完成，實機驗證中）

畫布 90×50 → 48×36、底圖 1448×1086 → 5792×4344（121 px/格）。做法與踩坑見 [PROGRESS.md](PROGRESS.md) 檔頭那條與 [PROBLEMS.md](PROBLEMS.md) **E30**；轉檔腳本 `resize_main_square.py`（專案根目錄），原檔備份 `DipanProj_MapEditor/Maps/Main/Main_Square.dipanmap.bak_90x50_20260907`。

- [ ] **實機驗收清單**：洞穴進廣場落點（`caveExit`）／關卡回來落點（`center`）／三座祭壇各開對面板（要完成 1 關才出現）／傳送門互動與光效位置／邪佛全貌 camZone 的取景／12 盞火把光暈貼不貼著火把／NPC 巡邏路徑。
- [ ] **可走層 `walkSubdiv` 仍是 4**（子格 360×200 → 192×144，x 方向掉 47%）。提到 **8** 就是 384×288、兩軸都比舊圖細、等於零損失重轉一次；兩端都是資料驅動（`MapData.Subdiv`／`MapModel.Subdiv`），**沒有任何地方寫死 4**，只有新建地圖的預設值是 4 —— 但**編輯器 UI 沒有這一欄，要手改 JSON**。牆碰撞子格會 72,000 → 110,592，B6 的 run-length 合併還在（實測 65,856 格 → 324 條 box），量級無感。**撞到「看起來能走卻卡住」再做。**
- [ ] **`cameraFocus` 的錨點只能手填數字**：`focusX`/`focusY` 已上線（見 [TRIGGER_CHAIN.md](TRIGGER_CHAIN.md)），但沒有傳送點那種「在畫布上拖曳對位」的模式（`TeleportMarkerPreview` + 對位模式鈕）。目前要對準得先去看目標地上物的 X/Y 再抄過來。之後若 cameraFocus 用得多，值得比照做一個。
- [ ] **`CharacterMipBias` 還不能關**：廣場這張的 bias 已降到 +0.04（等於不壓角色細節），但全域仍要留給還沒換圖的地圖（見上一節）。
- [ ] **背景 121 px/格 略低於 §4.1 訂的 128**（要達標得 6144×4608）。目前是 Upscayl 4x 放大 1448 原圖，**真實資訊量只有 30 px/格**——放大不會生出細節，買到的是銳利邊緣＋縮小取樣。要真正達標得重產原圖，另案。

---

## 可走層自動生成／試走 — 2026-09-07（程式完成，未編譯驗證）

做法與實測數字見 [PROGRESS.md](PROGRESS.md) 檔頭那條與 [MapEditor_DESIGN.md](MapEditor_DESIGN.md) §4.4。演算法細節在 `Core/WalkableAutoGen.cs` 檔頭。

- [ ] **在 Unity 開編輯器專案編譯＋實測一輪**（Cowork 這邊跑不了 Unity）。先拿邪佛廣場試：選種子 → 容差拉到 35 附近 → 看預覽形狀對不對 → 套用 → 按「試走」走一圈。
- [ ] **容差預設值 35 只在邪佛廣場校過**。其他圖的最佳值不同（實測落在 20~40），用幾張之後如果發現常要往同一個方向拉，再改 `WalkableAutoGen.DefaultTolerance`。
- [ ] **`ErodeRadius` 目前是寫死的公式** `max(1, min(3, min(FW,FH)/40))`。小地圖（18×10＝72×40 子格）算出來是 1、廣場是 3。如果遇到「窄走廊整條被侵蝕掉、連通塊擴不出去」，就是這個值太大——目前沒開成可調參數，先看實際會不會撞到。
- [ ] **試走沒有做「地上物碰撞」**：主遊戲的家具碰撞是 `MapLoader` 依素材 alpha 烘出來的（`ObjectFootprint`），編輯器沒有那套。所以試走只驗證可走層。要連家具一起驗證的話得把烘焙那段移植過來，工程不小，看實際需求再決定。
- [ ] **試走沒有做地圖切換／傳送點**：走到傳送點不會換圖。單張圖的走位驗證夠用了，之後若要驗證整關動線再說。

---

## 地圖編輯器右側面板改版 — 2026-09-07（作者提出，尚未開工）

作者：「這個視窗我覺得難用很久了，長度不夠，我常常填寫內容後看不到後面的文字，需要用滾輪拉到後面看。」

- [ ] **重新設計右側屬性面板**。已知問題：① 寬度固定 240px（`EditorUI.PaletteW`），欄位一多就得捲；② 參數多的面板（Trigger、劇情、NPC）內容長，填完常看不到後面；③ 面板全部共用 `PanelRect`＝整條右側，沒有分組或摺疊。
...
- [x] ~~**蟲皇的 `dead/` 只有一張圖**~~ → 2026-09-14 已解決：那張其實是 AutoSprite 的合圖，
  被切割工具的舊防呆誤判成「認不出格線」而拒切（[PROBLEMS.md](PROBLEMS.md) **C14**）；
  工具改用跨線率之後切出 25 張，與另外兩階一致。
  ⚠ **切完之後要在 Unity 跑一次 `Project Tools → Sync Map Assets`**——catalog 目前還記著舊的「單張」，
  沒同步的話變身演出仍然是瞬間倒地、瞬間爬起。
- [ ] **蟲族六個數字都還沒實機看過**：三階的 `BodyScale` 全填 1、`WalkSpeed` 留空（＝維持 Inspector 的 5）、
  五屬性是佔位值。改 CSV 即時生效。
- [ ] **蟲皇的神格特效還沒指派**（表B 的 `AuraVfxId`／`HaloStyle`／`TrailStyle`／`OrbitVfxId`／`AttackVfxId` 全空）。
...
## 新手夢境教學的夢境武器（2026-09-23）

- [ ] **數值全是起手值、沒實機打過**：八把都是耗魔 0、小怪一兩下死的方向。進夢境後開武器工坊（`Project Tools → 武器工坊`）載入 60~68 號邊射邊調，調完存回 CSV。
- [ ] **效能要看一眼**：化神期（每把劍命中都燒地）與旱魃（每顆火球落地都燒地）在怪很多時會疊不少「焚地」地面，進廣場大波次時按 **P** 看 PerfHud。
- [ ] **怪物身上的「燃燒狀態」不存在**：現在的「起火」是命中點放一塊燒 3 秒的地面。要真的讓怪身上著火、跟著走，得另做狀態效果系統（作者的候選清單裡也有「狀態效果」）。
- [ ] **之後新增的三階血統要填 `DreamWeaponId`**，留空會空手進夢（有警告）。目前 8 個三階都填了。
- [ ] **焚獄炎杖（ItemTable 69）、血鳴古鐘（70）、魔狼爪（71）是正式道具但耗魔 0**：現在只在夢裡用所以沒關係；哪天要放進抽選池或掉落，先把 `ManaCost` 調回正常值（或另做一把一般版）。被換掉的龍息（武器 62）、血月雙爪（63）、天狼焚爪（64，連同 VfxTable 46）已沒人用，留著備查。沒用到的素材資料夾 `Resources/Weapon/animation/iceWolfSlash/`（冰狼爪改成魔狼爪後）可以刪。
- [ ] **血鳴古鐘穿牆**（`BlockedByEnvironment 0`，為了「一路炸到螢幕外」）：在窄的房間裡會看到爆炸出現在牆裡／牆外。夢境廣場是開闊場地沒問題，之後若拿到一般關卡用，考慮改回 1。
- [ ] **地獄火柱（VfxTable 48）的 pivot 在圖中央**：火柱是「從地面往上噴」的圖，但特效以中心對齊沿路點，所以下半截會低於路徑。實機若覺得火從半空冒出來，把那 12 張的 Sprite pivot 改成 Bottom（或整體往上偏移半個高度）。

## 新手夢境教學的瀕死保護（2026-09-23）

- [ ] **瀕死跳結尾時血球幾乎是空的**：不死保護只保證不死，沒有回血。結尾演出期間畫面下方的紅球會停在很低的位置；覺得難看的話，在 `CheckNearDeath` 跳結尾那一刻補滿血即可。
- [ ] **門檻 20% 是起手值**（`DreamTutorialFlow.NearDeathRatio`）：實測看看是否太早／太晚。

## 特效庫第二批武器（2026-09-24，武器 36~48）

- [ ] **12 張 icon 全是代用圖**（`Resources/Weapon/single/weapon_*.png`，左上角標「代用」；檔名清單見 [EFFECT_WEAPONS.md](EFFECT_WEAPONS.md)〈第二批〉）：作者蓋同名檔即可。500px 的圖會被匯入防呆設成 **Point**，縮到 icon 大小會有鋸齒——換正式圖時比照十字弓把 Filter Mode 改回 Bilinear（霹靂火彈的 icon 同時是子彈圖，也要改）。
- [ ] **全部未編譯未實測、數值是起手值**：傷害／耗魔／`BulletScale`／判定 `Radius` 都是推算的，進武器工坊載入 36~48 邊射邊調。特別要看：無間輪（穿牆＋無限穿透，窄房間會在牆外看到它）、八卦護身印（沿用火焰環繞彈 `FireInterval 0.3` 的「按住一直重放」節奏，耗魔可能偏兇）、飛蝗石（0.15 秒連射）。
- [ ] **沒放進抽選池**：要讓玩家抽得到，在 `BaseWeaponRoll.csv` 加列或走 `unlockRoll` 解鎖。
- [ ] **物品名稱沒走 LanguageTable**：沿用 ItemTable 現有寫法直接填中文（全表目前都是這樣），之後統一做英文版時一起處理。
