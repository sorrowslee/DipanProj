...
## 角色影子定位資料化 — 2026-09-02

做角色場景融合 POC 時發現的缺口（過程與踩過的坑見 [PROBLEMS.md](PROBLEMS.md) **E28**）：
**`BlobShadow` 靠掃 alpha 猜「腳在哪」，而長袍、披風、背包、爪子這些突出物會把猜測帶偏。**
不同血統的服裝差異大到沒有一種猜法能全中——試過四種（整張外框中心／bounds 換算／rect·pivot·PPU 解析算／
只取最底部一帶的中心），每一種都有反例，毛殭那類長袍角色最明顯。走路時常常剛好正確、idle 卻偏掉，
因為姿勢會改變外框。

- [ ] **改成資料驅動**（專案鐵則：能用資料解決就不要靠程式猜）：血統表與怪物表各加 `ShadowOffsetX` / `ShadowOffsetY`，
      留空 = 目前的自動值；每個角色調一次到位。`BlobShadow` 讀該角色的偏移覆寫自動值即可，改動很小。
- [ ] **調整手段**（擇一）：遊戲內加一個調整模式（方向鍵拉影子、即時看、把數值印出來貼進 CSV，工量最小），
      或做成像武器工坊那樣的 EditorWindow（選角色→拖影子→一鍵存回 CSV）。
- [ ] ⚠ **不要再往「用程式自動偵測腳的位置」這個方向投工**——已經證明會有反例，而且症狀是
      「大部分角色看起來對、少數角色明顯錯」，很難靠抽樣發現。

> 2026-09-02 這一輪的所有影子改動**已全部還原**（`BlobShadow.cs` 回到未動過的狀態），
> 上面是待辦、不是半成品。
...
遊戲中 **P → G**（Shift+G 反向）切 0 原狀 / 1 色彩 / 2 色彩+邊緣。
...
- [ ] **Test C（邊緣融合）去留未定**：兩輪數據都顯示貢獻在雜訊級（p99 差 0.0007），1px 在實際顯示尺寸下不可見。留著只是還沒決定要不要砍。
...
- [ ] ⚠ **兩個已被證偽的假設，不要再走一次**：「角色沒參與 bloom」不成立（白外套本來就有 15.9% 的像素過門檻）；
      「邊緣對比是貼圖感主因」也不成立（Test C 幾乎沒有可量測的貢獻）。**真正有效的是黑階抬升**。
