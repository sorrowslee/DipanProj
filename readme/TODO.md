...
- [ ] **第二批能力珠（GemID 9~26、ItemTable 409~426）的 icon 是暫借的**（2026-08-26）：`GemTable.Icon` 先填舊的八個 key，作者說之後外型會重製。重製時只要畫 `gemIcon_<key>.png` 丟進 `gemIcons/`、把 `Icon` 欄改成新 key，零程式改動。
