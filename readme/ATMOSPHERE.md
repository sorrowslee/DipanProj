...
| `11` | 強風 | 去白霧、幾乎不調色，只留**斜上/斜下交叉的風絲** + 陣風 + 輕微吹拂 | 大風口、強風平原、看得清場景但風勢猛 |
...
> 風絲（10 橫向、11 斜向）用雜湊函式 `windStreak` 打散成不規則短截線；11 用 `rot2` 把風絲轉成斜上/斜下兩層交叉。
...
- **強風斜向風絲**（type 11）：兩層 `windStreak(rot2(i.uv, 角度), …)`，改 `rot2` 的角度 `0.38`/`-0.42`=斜上/斜下傾角、`scale`/`speed`=密度/快慢、整體 `* 0.08`=明顯度；type 11 無白霧、幾乎不調色。
