...
分系：**陰森系（2/3）**用提燈光圈把玩家身邊壓出一小圈可見區；**末日炎熱系（4/5/6）**不壓暗、改暖色調並帶「熱浪扭曲」；**海洋系（7/8/9）**藍綠水色 + 「水下折射晃動」，深海+恐怖再套潛水燈光圈；**山頂風系（10/11）**——10 風雪（陰冷暴風、翻騰白霧 + 橫向風絲）、11 強風（去白霧、只留單一斜向風絲）；**雨系（12/13）**——12 綿綿細雨、13 大雨（近垂直雨絲往下落）；**陰森森林鬼霧（14）**——畫面偏暗、陰綠冷調 + 漂移黑霧，偶爾飄來一陣濃霧。
...
- **type 2~14**：用同一個 shader `Custom/Atmosphere`，靠 `_Mode`（2~14）切換外觀。用提燈光圈的（2/3/9）由控制器每幀餵入玩家螢幕位置與光圈半徑，半徑以**油燈式 Perlin 明滅**呼吸；炎熱（4/5/6）、海洋（7/8/9）、風系（10/11）、雨系（12/13）、鬼霧（14）在 shader 內以 `_Time` 做扭曲/風絲/雨絲/漂移黑霧，不需控制器額外參數。
...
- `Assets/Resources/Shaders/Atmosphere.shader` — `Custom/Atmosphere`，`_Mode` 切 2~14。
- 接線：`Map/MapTable.cs`（解析 `atmosphere` 欄）、`Map/MapManager.cs`（載圖時套用）、`Data/MapsTable.csv`（`Atmosphere` 欄）。

## 怎麼調

- **每張地圖選型別**：改 `MapsTable.csv` 該列的 `Atmosphere`（1~14）。恐怖改 3；末日炎熱 4／5／6；海洋 7／8／9；風雪 10、強風 11；細雨 12、大雨 13；陰森森林鬼霧 14。
...
- **陰森森林鬼霧**（type 14）：底色在 `* float3(0.62,0.78,0.66)`（陰綠）、`* 0.70`（壓暗）、`lerp(0.40,…)`（暈影）。黑霧團＝每 `period`（14s）一團，從畫面外一側 `lerp(±0.6→∓1.6, cross)` 緩慢橫越，`cross = ph/0.72`（後 28% 是空檔）。要更「偶爾」加大 `period`、要更慢加大 `0.72`；霧團大小 `rad`、壓黑程度 `blob*0.85`、橢圓寬扁 `float2(q.x*0.6, q.y*1.3)`；方向/高度/大小由 `hash11(ci...)` 每團隨機。
...
- 設計沿革見 [PROGRESS.md](PROGRESS.md)。陰森系先前用 git 分支比較過 spotlight／full／colorgrade 三種雛形，整併成 type 2（= full-soft）與 type 3（= full）；末日炎熱系同樣分支比較 noon／ember／dust，整併成 type 4／5／6；海洋系（7/8/9）、山頂風系（10 風雪 / 11 強風）、雨系（12 細雨 / 13 大雨）、陰森森林鬼霧（14）直接加在 develop。
