...
| `14` | 陰森森林鬼霧 | 畫面偏暗 + 陰綠冷調 + 加深暈影 + 緩慢漂移的黑霧雲塊（陣霧起伏、偶爾一陣濃） | 鬼森林、墓地、陰宅外、詭譎沼澤 |
...
- **陰森森林鬼霧**（type 14）：底色在 `* float3(0.62,0.78,0.66)`（陰綠）、`* 0.70`（壓暗）、`lerp(0.40,…)`（暈影）；黑霧用三層 sin 疊 `fog` + `smoothstep(0.40,0.95,…)`（濃淡）+ `wave`（陣霧起伏）+ `lerp(…, 黑, fog*0.85)`（壓黑程度）。要更暗改 `0.70`/`fog*0.85`；霧更頻繁改 `wave` 的兩個 sin；漂移速度改 `t*0.02`/`t*0.015`。
