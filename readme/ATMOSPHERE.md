...
- **電視雜訊**（type 15）：雪花混入量 `lerp(…, snow.xxx, 0.18)`、掃描線 `0.90 + 0.10*sin(uv.y*540.0)`（540＝線數）、同步條速度 `t*0.25`、閃爍 `sin(t*30.0)`、灰調 `lerp(…,lum,0.35)`、雪花更新率 `floor(t*24.0)`；水平撕裂在 frag 頂部位移段（`step(0.93,…)`＝撕裂機率、`*0.05`＝幅度）。
