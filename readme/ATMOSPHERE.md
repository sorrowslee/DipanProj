...
| `10` | 山頂狂風 | 高空稀薄冷白光 + 亮霧邊（遠處發白）+ 橫掃冷白風絲 + 陣風時強時弱 + 水平吹拂 | 山頂、雪峰、高地、暴風稜線 |
...
- **狂風風絲/陣風**（type 10）：風絲密度改 `yLane * 120.0`、捲動速度改 `i.uv.x * 8.0 - tw * 6.0`、明顯度改 `* 0.06`、銳利度改 `pow(…, 8.0)`；陣風時強時弱的節奏改 `gust` 的 `sin(tw*0.6)`/`sin(tw*0.23…)`。
