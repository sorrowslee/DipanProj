...
> ⚠️ **已封存（2026-08-21）**：本檔為專案早期的規劃，**大半已完成或被後續設計超越，不代表現況**。
> 真正的「下一步」看 [../TODO.md](../TODO.md)（檔尾＝最新）與 [../CORE_LOOP_DESIGN.md](../CORE_LOOP_DESIGN.md)。
> 封存時未能確認完成度的殘餘點子已萃取到 TODO.md 尾端「來自舊 ROADMAP 的殘餘點子」一節。
> 以下為原文照錄（內部連結以原位置寫成，封存後不再維護）。
