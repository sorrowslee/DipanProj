...
道具格是**一條扁平陣列切成兩段**：前半 = 裝備包、後半 = 消耗品包
（⚠ **目前各 120 格＝各 10 頁，那是 2026-09-14 暫時放大的除錯值**；**常態設定是各 24 格＝兩頁**，還原就是把
`InventorySystem` 那兩個常數的 `* 10` 改回 `* 2`，見下方〈增減容量安全嗎〉）。之所以不做成兩個獨立容器，是因為「鍛造台鎖住哪一格」「存檔的格位」「新手教學要指哪一格」全都用同一個**格子編號**在對話——切段能讓那些地方一行都不用改。

| 概念 | 位置 | 說明 |
|---|---|---|
| 版面 | `InventorySystem.PageCols` / `PageRows` | **一頁的欄 × 列＝ 4×3（2026-09-14 由 5×4 改）。改這兩個數字就換掉整個背包版面**——面板會照它重排格子、重鋪格線，分頁與容量都自動跟上，素材一張都不用動（見〈道具區格線是程式鋪的〉） |
| 一頁幾格 | `InventorySystem.PageSlots` | ＝ `PageCols × PageRows` ＝ 12。`PagesOf(bag)` 由容量除出來 |
| 容量 | `InventorySystem.EquipBagCount` / `ItemBagCount` | **目前各 `PageSlots * 10` ＝ 120（各 10 頁，暫時除錯值；常態是 `* 2` ＝ 24）**。**要加格只改這兩個常數**，介面會自動多分幾頁；想要不是整頁的容量就直接填數字 |
...
- 一頁的格子若超出該包容量（容量不是 `PageSlots` 的整數倍時會發生），那些格子直接 `SetActive(false)`，而不是留一個 index 越界的格子在那裡。
- **縮小容量是安全的**（2026-09-14 由 40 → 24 驗證過）：`RestoreState` 對「格號越界／分錯包／被占」一律改走 `AddStack` 丟進正確那一包的空位，不會掉東西。⚠ 唯一的例外是**舊存檔某一包裝的件數超過新容量**——那時 `AddStack` 會塞不下，超出的就沒了。改容量前先想一下手上的存檔。

### 增減容量安全嗎

| 方向 | 安全性 | 說明 |
|---|---|---|
| **放大** | **完全安全** | 容量**不存進存檔**——`RestoreState` 一律以程式的常數為準，舊存檔的格號照填、填不下的走 `AddStack`，多出來的格子就是空的 |
| **縮小** | 一種情況會掉東西 | 舊存檔裡**某一包的件數 > 新容量**時，越界的走 `AddStack` 也塞不下 ⇒ 超出的沒了。還原前先確認每包不超過新容量，不放心就先把東西丟進倉庫 |

⚠ **這一點與倉庫不同**：倉庫的 `ItemGridData.RestoreFrom` 曾經**用存檔裡的 `cols/rows` 反過來覆寫程式設定**，
症狀是「改小了沒反應、而且資料多出 UI 拿不到的格子」（[PROBLEMS.md](PROBLEMS.md) **D25**）。
**背包沒有那條路，容量的真相只有 `InventorySystem` 那兩個常數一個。**

**UI 完全不必跟著改**：格子只建一頁重複使用，頁數由 `PagesOf(bag)` 算，分頁／箭頭亮暗／頁碼／
新手教學 `FindGridSlotRect` 的自動切頁全部跟著走；面板與存檔裡沒有任何寫死的頁數。

⚠ **之後要做「背包擴充道具」時，那兩個 `const` 會擋路**：const 是編譯期的值，執行期加格得改成
`static int`（或讓每個存檔各自帶一個容量欄位），並在變動時重建 `_grid` 陣列 ＋ `Raise()`。
改的時候記得 `GridCount` 是**存檔格號的值域**，縮小時仍要保留 `AddStack` 那條保命路徑。
...
|---|---|
| 道具格（一頁） | **沒有逐格座標**（2026-09-14 改）：由網格外框內緣 `331,396 ~ 920,875` ＋ `GridPad=6` ＋ `PageCols×PageRows` 算出格單元節距與每格中心，整組在網格區裡置中。4×3 時節距 144.2、格內容 124（原本 5×4 是 95）。詳見下一節 |
...
## 道具區的格線是程式鋪的（2026-09-14）

**背景圖上原本畫死了 5×4 的金屬格線**（連交叉點的鉚釘都畫在裡面），所以早期的格子座標只能硬編成
`GridCx/GridCy` 兩個陣列去對位——想換成 4×3 就得重畫整張背景。現在改成：

| 素材 | 尺寸 | 做什麼 |
|---|---|---|
| `inventoryPanel_GridBlank` | 572×462 | 貼在 `(340,405)`，**蓋掉底圖畫死的 5×4 格線**。內容是從底圖 20 個格子的「格內乾淨區」隨機拼貼出來的，連噪點都是原圖那一批，所以貼上去看不出接縫（不是純色塊——純色在 Linear 色彩空間下會跟底圖對不上，見 [PROBLEMS.md](PROBLEMS.md) E11） |
| `inventoryPanel_CellFrame` | 110×110 | **一個格單元**，從底圖第 2 列第 2 格裁下來（`458,519 ~ 568,629`）。左右上下各含**半條暗縫**，所以照節距平鋪就自動還原原圖的「雙線＋鉚釘」，不必做九宮格 |

兩張圖都 `raycastTarget = false`，由 `BuildGridArt()` 鋪，**一定要在 `BuildGridSlots()` 之前**——
透明命中區要蓋在圖上面才收得到點擊。

**排版算式**（`InventoryPanel.CellPitch` / `CellCenter`）：

```
可用區 = 網格外框內緣 (331,396)~(920,875) 各內縮 GridPad=6  →  577 × 467
節距   = min(577 ÷ 欄數, 467 ÷ 列數)      ← 等比方格，整組置中
格內容 = 節距 × CellInnerRatio(95/110.4 ≈ 0.86)   ← 命中區與 icon 用這個大小
```

⚠ **格子大小是由「列數」決定的，欄數只影響左右留白**——高度 467 才是瓶頸：

| 版面 | 節距 | 格內容 | 對 5×4 |
|---|---|---|---|
| 5×4（舊） | 116.8 | 100 | — |
| **4×3（現行）** | **144.2** | **124** | **+31%** |
| 4×4 / 5×3 | 116.8 | 100 | **一樣大**（白改） |
| 3×3 | 155.7 | 134 | +41%（但左右各空一塊） |

所以「想讓 icon 看清楚」只有一條路：**減列數**。2026-09-14 之前的 5×4 一格只有 95px，
血統藥劑那種細節多的 icon 在裡面就糊成一團，這才是重排的起因。

換底圖的話，上表那四個邊界與 `CellInnerRatio` 都要重量；量法是掃描底圖某一列/欄的亮度峰值找線條位置。
...
- **tooltip**：**共用元件 `UI/ItemTooltip.cs`**（2026-09-14 抽出，背包／倉庫／鍛造同一份——以前三個面板各抄一份，改了背包就忘了另外兩個）。移到物品上跳出浮動說明（**掛在 `UILayer.Popup` 層**——不是面板底下，否則背包與倉庫同開時會被另一個面板蓋住，見 [PROBLEMS.md](PROBLEMS.md) **E34**；不受 frame 縮放、跟著游標、不擋 hover）。**最上面一張大圖預覽** ＋ **名稱（粗體金）**＋ **`TipStats`（正楷）**＋ **`TipLore`（斜體）**;高度由 `VerticalLayoutGroup + ContentSizeFitter` 自動撐開,空欄自動隱藏該段。面板端只要三行：`OnBuild` 建一個、hover 進出 `Show/Hide`、`Update` 呼叫 `Follow()`。
  - **尺寸常數全部放在 `ItemTooltip` 最上面一區**（`Width` 520、`NameSize/StatsSize/LoreSize` 30/26/24、`PadX/PadY/Spacing`、`GapX/GapY`），要再放大縮小改那一區就好，三個面板一起生效。
  - **鍛造那段「這顆珠子對這把武器無效」的說明**不在共用元件裡：由面板算好、透過 `Show(st, extraStats)` 的第二個參數帶進來，接在 `TipStats` 後面。
  - **大圖預覽**：邊長 `PreviewSize = 280`（素材本身 500×500，所以是原圖在縮、不會糊）。走 `ItemIcons.Apply` 這個唯一入口畫，**血統藥劑的三層疊圖、珠子的能力符號自動跟著對**。起因是格子只有 124px，三層疊出來的血統藥劑在格子裡看不出是哪個系列。
  - 版面上它是**一個只負責占高度的容器（`LayoutElement.preferredHeight`）＋ 裡面一張固定 `sizeDelta` 置中的圖**——直接把 Image 當 VerticalLayoutGroup 的子物件會被撐滿寬度、拉成長方形。
  - `PreviewAllItems`（常數）＝ `false` 時**只有血統藥劑**畫預覽，其餘物品維持純文字。
  - ⚠ **`Follow()` 一律把 tooltip 開在游標上方**（2026-09-14 作者指定：位置要統一、不要隨游標高低換方向）。作法是 **pivot 的 y 固定在下緣**（`anchoredPosition.y` ＝ tooltip 底端），再夾制頂端不超出面板上緣——畫面上半 hover 時會整個往下壓，**不翻方向**，因為翻方向正是「tooltip 在游標上下跳」的來源。左右仍照舊：游標在右半邊就往左開。`Show()` 會先 `LayoutRebuilder.ForceRebuildLayoutImmediate` 再定位，否則 `ContentSizeFitter` 還沒重算、第一幀會拿上一件的高度去夾制。
...
- ~~（可選）背包浮動 tooltip 目前各面板各建一份（背包、倉庫各有），日後可抽成共用元件。~~ **2026-09-14 完成**：抽成 `UI/ItemTooltip.cs`，背包／倉庫／鍛造共用。
