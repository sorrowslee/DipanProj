...
>
> ⚠️ **本檔部分內容已過時（2026-08-03 能力珠鑲嵌系統上線後；2026-08-21 標註）**：
> 檔內「孔位永遠全鎖」「還沒有寶石道具」「`ForgeSockets` 固定回 0」「鍛造結果不進存檔」等描述，
> 是鑲嵌系統完成**之前**的狀態。**孔位數（`ItemManager` 骰）、能力珠（`GemTable.csv`）、鑲嵌生效與存檔的現況，
> 一律以 [GEM_SOCKET.md](GEM_SOCKET.md) 為準**；本檔仍是**面板版面／拖放／鐵砧借放機制**的正典。
> 兩顆按鈕（移除鑲嵌／拆除裝備）的現況見 [TODO.md](TODO.md)「能力珠鑲嵌」節。
