...
| `Resources/UI/ForgingPanel/ForgingPanel_ItemFrame.png` | 方框：**沒有鎖鏈**的版本（鐵砧＋已開啟的孔） | 1360×1210 |
...
顯示大小由 `displayHeight`（預設 **636**）決定：1536×1024 的圖 → 954×636，靠左擺在 `PairLeftX = -483`，
右邊剛好留給並排的背包（背包 `PairRightX = 420`、寬約 838），兩者不重疊。
...
> ⚠️ **重新輸出任何一張圖之後**，`LoadArt` 會比對畫布尺寸並印警告（不會靜默跑位）。
...
ForgingPanel_ItemFrame           1360×1210   內容 (155,76,1045,1084)  ← 只記「方形本體」，兩側尖角自然凸出去
ForgingPanel_ItemFrameWithChain  1412×1302   內容 (109,65,1171,1185)  ← 同上，本體對齊靠這組數字
ForgingPanel_Btn                 2416×676    內容 (8,66,2386,565)     ← ⚠ 見下方
CloseBtn_2                       519×481     內容 (17,14,475,459)
```

> ⚠️ **`ForgingPanel_Btn.png` 目前沒有去背**：整張不透明、四周填著深灰 `(40,40,40)`，
> 所以按鈕底板外會露出一圈灰。上面記的邊界框是用**顏色**量的（不是 alpha），
> 位置與大小是對的；等重新輸出成透明 PNG 之後，這組數字仍然適用，不必改程式。
...
兩張框的**側邊尖角大小不同**（無鎖鏈那張的尖角相對比較大），這是素材本身的差異、不是對位跑掉；
對齊基準是**方形本體的外緣**，那才是玩家看到的「格子」。
...
- **`ForgingPanel_Btn.png` 沒去背** → 按鈕會露出灰底方塊（見 §3）。
