...
- **自動生成可走區（2026-09-07）**：面板下方「── 自動生成（依背景圖）──」一區。流程＝**① 點一塊地板當種子 → ② 拖容差滑桿（即時預覽）→ ③ 套用**。
  ...
  **預覽中可以直接用筆刷改**（改的是預覽本身，塗完立刻變色，改完再一起套用）——自動生成一定有猜錯的地方（例如可破壞的桌子擋在路中間、破壞後才該能走，影像上分不出來），這樣就不必「套用→再修→再看」跑兩趟。預覽中不能塗水/坑（遮罩只有兩態），要塗請先套用。
  ...
  - **⚠ 有天花板**：背景圖裡沒有「可走」這個資訊，只有顏色。兩類東西永遠猜不到——(a) 牆的受光面跟地板同色同亮（邪佛廣場四周的山壁就是）；(b) 你**刻意**讓某塊深色地面可走／某塊像地板的地方不可走（那是設計意圖，不在圖裡）。
