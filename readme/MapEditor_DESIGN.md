...
- **自動生成可走區（2026-09-07）**：面板下方「── 自動生成（依背景圖）──」一區。流程＝**① 點一塊地板當種子 → ② 拖容差滑桿（即時預覽）→ ③ 套用**。
  預覽色：**綠＝可走、紅＝牆**——直接畫「套用後會變成什麼」，語意與平常的筆刷完全一致（只是濃一點）；黃十字＝種子點。套用是一次 Undo，不滿意可以 Cmd/Ctrl+Z。
  **預覽中一動筆刷，會先自動把預覽套用進資料、再照常塗**（一次 Undo）——自動生成一定有猜錯的地方（例如可破壞的桌子擋在路中間、破壞後才該能走，影像上分不出來），所以看到不對直接塗就好，不必先按套用。
  > ⚠ **不要改回「筆刷只改預覽、按套用才寫入」**：那樣做過一次，當天就造成整張圖的可走層遺失（塗的東西留在沒套用的記憶體遮罩裡、存檔存到舊資料，進遊戲角色卡死）。詳見 [PROBLEMS.md](PROBLEMS.md) **C11**。同理，**換地圖要清掉預覽、存檔前要自動套用預覽**，三道缺一不可。
  > ⚠ **不要再做「差異色」**：一度用「青＝會變可走／橘＝現在可走但這次沒抓到」，結果作者拿綠筆去塗橘色區域、顏色卻不變（那格仍然同時滿足「現在可走」與「預覽沒抓到」兩個條件），完全誤導。**預覽該顯示的是結果，不是差異。**
  - **定位是「一鍵鋪好底稿」，不是「一鍵完成」**：拿全庫 22 張手塗的可走層實測，平均 IoU 76%；但**錯誤集中在少數大塊**（最大 3 塊佔 73%），生成後平均再補 **6 筆**左右就好（最多 14 筆）。比起「從全牆開始整張手塗」是數量級的差別。
  - **演算法與為什麼是這幾步**寫在 `Core/WalkableAutoGen.cs` 檔頭（色度區域生長＋侵蝕→取連通塊→膨脹）。三個實測結論：用亮度門檻會有 5 張圖**完全失敗**（「地板比較亮」在室內不成立）；用色度（去亮度）比 RGB 好，因為陰影下的同一片地板色度不變；**多種子點反而更差**（等於放寬條件、洩漏到牆上），所以只吃單一種子。
  - **⚠ 有天花板**：背景圖裡沒有「可走」這個資訊，只有顏色。三類情況永遠猜不到——(a) 牆的受光面跟地板同色同亮（邪佛廣場四周的山壁就是）；(b) 你**刻意**讓某塊深色地面可走／某塊像地板的地方不可走（那是設計意圖，不在圖裡）；(c) **整張圖的牆與地板同材質**，例如 `BloodFang_TrainingGround`（白色大理石地板＋白色大理石欄杆與石柱）實測容差 16→0%、18→直接跳到 86.6%，中間沒有任何可用區間，綠色會蓋滿包含牆在內的整張圖。**遇到 (c) 就別用自動生成，手塗還比較快。**
  - **⚠ 套用是整張覆蓋**，水/坑 `'2'` 也會被沖掉 → **先自動生成，再塗水**，不要反過來。
  - 沒有背景圖的地圖不顯示這一區（無從判斷）。
- **試走（2026-09-07）**：頂部列「試走」鈕。**WASD／方向鍵**走、**Tab** 顯示/隱藏可走層疊加、**Esc** 結束。
  不必存檔→Sync→進主遊戲，塗完當場就能走。刻意與主遊戲對齊三件事：碰撞用**圓**（半徑同 `CircleCollider2D`）、**分軸移動**（沿牆滑動而不是卡死）、相機 `orthographicSize` 固定成跟隨模式的值（畫面高 10 格）並夾在地圖邊界內——**視野一樣，才看得出「這條路在遊戲裡會不會太窄」**。
  起點取 `playerSpawn` trigger，沒有就地圖中心；落在牆上會自動往外找最近的可走格。
  **⚠ 只判斷可走層，不含地上物碰撞**（那是主遊戲 `MapLoader` 依素材 alpha 烘的，編輯器沒有那套）——所以擋路的家具在這裡走得過去是正常的，不是 bug（見 [PROBLEMS.md](PROBLEMS.md) **B9**：擋路碰撞與可走層本來就是兩份獨立的真相）。
