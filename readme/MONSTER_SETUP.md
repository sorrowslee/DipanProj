...
| **`Controllable`** | **可被控制**（表尾，2026-09-22）：這隻怪能不能被「定身類」控制效果抓住。目前唯一的來源是骨牢（玩家武器 `Mode=Cage`，見 [RECIPE_DESCRIBE.md](RECIPE_DESCRIBE.md)）。<br>**留空／1 ＝ 可以被控制**（既有怪零影響）；boss 與強怪填 **0** 就免疫。<br>⚠ 方向刻意是「預設可控、逐隻標免疫」而不是反過來——否則每加一隻新怪都要記得填，忘了就抓不住，而且沒有任何錯誤訊息。 |
| **`ObserveTime`** | **射手型觀察秒數**（表尾，2026-09-23；僅 `BrainType=Archer`）：每次「站定評估」停多久。<br>**留空 ＝ 0.4~0.8 秒隨機**（既有怪零影響）；**填 0 ＝ 不觀察**，一進射程、視線通就立刻舉槍，且取消生成後的武器起手緩衝；填正數 ＝ 固定秒數。<br>目前只有新手夢境教學的 32 號 ZhaYu_Gun 填 0。細節見 [BOSS_MODULE.md](BOSS_MODULE.md) §8.1。 |
