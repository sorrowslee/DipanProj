...
| `Atmosphere` | 1~6 | **氛圍後處理**。`1` = 正常；`2` = 幽暗+打光（周邊仍看得到美術）；`3` = 噩夢+打光（周邊近全黑、最壓迫）；`4` = 烈日曝曬；`5` = 焦土餘燼；`6` = 沙塵暴（4/5/6 為末日炎熱系、帶熱浪扭曲）。**留空 / 缺欄 = 預設 1。** 換地圖即時切換，所以可「室外白天 → 傳送 → 古墓變陰森」。詳見 [ATMOSPHERE.md](ATMOSPHERE.md)。 |
