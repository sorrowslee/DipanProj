...
| `SceneTip` | 字串 | **場景說明文字圖的 key**（第 12 欄）。進圖後在畫面上方淡入一次場景名（金色毛筆字＋血紅分隔線），約 2.6 秒後自動淡出；**整段暫停遊戲、鎖操作**，播完才放行這張圖的進場自動劇情、再點火進場觸發（對話/教學）。**留空 / 缺欄 = 這張圖不顯示（舊地圖零行為變化）。** 圖 = `Resources/UI/Texts/SceneTipPanel_Text_<key>`（語言資料夾自動解析）。⚠️ **key 不是 `Name`**：`Name` 是內部名（`Main_Square`）、key 是美術命名（`BuddhaSquare`），刻意不綁在一起。⚠️ **同一趟關卡同一個 key 只顯示一次**，所以整個關卡的房間可以全部填同一個 key。目前填的是紅嫁衣 10 張全填 `RedBridalGown`、`Main_Square`(12) 填 `BuddhaSquare`；初始森林 1/2 與初始洞窟刻意留空。詳見 [SCENE_TIP.md](SCENE_TIP.md)。 |
...
### 2.3 螢幕震動（`MapCameraController.AddShake`）

`AddShake(秒數, 振幅世界單位)` 讓鏡頭抖一下（擊中、爆炸這類衝擊感），`StopShake()` 立刻停。
目前的使用者是血統變身演出的雷擊瞬間（0.25 秒 / 0.16 單位）。

四個要點（都是踩過或想清楚才這樣寫的）：

- **必須做在 `MapCameraController` 裡，不可另外掛一個「震屏元件」。** 相機位置每幀由這支的
  `LateUpdate` 獨佔寫入，兩邊都寫 `transform.position` 會互相蓋掉，而誰先跑取決於
  Script Execution Order ——那是抓不到的隨機 bug。
- **偏移疊在 `LateUpdate` 最末端、`SmoothDamp` 之後。** 若把偏移加進 `desired` 再平滑，
  高頻抖動會被平滑整個吃掉，看起來只是鏡頭「軟軟地飄一下」而不是震。
  也因為不寫回 `_vel` / `desired`，震完自然回到正確位置，不會累積漂移。
- **振幅線性衰減；重複呼叫只取「較強的那一次」，不累加也不延長。** 延長已經衰減到一半的震動
  會讓它彈回滿強度（看起來像抖到一半又暴衝一次）。
- **計時用 `unscaledDeltaTime`**：演出可能在暫停狀態下播。

不影響既有的鏡頭跟隨、鏡頭區（`camZone`）與對準點（`SetFocusPoint`）——那些都在前面算完了，
震動只是最後疊上去的一個偏移。
...
- **對位改成所見即所得（2026-08-20）**：門的美術畫在背景圖裡，資料裡沒有門的位置，**外型對齊無法自動化**。編輯器改為提供「傳送點對位」模式：把遊戲真正的傳送點特效畫在畫布上、整張圖一次全顯示、直接拖曳定位。遊戲端**完全不變**（仍只讀 markerX/markerY）。見 [MapEditor_DESIGN.md](MapEditor_DESIGN.md) §4.5。
- **⭐ 傳送點改成「點模式」：錨點＝外型＝踩踏區＝落點（2026-08-20）**。正典在 `Map/TeleportAnchor.cs`。
  - **為什麼**：格子同時兼三個職務（踩踏／落點／外型中心），但**只有整格精度**，而門畫在背景圖裡、位置任意 ⇒「看到的門」與「實際會傳送的地方」必然分家（作者原話：「我放傳送點的位置不等於真正玩家踩到就會傳送的點」）。
  - **現在**：`markerX`/`markerY` 是錨點（外型光盤畫在這、落點也是這），`markerW`/`markerH` 是以錨點為中心的**踩踏矩形**（預設 1.0×0.6，門通常寬而矮）。判定點是 **`transform.position`**（＝碰撞圓心，物理與可走層都用它）。
  - **落點安全**：錨點可能落在門上（牆裡），但 `MapManager.ResolveSpawnPos` 外面包了 `FreeSpotNear`，被擋住會自動推到最近的可站處，推出來的正好是門前那塊地板。**這是這個改動可行的關鍵前提。**
  - **⚠ 判定點不要換成腳底**：試過，結果是牆邊的傳送點結構上不可能被觸發（腳底比碰撞圓心低約一整格，而牆擋的是圓心 ⇒ 腳底永遠靠不近牆一格以內，門就在牆上）。見 [PROBLEMS.md](PROBLEMS.md) **B13**。
  - **向下相容**：沒設錨點的傳送點自動走舊的「格子模式」，兩種可同圖並存。有錨點時**刻意不建它的格表**，否則同一個傳送點會有兩個觸發區、其中一個還看不見。
  - **已全數轉換（28 個）**：錨點＝原格子平均中心、矩形＝格子外接框 +0.2。**等價於原本的格子行為**，之後在編輯器逐一拖到門上即可。已有錨點的 5 個沿用原值。
  - 相依檢查過都不會斷：觸發鏈啟用/停用、`linkedFx` 綠幕、`togglePortal` 全走 region id/名稱；`cameraFocus` 是另一種 trigger。
  - **診斷**：遊戲中按 P 再按 C，會畫出踩踏矩形（青綠）＋玩家碰撞圓（黃）＋判定點讀數。
