...
| `Atmosphere` | 1/2/3 | **氛圍後處理**。`1` = 正常（不做處理，室外白天等）；`2` = 幽暗場景+打光（壓暗+冷色+提燈光圈，周邊仍看得到美術）；`3` = 噩夢場景+打光（周邊近全黑、最壓迫）。**留空 / 缺欄 = 預設 1。** 換地圖即時切換，所以可「室外白天 → 傳送 → 古墓變陰森」。詳見 [ATMOSPHERE.md](ATMOSPHERE.md)。 |
