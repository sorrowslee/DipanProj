...
>
> **狀態：✅ 程式 + 真素材完成（2026-06-23），待實機微調座標。** 5 分頁資料層、存檔、點擊＋拖放、切頁、排序皆完成；UI 已接上拆好的素材（外框 StoragePanelBG ＋ CellBG_normal/pressed ＋ Cell_number_1~5 ＋ RefreshBG_normal/pressed ＋ RefreshIcon）。座標於 2026-09-14 **重新量過並改成由欄列數自動排版**（格網 5×5、頁籤對齊每一欄、格線改由程式鋪），見〈格子與頁籤的版面〉。
>
...
| 每頁格數 | `StorageSystem.DefaultCols×DefaultRows` ＝ **5×5**（2026-09-14 由 10×10 改）。**改這兩個數字就換掉整個倉庫版面**——格子、格線、頁籤位置都跟著算 |
...
- **hover**：倉庫與背包**行為一致**——滑過格子有**金色細外框**（`SlotOutline`，只描邊不填滿）＋ 浮動 tooltip。tooltip 自 2026-09-14 起是**共用元件 `UI/ItemTooltip.cs`**（與背包／鍛造同一份）：大圖預覽＋名稱＋功能＋劇情，一律開在游標上方。倉庫格子傳的是整個 `ItemStack`，所以**鑲嵌內容與珠子等級也看得到**（以前只傳 itemId，那些資訊顯示不出來）。
...
## 格子與頁籤的版面（2026-09-14 重做）

**這一區 2026-09-14 之前是壞的**：`GridX0=167`、`CellW=84.8`、10×10、頁籤 x 寫死 `252/421/591/760/930`——
這些值沒有真的對過底圖，所以格子與頁籤整片往右下溢出到外框之外（作者實機截圖才發現）。

**底圖實際量出來的值**（`StoragePanelBG.png`，**1122×1402**；掃描亮度找格線峰值）：

| 項目 | 值 |
|---|---|
| 底圖畫死的格線 | **10 欄 × 9 列**（不是 10×10） |
| 垂直線中心 | 196 / 268.6 / 341 / … / 921（節距 **72.6**） |
| 水平線中心 | 408 / 484.5 / 561 / … / 1096（節距 **76.5**） |
| 格線區 | x **196~921**、y **408~1096**（725 × 688） |

**格線改由程式鋪**（與背包同一套做法，見 [INVENTORY.md](INVENTORY.md)〈道具區的格線是程式鋪的〉）：

| 素材 | 尺寸 | 做什麼 |
|---|---|---|
| `StoragePanel_GridBlank` | 740×702 | 貼在 `(190,400)`，蓋掉底圖畫死的 10×9 格線。內容是從 90 個格子的格內乾淨區隨機拼貼的，連木紋噪點都是原圖那批 |
| `StoragePanel_CellFrame` | 73×77 | 一個格單元（從 `341,561~414,638` 裁下），左右上下各含半條格線，平鋪就還原原圖的線與鉚釘 |

**排版算式**（`CellPitch` / `CellTopLeft`，與背包同形）：

```
可用區 = 格線區 (196,408)~(921,1096) 各內縮 GridPad=5  →  715 × 678
節距   = min(715 ÷ 欄數, 678 ÷ 列數)      ← 等比方格，整組置中
格內容 = 節距 × CellInnerRatio(0.90)      ← 命中區與 icon 用這個大小
```

**為什麼是 5×5**：作者要求倉庫格子**與背包一樣大**。背包一格在螢幕上約 89 單位
（底圖 124px × 900/1254）；倉庫底圖縮放是 `FrameScale = 0.72`，所以

| 版面 | 節距 | 格內容 | 螢幕單位 | 對背包 |
|---|---|---|---|---|
| 10×10（舊） | 84.8 | 約 85 | **61** | 小一大截 |
| 5×4 | 143 | 129 | 93 | +4%，但上下各空一條 |
| **5×5（現行）** | **135.6** | **122** | **88** | **−1%，且剛好填滿** |

容量因此由每頁 100 降到 **25**（五頁共 125）。⚠ 這會動到存檔，見下一節。

**頁籤不再寫死 x**：改成對齊每一欄的中心（`CellTopLeft(i,0).x + CellPitch/2`），剛好 5 欄對 5 頁；
寬度 ＝ 一欄的 92%、高度照素材比例（`CellBG_normal` 612×408）算。所以欄數改了頁籤也自動跟上。

### ⚠ 縮小容量會動到存檔

`ItemGridData.RestoreFrom` 原本會**用存檔裡的 `cols/rows` 覆寫程式的設定**，所以改小格數之後，
舊存檔一載入就把格數變回 10×10，UI 只畫得出 25 格、資料卻有 100 格 ⇒ 後面 75 格拿不出來。
2026-09-14 已把那一行拿掉（格數一律以 `StorageSystem` 為準），詳見 [PROBLEMS.md](PROBLEMS.md) **D25**。

超出新容量的物品會走既有的 `AddStack` 塞回前面的空位；**真的塞不下的會消失**——
手上的存檔若某一頁裝超過 25 件，改容量前先清一清。
